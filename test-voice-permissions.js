...
async function testPermissions() {
  ...
  try {
    ...
    console.warn('⚠️ Could not check permissions (this is normal in some browsers)');
    return null;
  }
}
...
async function testMicrophoneAccess() {
  ...
  try {
    ...
    audioTracks.forEach((track, index) => {
      console.log(`   Track ${index}: ${track.label} - enabled: ${track.enabled}, state: ${track.readyState}`);
    });
...
async function testCameraAccess() {
  ...
  try {
    ...
    videoTracks.forEach((track, index) => {
      ...
      console.log(`   Track ${index}: ${track.label} - ${settings.width}x${settings.height}@${settings.frameRate}fps`);
    });
...
async function testCombinedAccess() {
  ...
  try {
    ...
    console.log(`🎬 Total tracks: ${stream.getTracks().length} (${stream.getAudioTracks().length} audio, ${stream.getVideoTracks().length} video)`);
    ...
    console.error(`❌ Combined access failed: ${error.name} - ${error.message}`);
    return false;
  }
}
...
async function runAllTests() {
  ...
  console.log(`MediaRecorder Support: ${results.mediaRecorderSupport ? '✅' : '❌'}`);
  ...
  const allPassed = results.apiAvailable && results.microphoneAccess && results.mediaRecorderSupport;
...
module.exports = { runAllTests, testMicrophoneAccess, testCameraAccess, testMediaRecorderSupport };