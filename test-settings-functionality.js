...
async function testSettingsAPI() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const requiredFields = ['sarcasm', 'verbosity', 'humor', 'voice_responses', 'ambient_listening', 'sound_effects'];
...
async function testUserProfile() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('User data:', { id: data.id, name: data.name, email: data.email });
      return { success: true, data };
    } else {
      const errorText = await response.text();
...
function testFrontendForms() {
  ...
  const settingsElements = {
    ...
    personalityInputs: document.querySelectorAll('input[type="range"], select').length,
    profileInputs: document.querySelectorAll('input[type="text"], input[type="email"]').length,
...
async function testVoiceSettings() {
  ...
  try {
    ...
    if (response.ok || response.status === 404) { // 404 is acceptable - means endpoint exists
...
async function runAllSettingsTests() {
  ...
  const testResults = [
    ...
    ['Frontend Forms', results.frontendForms.audioForm && results.frontendForms.personalityForm],
  ...
  if (overallSuccess) {
    ...
    if (!results.voiceSettings.mediaSupport) {
      console.log('- Browser does not support media devices (use Chrome/Firefox)');
    }
  }
...
module.exports = { runAllSettingsTests, testSettingsAPI, testAudioSettingsUpdate };