-- 32_workflow_advanced_scheduling_schema_corrected.sql
-- Component 5: Advanced Scheduling System - Database Schema (Corrected)
-- Fixed to work with existing integer-based workflow IDs

-- Create workflow_schedules table with integer IDs to match existing schema
...
CREATE TABLE IF NOT EXISTS workflow_schedule_executions (
    ...
    execution_id INTEGER, -- Reference to workflow execution if exists
...
CREATE OR REPLACE FUNCTION calculate_schedule_health_score(schedule_id_param INTEGER)
RETURNS numeric AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION get_schedule_next_execution(schedule_id_param INTEGER)
...
COMMENT ON FUNCTION calculate_schedule_health_score(INTEGER) IS 'Calculates a health score (0-100) for a workflow schedule based on success rate, error frequency, and performance metrics';
COMMENT ON FUNCTION get_schedule_next_execution(INTEGER) IS 'Estimates the next execution time for a workflow schedule based on its configuration';
