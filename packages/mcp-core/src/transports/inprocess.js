...
export class MCPInProcessTransport {
    ...
    constructor(options = {}) {
        ...
        if (options.maxListeners)
            this.bus.setMaxListeners(options.maxListeners);
    }
    publish(raw) {
        ...
        try {
            this.bus.emit(msg.recipient, msg);
        }
        finally {
            span.end();
        }
    ...
    subscribe(recipient, handler) {
        ...
        this.bus.on(recipient, wrapped);
        this.subs.push({ recipient, handler });
        return () => this.bus.off(recipient, wrapped);
    }
