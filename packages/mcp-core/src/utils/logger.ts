...
export class Logger {
   ...
   */
  static create(name: string, options: Omit<LoggerOptions, 'name'> = {}): Logger {
   ...
   */
  private log(level: string, message: string, error: Error | null = null, context: LogContext = {}): void {
   ...
   * Log audit event
   */
  audit(action: string, resource: string, actor: string, context: LogContext = {}): void {
...
export const log = {
  ...
  fatal: (message: string, error?: Error, context?: LogContext) => defaultLogger.fatal(message, error, context),
  trace: (message: string, context?: LogContext) => defaultLogger.trace(message, context),
  event: (eventName: string, eventData?: LogContext) => defaultLogger.event(eventName, eventData),
  metric: (metricName: string, value: number, labels?: LogContext) => defaultLogger.metric(metricName, value, labels),
  audit: (action: string, resource: string, actor: string, context?: LogContext) => defaultLogger.audit(action, resource, actor, context),
};
