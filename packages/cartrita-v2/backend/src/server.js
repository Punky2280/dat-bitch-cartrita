...
import chatHistoryRoutes, {
  initializeChatAIHubService,
} from './routes/chatHistory.js';
...
const limiter = rateLimit({
  ...
  skip: req => {
    return (
      req.path.includes('/health') || req.path.includes('/api/auth/verify')
    );
  },
});
...
// V2 System Health endpoints
app.get('/api/v2/system/health', (_req, res) =>
  res.json({
    success: true,
    data: {
      status: 'healthy',
...
app.get('/api/v2/system/health/detailed', (_req, res) =>
  res.json({
    success: true,
    data: {
      status: 'operational',
...
// V2 Security dashboard endpoint (authenticated)
app.get('/api/v2/security/dashboard', authenticateToken, async (req, res) => {
  try {
    const dashboard = {
...
// Legacy V1 endpoint (maintained for compatibility)
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      setTimeout(() => {
        console.log(
          `[Security] ${scanType} scan ${scanId} completed for ${target}`
        );
      }, 2000);
...
// V2 Security events endpoint (authenticated)
app.get('/api/v2/security/events', authenticateToken, async (req, res) => {
  try {
...
// Legacy V1 Security events endpoint (authenticated)
...
// V2 Security configuration endpoint (authenticated)
app.get('/api/v2/security/config', authenticateToken, async (req, res) => {
  try {
    const config = {
      rateLimiting: {
        enabled: true,
...
// Legacy V1 Security configuration endpoint (authenticated)
...
// V2 Knowledge Hub status and statistics
app.get('/api/v2/knowledge/status', authenticateToken, async (req, res) => {
  try {
    const stats = {
...
// Legacy V1 Knowledge Hub status and statistics
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `[Knowledge Hub] Starting ingestion ${ingestionId} from ${source}: ${data.length} items`
      );
...
app.get('/api/rotation-test', authenticateToken, async (req, res) => {
  try {
    const { default: RotationSchedulingService } = await import(
      './services/RotationSchedulingService.js'
    );
    const policies = await RotationSchedulingService.getRotationPolicies(
      req.user.id
    );
    res.json({
      success: true,
...
app.post('/api/knowledge/search', async (req, res) => {
  try {
    ...
    const result = await KnowledgeHubService.semanticSearch(
      query,
      userId,
      options
    );
...
app.post('/api/knowledge/cluster', async (req, res) => {
  try {
    ...
    const result = await KnowledgeHubService.performClustering(
      userId,
      num_clusters
    );
...
app.post('/api/voice/ambient/stop', async (req, res) => {
  try {
    ...
    if (!session_id) {
      return res
        .status(400)
        .json({ success: false, error: 'session_id required' });
    }
...
app.post('/api/voice/ambient/audio', async (req, res) => {
  try {
    ...
    if (!session_id || !audio_data) {
      return res
        .status(400)
        .json({ success: false, error: 'session_id and audio_data required' });
    }
    const result = await AmbientVoiceService.processAudioChunk(
      session_id,
      audio_data,
      metadata
    );
...
app.post('/api/vision/analyze', async (req, res) => {
  try {
    ...
    if (!images) {
      return res
        .status(400)
        .json({ success: false, error: 'images array required' });
    }
...
app.post('/api/vision/generate', async (req, res) => {
  try {
    ...
    const result = await VisionAnalysisService.generateImage(
      prompt,
      options || {}
    );
    res.json(result);
...
app.post('/api/vision/compare', async (req, res) => {
  try {
    ...
    const result = await VisionAnalysisService.compareImages(
      images,
      options || {}
    );
    res.json(result);
...
app.post('/api/vision/extract-text', async (req, res) => {
  try {
    ...
    const result = await VisionAnalysisService.extractText(
      images,
      options || {}
    );
    res.json(result);
...
app.get('/api/agent/metrics', (req, res) => {
  res.json({
    success: true,
    metrics: { active_agents: 0, total_requests: 0, uptime: Date.now() },
  });
});
...
if (!global.aiHubServiceInitialized) {
  try {
    ...
    // Initialize chat routes with AI Hub service
    initializeChatAIHubService(aiHubService);
    console.log('[AI-Hub] ✅ AIHubService connected to chat routes');
...
app.get('/api/huggingface/test', (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: 'HF integration test failed' });
  }
});
...
app.get('/api/huggingface/models', (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: 'Models listing failed', models: [] });
  }
});
...
app.post('/api/huggingface/chat/completions', async (req, res) => {
  try {
    const { messages, model, provider, temperature, max_tokens, stream } =
      req.body;
    if (!messages) {
      return res
        .status(400)
...
app.post('/api/huggingface/audio', async (req, res) => {
  try {
    ...
    if (!audio) {
      return res
        .status(400)
        .json({ success: false, error: 'audio data required' });
    }

    const result = await HuggingFaceRouterService.speechToText({
      audio,
      model,
    });
    res.json(result);
...
app.post('/api/huggingface/embeddings', async (req, res) => {
  try {
    ...
    const result = await HuggingFaceRouterService.createEmbeddings({
      inputs,
      model,
    });
    res.json(result);
...
app.post('/api/huggingface/rag', async (req, res) => {
  try {
    ...
    console.log(
      `[RAG Pipeline] Processing query: "${query.substring(0, 100)}..."`
    );
    console.log(
      `[RAG Pipeline] Document store: ${documentStore?.length || 0} documents`
    );
    console.log(
      `[RAG Pipeline] Models: ${generationModel || model || 'deepseek-v3'}`
    );
    ...
    if (result.success) {
      ...
      const ragResult = {
        ...
        context_used:
          documentStore?.slice(0, 5).map((doc, idx) => ({
            document: {
              name: `Document ${idx + 1}`,
        ...
        sources:
          documentStore?.slice(0, 3).map((doc, idx) => ({
            id: idx + 1,
  ...
  try {
    ...
    if (!task || !input) {
      return res.status(400).json({
        success: false,
        error:
          'task and input are required. Available tasks: text-classification, zero-shot, ner, qa, generation, asr, vision-detection, vision-nsfw',
      });
    }

    console.log(
      `[MultiProviderAI] Inference request: task=${task}, preferredModels=${preferredModels?.join(',') || 'auto'}`
    );
...
app.post('/api/unified/inference', async (req, res) => {
  try {
    ...
    console.log(
      `[UnifiedAI] Inference request: task=${task}, model=${options?.model || 'auto'}, provider=${options?.provider || 'auto'}`
    );
