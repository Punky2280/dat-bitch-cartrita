...
Create for: "${task}" with artistic excellence and commercial impact.
  ...
  selectOptimalPrompt(request, userContext = {}, systemContext = {}) {
    ...
    const optimalArchitecture = this.selectReasoningArchitecture(
      analysis,
      complexity
    );
    ...
    if (analysis.requiresSpecialist) {
      return this.generateAgentSpecificPrompt(
        analysis.bestAgent,
        request,
        context
      );
    ...
    return (
      this.reasoningArchitectures[optimalArchitecture].call(this, context) +
  ...
  assessComplexity(request, userContext) {
    ...
    if (request.includes('analyze') || request.includes('research'))
      complexity += 1;
...
export const generateCartritaPrompt = (
  request,
  userContext = {},
  systemContext = {}
) => {
  return cartritaPromptSystem.selectOptimalPrompt(
    request,
    userContext,
    systemContext
  );
...
export const generateAgentPrompt = (agentName, task, context = {}) => {
  return cartritaPromptSystem.generateAgentSpecificPrompt(
    agentName,
    task,
    context
  );
