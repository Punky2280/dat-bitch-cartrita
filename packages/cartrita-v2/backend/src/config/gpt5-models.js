...
export function getOptimalModelForAgent(agentName) {
  ...
  if (!assignment) {
    console.warn(
      `[GPT5Models] No specific assignment for agent '${agentName}', using default GPT5_MINI`
    );
    return {
      ...GPT5_MODELS.GPT5_MINI,
...
export function updateAgentModelAssignment(agentName, modelConfig) {
  ...
  console.log(
    `[GPT5Models] Updated model assignment for ${agentName}: ${modelConfig.model}`
  );
}
...
export function getModelFeatures(modelName) {
  const modelConfig = Object.values(GPT5_MODELS).find(
