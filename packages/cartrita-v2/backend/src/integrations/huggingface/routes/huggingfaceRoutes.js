...
(async () => {
  try {
    ...
    console.log(
      '[HF Routes] 🎭 Agent orchestrator initialization:',
      orchestratorReady ? 'SUCCESS' : 'FAILED'
    );
  } catch (error) {
    console.error(
      '[HF Routes] ❌ Failed to initialize agent orchestrator:',
      error
    );
  }
})();
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (labels)
        inputs.labels = Array.isArray(labels)
          ? labels
...
router.post(
  ...
  async (req, res) => {
    try {
      const { taskType = 'automatic-speech-recognition', options = {} } =
        req.body;
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (question) inputs.question = question;
      if (sentences)
        inputs.sentences = Array.isArray(sentences) ? sentences : [sentences];
...
router.get('/tasks', requireOrchestrator, async (req, res) => {
  try {
    ...
    const taskCategories = {
      ...
      audio: availableTasks.filter(
        t => t.includes('audio') || t.includes('speech') || t.includes('voice')
      ),
      text: availableTasks.filter(
        t => t.includes('text') && !t.includes('image') && !t.includes('audio')
      ),
      multimodal: availableTasks.filter(
        t =>
          t.includes('visual-question') ||
          t.includes('document-question') ||
          t.includes('multimodal')
      ),
      data: availableTasks.filter(
        t =>
          t.includes('tabular') ||
          t.includes('time-series') ||
          t.includes('data')
      ),
    };
