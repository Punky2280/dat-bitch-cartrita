...
export default class HFDataAgent {
  ...
  async execute(message, language, userId) {
      ...
      if (stored && !datasetBuffer) {
        if (
          ...
          if (global.otelCounters?.hfTokenMisuse) {
            try {
              global.otelCounters.hfTokenMisuse.add(1, { modality: 'data' });
            } catch (_) {}
          }
          continue;
        }
    ...
    return await OpenTelemetryTracing.traceAgentOperation(
      'DataSage',
      'bridge.execute',
      { 'hf.modality': 'data', 'user.id': userId || 'anonymous' },
      async () => {
        ...
        try {
          if (lower.includes('forecast') || lower.includes('time series')) {
            structured = {
              task: 'data.forecast',
              status: 'ok',
              data: { forecast: 'placeholder', hasDataset: !!datasetBuffer },
            };
          } else if (lower.includes('regression')) {
            structured = {
              task: 'data.regression',
              status: 'ok',
              data: { model: 'placeholder', r2: 0.9 },
            };
          } else if (
            ...
            structured = {
              ...
              status: 'ok',
              data: { classes: ['A', 'B'], distribution: { A: 0.6, B: 0.4 } },
            };
          } else if (lower.includes('analyze') || lower.includes('data')) {
            structured = {
              task: 'data.summary',
              status: 'ok',
              data: { rows: datasetBuffer ? 100 : null, columns: 10 },
            };
          } else {
            structured = {
              task: 'data.generic',
        ...
        const summary =
          structured.status === 'error'
            ? `Data error: ${structured.error}`
            : `Data ${structured.task} complete.`;
        return {
