...
export default class HFVisionAgent {
  ...
  async execute(message, language, userId) {
    ...
    const dataUriMatch = message?.match(
    ...
    if (
      !imageBuffer &&
      tokenMatch &&
      global.hfBinaryStore?.has(tokenMatch[1])
    ) {
      ...
      async () => {
        ...
        const summary =
          structured.status === 'error'
            ? `Vision error: ${structured.error}`
            : `Vision ${structured.task} complete.`;
        return {
