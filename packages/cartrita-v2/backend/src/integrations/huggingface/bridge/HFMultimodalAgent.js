...
export default class HFMultimodalAgent {
  ...
  async execute(message, language, userId) {
    ...
    const audMatch = message?.match(
      /data:audio\/(wav|mpeg|mp3|ogg);base64,([A-Za-z0-9+/=]+)/
    );
    ...
    for (const t of tokenMatches) {
      ...
      if (stored) {
        ...
        if (stored.type.startsWith('image/') && !imageData)
          imageData = stored.buffer;
        if (stored.type.startsWith('audio/') && !audioData)
          audioData = stored.buffer;
      }
    }
    return await OpenTelemetryTracing.traceAgentOperation(
      'MultiModalOracle',
      'bridge.execute',
      { 'hf.modality': 'multimodal', 'user.id': userId || 'anonymous' },
      async () => {
        let structured = {
          task: 'multimodal.analysis',
          status: 'ok',
          data: {},
        };
        try {
          if (lower.includes('visual question') || lower.includes('vqa')) {
            structured = {
              task: 'multimodal.vqa',
              status: 'ok',
              data: { answer: 'placeholder', image: !!imageData },
            };
          } else if (lower.includes('document') && lower.includes('question')) {
            structured = {
              task: 'multimodal.doc_qa',
              status: 'ok',
            ...
            structured = {
              task: 'multimodal.cross_modal',
              status: 'ok',
              data: { image: !!imageData, audio: !!audioData },
            };
          } else {
            structured = {
              task: 'multimodal.generic',
              status: 'ok',
        ...
        const summary =
          structured.status === 'error'
            ? `Multimodal error: ${structured.error}`
            : `Multimodal ${structured.task} complete.`;
        return {
