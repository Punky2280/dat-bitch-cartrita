...
export default class VisionMasterAgent {
  constructor() {
    ...
    this.personality =
      'Analytical and detail-oriented visual intelligence expert';
      ...
      switch (analysisType) {
        case 'classification':
          results.results.classification =
            await this.hfService.imageClassification(imageData, options);
          break;

        case 'objects':
          results.results.objects = await this.hfService.objectDetection(
            imageData,
            options
          );
          break;
        ...
        case 'description':
          results.results.description = await this.hfService.imageToText(
            imageData,
            options
          );
          break;
        ...
        case 'depth':
          results.results.depth = await this.hfService.depthEstimation(
            imageData,
            options
          );
          break;
        ...
        case 'segmentation':
          results.results.segmentation = await this.hfService.imageSegmentation(
            imageData,
            options
          );
          break;
        ...
        case 'comprehensive':
          ...
          const [classification, objects, description] =
          ...
          results.results.classification =
            classification.status === 'fulfilled' ? classification.value : null;
          results.results.objects =
            objects.status === 'fulfilled' ? objects.value : null;
  ...
  async answerVisualQuestion(imageData, question, options = {}) {
    ...
    const result = await this.hfService.visualQuestionAnswering(
      imageData,
      question,
      options
    );
  ...
  async classifyWithLabels(imageData, labels, options = {}) {
    ...
    const result = await this.hfService.zeroShotImageClassification(
      imageData,
      labels,
      options
    );
