...
export default class AudioWizardAgent {
  constructor() {
    ...
    this.personality =
      'Sophisticated audio engineer with deep understanding of sound and speech';
  ...
  async detectVoiceActivity(audioData, options = {}) {
    ...
    try {
      ...
      return {
        agent: this.name,
        hasVoice: result.some(
          r => r.label.includes('speech') || r.label.includes('voice')
        ),
        segments: result,
      ...
      const hasVoice = result.classifications.some(
        c =>
  ...
  async analyzeAudio(audioData, analysisType = 'comprehensive', options = {}) {
    ...
    try {
      switch (analysisType) {
        case 'transcription':
          results.results.transcription = await this.transcribeAudio(
            audioData,
            options
          );
          break;
        ...
        case 'classification':
          results.results.classification = await this.classifyAudio(
            audioData,
            options
          );
          break;
        ...
        case 'voice-detection':
          results.results.voiceActivity = await this.detectVoiceActivity(
            audioData,
            options
          );
          break;
        ...
        case 'comprehensive':
          ...
          const [transcription, classification, voiceActivity] =
          ...
          results.results.transcription =
            transcription.status === 'fulfilled' ? transcription.value : null;
          results.results.classification =
            classification.status === 'fulfilled' ? classification.value : null;
          results.results.voiceActivity =
            voiceActivity.status === 'fulfilled' ? voiceActivity.value : null;
          break;
