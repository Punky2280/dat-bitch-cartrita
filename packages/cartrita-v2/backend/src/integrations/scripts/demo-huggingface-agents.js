...
async function demonstrateAgent(agent, agentName) {
  ...
  try {
    ...
    if (typeof agent.getCapabilities === 'function') {
      ...
      console.log(
        `   Specializations: ${capabilities.specializations.join(', ')}`
      );
...
async function demonstrateLanguageCapabilities(agent) {
  ...
  try {
    ...
    console.log(
      `✅ Text Generation:`,
      generationResult.generatedText.substring(0, 100) + '...'
    );
  } catch (error) {
...
async function demonstrateDataCapabilities(agent) {
  ...
  try {
    ...
    const analysisResult = await agent.analyzeTabularData(
      sampleData,
      'comprehensive'
    );
    console.log(
      `✅ Data Summary:`,
      `${analysisResult.dataShape.rows} rows, ${analysisResult.dataShape.columns} columns`
    );
    console.log(
      `📊 Recommendations:`,
      analysisResult.results.recommendations.slice(0, 2).join('; ')
    );
    ...
    const forecastResult = await agent.forecastTimeSeries(timeSeriesData, {
      periods: 3,
    });
    console.log(
      `✅ Time Series Forecast: Next 3 values:`,
...
async function demonstrateOrchestrator() {
  ...
  try {
    ...
    const testTasks = [
      {
        ...
        expectedAgent: 'LanguageMaestro',
      },
      {
        type: 'data-analysis',
        input: [{ x: 1, y: 2 }],
        expectedAgent: 'DataSage',
      },
      {
        type: 'multimodal-analysis',
        input: { text: 'Hello world' },
        expectedAgent: 'MultiModalOracle',
      },
    ];
    ...
    console.log(
      `📊 Healthy Agents: ${Object.values(health.agents).filter(a => a.status === 'healthy').length}/${Object.keys(health.agents).length}`
    );
  } catch (error) {
