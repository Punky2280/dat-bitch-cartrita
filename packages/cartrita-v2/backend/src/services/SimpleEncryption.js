...
class SimpleEncryption {
  ...
  validateApiKeyFormat(provider, apiKey) {
    ...
    console.log(
      `[Encryption] API key length: ${apiKey ? apiKey.length : 'null'}`
    );
    console.log(
      `[Encryption] API key prefix: ${apiKey ? apiKey.substring(0, 15) : 'null'}...`
    );
    ...
    if (!pattern) {
      console.log(
        `[Encryption] No pattern for provider ${provider}, using length check`
      );
