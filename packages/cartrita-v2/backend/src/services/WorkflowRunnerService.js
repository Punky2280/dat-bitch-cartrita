...
export default class WorkflowRunnerService {
  ...
  async initialize() {
    try {
      console.log(
        '[WorkflowRunnerService] 🚀 Initializing workflow execution engine...'
      );
      ...
      console.log(
        '[WorkflowRunnerService] ✅ Service initialized successfully'
      );
      return true;
    } catch (error) {
        ...
        try {
          ...
          if (this.activeExecutions.has(executionId)) {
            ...
            await this.dbPool.query(
              ...
              WHERE id = $3
            `,
              [error.message, Date.now() - execution.startTime, execution.id]
            );
          }
      ...
      async span => {
        try {
          switch (step.type) {
            ...
            default:
              console.warn(
                `[WorkflowRunnerService] Unknown step type: ${step.type}`
              );
              return {
                success: true,
  ...
  async executeDataTransform(step, context) {
    ...
    try {
      // Simple transformation logic using safe evaluation
      // Sanitize the script to only allow safe operations
      const safeScript = script
        .replace(/context\.(\w+)/g, (match, prop) => {
          return `context['${prop}']`;
        })
        .replace(/[^a-zA-Z0-9\[\]'"\.\s<>=!&|()]/g, '');

      // Use eval with sanitized script (safer than Function constructor)
  ...
  async executeCondition(step, context) {
    ...
    try {
      // Sanitize condition to only allow safe operations
      ...
      // Use eval with sanitized condition (safer than Function constructor)
      const result = Boolean(eval(safeCondition));
  ...
  getHealthStatus() {
    return {
      ...
      sse_clients: Array.from(this.sseClients.values()).reduce(
