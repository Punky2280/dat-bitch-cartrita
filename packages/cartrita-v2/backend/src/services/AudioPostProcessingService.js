...
class AudioPostProcessingService {
  ...
  initializeMetrics() {
    try {
      ...
      console.warn(
        '[AudioPostProcessing] Failed to initialize metrics:',
        error.message
      );
      this.metrics = {};
    }
  ...
  selectBestRecording(recordings, options = {}) {
    ...
    try {
      ...
      recordings.forEach(recording => {
        if (
          this.metrics.qualityDistribution &&
          typeof recording.quality === 'number'
        ) {
      ...
      const qualityThreshold =
        options.qualityThreshold || this.qualityThreshold;
      const qualityFiltered = recordings.filter(
        r => r.quality !== undefined && r.quality >= qualityThreshold
      );

      if (qualityFiltered.length === 0) {
        console.warn(
          '[AudioPostProcessing] No recordings meet quality threshold, using first available'
        );
        return recordings[0];
      }
      ...
      const sorted = qualityFiltered.sort((a, b) => {
        ...
        const optimalMin =
          options.optimalDurationMin || this.optimalDurationMin;
        const optimalMax =
          options.optimalDurationMax || this.optimalDurationMax;

        const aIsOptimal =
          a.duration >= optimalMin && a.duration <= optimalMax ? 1 : 0;
        const bIsOptimal =
          b.duration >= optimalMin && b.duration <= optimalMax ? 1 : 0;
    ...
    try {
      ...
      if (analysis.metrics.estimatedDuration > 0) {
        if (analysis.metrics.estimatedDuration < 0.5) {
          ...
          analysis.metrics.estimatedDuration >= 2 &&
          analysis.metrics.estimatedDuration <= 5
        ) {
      ...
      if (analysis.metrics.sampleRate >= 16000) {
        ...
        analysis.issues.push(
          'Low sample rate (may affect transcription quality)'
        );
        analysis.quality -= 0.1;
      }
      ...
      analysis.confidence =
        analysis.issues.length === 0
          ? 0.9
          : Math.max(0.1, 0.9 - analysis.issues.length * 0.2);
  ...
  async processBatch(recordingBatch, options = {}) {
    try {
      console.log(
        `[AudioPostProcessing] Processing batch of ${recordingBatch.length} recordings`
      );
      ...
      const selectedRecording = this.selectBestRecording(
        analyzedRecordings,
        options
      );
      ...
      return {
        ...
        qualityScores: analyzedRecordings.map(r => ({
          id: r.id,
          quality: r.quality,
        })),
