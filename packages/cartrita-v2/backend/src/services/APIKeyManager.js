...
class APIKeyManager {
  ...
  initializeKeyPermissions() {
    const defaults = {
      ...
      // Computer Use Agent - requires special permissions and supervision
      computer_use: {
        openai: process.env.OPENAI_API_KEY, // For computer-use-preview model
        openai_finetuning: process.env.OPENAI_FINETUNING_API_KEY, // Preferred for computer use if available
      },
    ...
    try {
      ...
      if (fs.existsSync(cfgPath)) {
        ...
        console.log(
          '[APIKeyManager] 🔄 Permissions overlay applied from config/api-key-permissions.json'
        );
      }
    } catch (err) {
      console.warn(
        '[APIKeyManager] ⚠️ Failed loading permissions config:',
        err.message
      );
    }
    return defaults;
