...
class ContextualAwarenessService extends EventEmitter {
  ...
  async analyzeMessageContext(message, userContext) {
    ...
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are an expert in contextual analysis and emotional intelligence. Provide accurate, nuanced analysis in valid JSON format.',
          },
          {
            role: 'user',
            content: prompt,
      ...
      console.error(
        '[ContextualAwareness] Error analyzing message context:',
        error
      );
  ...
  updateContextFromAnalysis(userId, analysis) {
    ...
    if (analysis.sentiment) {
      ...
      if (analysis.contextualCues?.stressIndicators?.length > 0) {
        context.emotionalContext.stressLevel = Math.min(
          1.0,
          context.emotionalContext.stressLevel +
            0.1 * analysis.contextualCues.stressIndicators.length
        );
      } else if (analysis.sentiment.overall === 'positive') {
        context.emotionalContext.stressLevel = Math.max(
          0.0,
      ...
      if (analysis.contextualCues?.engagementLevel !== undefined) {
        context.emotionalContext.engagementLevel =
          (context.emotionalContext.engagementLevel +
            analysis.contextualCues.engagementLevel) /
          2;
      }
    }
    ...
    if (analysis.topic) {
      if (analysis.topic.isNewTopic) {
        context.conversationContext.topicHistory.push(
      ...
      context.conversationContext.complexity =
        analysis.intent?.complexity || 'medium';
    }
    ...
    if (analysis.adaptiveRecommendations) {
      ...
      context.adaptiveSettings.responseStyle =
        recs.responseStyle || context.adaptiveSettings.responseStyle;
      context.adaptiveSettings.responseLength =
        recs.responseLength || context.adaptiveSettings.responseLength;
  ...
  getResponseRecommendations(userId, proposedResponse) {
    const context = this.userContexts.get(userId);
    if (!context)
      return { style: 'casual', length: 'moderate', tone: 'neutral' };
  ...
  getAdaptations(context) {
    ...
    if (context.conversationContext.questionCount > 5) {
      adaptations.push(
        'User has asked many questions - check if they need different approach'
      );
    }

    if (context.emotionalContext.frustrationLevel > 0.6) {
      adaptations.push(
        'High frustration detected - prioritize solution-focused responses'
      );
    }
  ...
  getContextualWarnings(context) {
    ...
    const recentNegative = recentSentiments
      .slice(-5)
      .filter(s => s.sentiment === 'negative');
    if (recentNegative.length >= 3) {
      warnings.push(
        'NEGATIVE_TREND: Multiple negative sentiments detected recently'
      );
    }
  ...
  getFallbackAnalysis(message) {
    ...
    const hasNegativeWords =
      /\b(error|fail|problem|issue|broken|wrong|bad)\b/i.test(message);
    const hasPositiveWords =
      /\b(good|great|awesome|perfect|love|excellent)\b/i.test(message);

    return {
      sentiment: {
        overall: hasNegativeWords
          ? 'negative'
          : hasPositiveWords
            ? 'positive'
            : 'neutral',
  ...
  async performPeriodicContextAnalysis() {
    for (const [userId, context] of this.userContexts.entries()) {
      try {
        ...
        console.error(
          `[ContextualAwareness] Error in periodic analysis for user ${userId}:`,
          error
        );
      }
    }
    ...
    for (const [userId, sentiments] of this.sentimentHistory.entries()) {
      try {
        ...
        if (context) {
          ...
          context.metrics.satisfactionScore = Math.max(
            0,
            Math.min(1, 0.5 + avgSentimentScore * 0.3)
          );
        ...
        console.error(
          `[ContextualAwareness] Error in sentiment analysis for user ${userId}:`,
          error
        );
      }
    }
  ...
  checkForConcerningPatterns(userId, context) {
    ...
    const history = this.conversationHistory.get(userId) || [];
    const last5Minutes = history.filter(
    ...
    if (last5Minutes.length > 10) {
      ...
      context.emotionalContext.stressLevel = Math.min(
        1.0,
        context.emotionalContext.stressLevel + 0.2
      );
    }
    ...
    if (
      questionTexts.length > 3 &&
      uniqueQuestions.size < questionTexts.length * 0.5
    ) {
      warnings.push(
        'REPETITIVE_QUESTIONS: User asking similar questions repeatedly'
      );
      context.emotionalContext.frustrationLevel = Math.min(
        1.0,
        context.emotionalContext.frustrationLevel + 0.3
      );
    }
  ...
  getServiceStats() {
    return {
      ...
      avgSatisfaction:
        Array.from(this.userContexts.values()).reduce(
          (sum, ctx) => sum + ctx.metrics.satisfactionScore,
          0
