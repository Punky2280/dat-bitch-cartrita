...
class WorkflowEngine {
  ...
  mapStepTypeToAgent(stepType) {
    const agentMapping = {
      ...
      'http-request': 'codewriter',
      'data-processing': 'analyst',
      'security-check': 'security',
    };

    return agentMapping[stepType] || 'cartrita';
  }
...
class LogicEngine {
  ...
  evaluateExpression(expression, data) {
    // Basic expression evaluation using safe operators
    // In production, use a proper expression parser like JSONata
    try {
      // Only allow safe property access and basic comparisons
      ...
      // Use eval only for basic comparisons (safer than Function constructor)
      // This is still not ideal - consider using a proper expression parser
