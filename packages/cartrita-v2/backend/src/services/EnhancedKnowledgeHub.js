...
class EnhancedKnowledgeHub {
  constructor() {
    ...
    if (process.env.KH_FAST_RETRY === '1') {
      ...
      console.log(
        '[EnhancedKnowledgeHub] ⚡ KH_FAST_RETRY=1 -> Using fast retry profile'
      );
    }
  ...
  async initialize() {
    ...
    if (process.env.LIGHTWEIGHT_TEST === '1') {
      console.log(
        '[EnhancedKnowledgeHub] 🧪 LIGHTWEIGHT_TEST=1 -> Skipping initialization'
      );
      this.disabled = true;
      return false;
    }
    if (process.env.DISABLE_KNOWLEDGE_HUB === '1') {
      console.warn(
        '[EnhancedKnowledgeHub] ⚠️ DISABLE_KNOWLEDGE_HUB=1 -> Service disabled by env'
      );
      this.disabled = true;
      return false;
    }
    const attemptInit = async () => {
      try {
        ...
        if (willRetry) {
          ...
          console.warn(
            `[EnhancedKnowledgeHub] ⚠️ Init attempt ${this.retryCount} failed (${error.message || error}) - retrying in ${delay}ms`
          );
        ...
        if (process.env.KNOWLEDGE_HUB_OPTIONAL === '1') {
          console.warn(
            '[EnhancedKnowledgeHub] ❌ Initialization failed but continuing (optional mode):',
            error.message
          );
          this.disabled = true;
          return false;
        ...
        console.error(
          '[EnhancedKnowledgeHub] ❌ Initialization failed (giving up):',
          error.message || error
        );
        this.disabled = true;
        return false;
