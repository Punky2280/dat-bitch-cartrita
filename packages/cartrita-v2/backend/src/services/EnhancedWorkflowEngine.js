...
class EnhancedWorkflowEngine {
  ...
  initializeNodeHandlers() {
    ...
    const safeRegister = (nodeType, handlerName) => {
      if (typeof this[handlerName] === 'function') {
        ...
        if (
          !shouldQuietLogs() &&
          (process.env.WORKFLOW_DEBUG || process.env.DEBUG_LOGS)
        ) {
          console.warn(
  ...
  initializeScheduler() {
    ...
    if (process.env.DISABLE_SCHEDULER === '1') {
      if (!shouldQuietLogs()) {
        console.log(
          '[EnhancedWorkflowEngine] ⏩ Scheduler disabled for environment'
        );
      }
      return;
    }
  ...
  async executeWorkflow(
    ...
    try {
      ...
      // Derive node metrics
      executionContext.nodeCount =
        executionContext.nodeResults?.size ||
        workflowData.nodes?.length ||
        null;
      executionContext.successNodeCount = executionContext.nodeResults
        ? Array.from(executionContext.nodeResults.values()).filter(
            v => v && !v.error
          ).length
        : null;
      ...
      if (executionId && this.activeExecutions.has(executionId)) {
        ...
        if (context.executionDbId) {
          ...
          context.failedNodeCount = context.nodeResults
            ? Array.from(context.nodeResults.values()).filter(v => v && v.error)
                .length
            : null;
          await pool.query(
            `UPDATE workflow_executions 
  ...
  async handleGPTNode(node, executionContext) {
    ...
    if (this.modelRegistry && useModelRegistry) {
      try {
        ...
        console.warn(
          '[EnhancedWorkflowEngine] Model registry selection failed, using fallback:',
    ...
    if (this.modelRegistry && modelSelectionResult) {
      try {
        ...
        return {
          content:
            inferenceResult.result.text ||
            inferenceResult.result.choices?.[0]?.message?.content,
          model: selectedModel,
        ...
        console.warn(
          '[EnhancedWorkflowEngine] Model registry execution failed, using direct OpenAI:',
  ...
  async handleRAGEmbeddingsNode(node, executionContext) {
    ...
    if (this.modelRegistry && useModelRegistry) {
      try {
        ...
        console.warn(
          '[EnhancedWorkflowEngine] Embedding model selection failed, using fallback:',
          modelSelectionError.message
        );
      }
    }
  ...
  async handleRAGSearchNode(node, executionContext) {
    ...
    const { query, limit = 5, threshold = 0.7, documentIds } = node.data || {};
