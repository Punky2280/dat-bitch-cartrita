...
class EmbeddingPipeline extends EventEmitter {
  constructor() {
    ...
    console.log(
      '[EmbeddingPipeline] 🔄 Embedding Pipeline Service initialized'
    );
  ...
  async queueEmbedding(entryId, text, type = 'content') {
    ...
    console.log(
      `[EmbeddingPipeline] ➕ Queued ${type} embedding for entry ${entryId}`
    );
  ...
  async startProcessing() {
    ...
    console.log(
      `[EmbeddingPipeline] 🚀 Starting to process ${this.jobQueue.length} queued jobs`
    );
    ...
    try {
      ...
      while (
        ...
        processingPromises.length < this.concurrency
      ) {
  ...
  async processJob(job) {
    ...
    try {
      console.log(
        `[EmbeddingPipeline] 🔄 Processing ${job.type} embedding for entry ${job.entryId} (attempt ${job.attempts})`
      );
      ...
      console.log(
        `[EmbeddingPipeline] ✅ Completed ${job.type} embedding for entry ${job.entryId} in ${processingTime}ms`
      );
      ...
      console.error(
        `[EmbeddingPipeline] ❌ Error processing ${job.type} embedding for entry ${job.entryId}:`,
        error
      );
      ...
      if (job.attempts < this.retryCount) {
        console.log(
          `[EmbeddingPipeline] 🔄 Retrying job ${job.id} in ${this.retryDelay}ms`
        );
        ...
        this.stats.failed++;
        console.error(
          `[EmbeddingPipeline] ❌ Job ${job.id} failed after ${job.attempts} attempts`
        );
  ...
  async generateEmbedding(text) {
    try {
      ...
      if (error.status === 429) {
        console.warn(
          '[EmbeddingPipeline] ⚠️ Rate limited, waiting before retry...'
        );
  ...
  async updateEntryEmbedding(entryId, type, embedding) {
    ...
    console.log(
      `[EmbeddingPipeline] 💾 Updated ${column} for entry ${entryId}`
    );
  }
  ...
  async discoverRelationships(entryId, embedding) {
    try {
      console.log(
        `[EmbeddingPipeline] 🕸️ Discovering relationships for entry ${entryId}`
      );
      ...
      console.log(
        `[EmbeddingPipeline] ✅ Discovered ${relationships.length} relationships for entry ${entryId}`
      );
      ...
      console.error(
        `[EmbeddingPipeline] ❌ Relationship discovery error for entry ${entryId}:`,
        error
      );
    }
  }
  ...
  async processAllMissingEmbeddings() {
    try {
      console.log(
        '[EmbeddingPipeline] 🔍 Finding entries missing embeddings...'
      );
      ...
      console.log(
        `[EmbeddingPipeline] 📋 Found ${result.rows.length} entries needing embeddings`
      );
      ...
      console.error(
        '[EmbeddingPipeline] ❌ Error processing missing embeddings:',
        error
      );
    }
  }
