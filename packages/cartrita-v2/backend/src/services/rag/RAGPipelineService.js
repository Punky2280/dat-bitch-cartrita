      ...
      async () => {
        ...
        for (const text of textArray) {
          ...
          try {
            ...
            console.error(
              `Embedding generation failed for model ${model}:`,
              error.message
            );
      ...
      try {
        ...
        const pairs = documents.map(doc => ({
          ...
          text:
            typeof doc === 'string'
              ? doc
              : doc.text || doc.content || String(doc),
        }));
        ...
        return rankedResults.sort((a, b) => b.score - a.score).slice(0, topK);
      } catch (error) {
      ...
      async () => {
        ...
        try {
          ...
          for (const q of queries) {
            ...
            const candidates = await this.retrieveDocuments(
              queryEmbedding,
              documentStore,
              topKRetrieval
            );
          ...
          const confidence = await this.evaluateAnswerConfidence(
            answer,
            context
          );
      ...
      const isDuplicate = unique.some(existingDoc => {
        const existingContent =
          typeof existingDoc === 'string'
            ? existingDoc
            : existingDoc.text || existingDoc.content;
        return (
          this.calculateStringSimilarity(content, existingContent) > threshold
        );
      });
    ...
    return rerankedDocs
      .map((item, index) => {
        const content =
          typeof item.document === 'string'
            ? item.document
            : item.document.text || item.document.content;
...
Answer (with citations if applicable):`;
    ...
    const answerStart = answer.lastIndexOf(
      'Answer (with citations if applicable):'
    );
    if (answerStart !== -1) {
  ...
  async evaluateAnswerConfidence(answer, context) {
    ...
    // Citation presence
    const citationScore =
      (answer.match(/\[Source \d+\]/g) || []).length > 0 ? 0.2 : 0;
