...
class AmbientVoiceService extends EventEmitter {
  constructor() {
    ...
    console.log(
      '[AmbientVoiceService] ✅ Initialized with features:',
      this.features
    );
  }
  ...
  async startSession(userId, options = {}) {
    ...
    console.log(
      `[AmbientVoiceService] 🎤 Started session ${sessionId} for user ${userId}`
    );
  ...
  async stopSession(sessionId) {
    ...
    const finalStats = {
      ...
      averageProcessingTime:
        session.stats.audioChunksProcessed > 0
    ...
    console.log(
      `[AmbientVoiceService] 🛑 Stopped session ${sessionId}`,
      finalStats
    );
  ...
  async processAudioChunk(sessionId, audioData, metadata = {}) {
    ...
    try {
      ...
      if (session.buffers.audioBuffer.length > bufferLimit) {
        session.buffers.audioBuffer =
          session.buffers.audioBuffer.slice(-bufferLimit);
      }
      ...
      console.error(
        `[AmbientVoiceService] ❌ Audio processing failed for ${sessionId}:`,
        error.message
      );
  ...
  async detectVoiceActivity(audioData, session) {
    ...
    const voiceDetected =
      rms > this.vadSettings.silenceThreshold && zcr > 0.01 && zcr < 0.5; // Typical voice characteristics

    // Spectral analysis for enhanced detection (simplified)
    const spectralCentroid = this.calculateSpectralCentroid(audioBuffer);
    const enhancedVoiceDetected =
      voiceDetected && spectralCentroid > 500 && spectralCentroid < 4000; // Human voice range
  ...
  async detectWakeWord(audioData, session) {
    ...
    const allWakeWords = [
      ...this.wakeWords,
      ...session.options.customWakeWords,
    ];
    ...
    try {
      ...
      console.warn(
        `[AmbientVoiceService] Transcription failed for wake word detection:`,
        error.message
      );
    }
    ...
    for (const wakeWord of allWakeWords) {
      ...
      if (transcriptionText.includes(normalizedWakeWord)) {
        console.log(
          `[AmbientVoiceService] 🎯 Wake word detected: "${wakeWord}"`
        );
  ...
  async handleWakeWordDetection(session, wakeResult) {
    console.log(
      `[AmbientVoiceService] 🎤 Activating voice assistant for session ${session.id}`
    );
  ...
  async startCommandRecording(session) {
    ...
    console.log(
      `[AmbientVoiceService] 🎙️ Started command recording for session ${session.id}`
    );
  }
  ...
  async processCommand(session) {
    ...
    try {
      ...
      const commandAudio = this.concatenateAudioChunks(
        session.commandRecording.audioChunks
      );

      if (commandAudio.length === 0) {
        console.log(
          `[AmbientVoiceService] No command audio recorded for session ${session.id}`
        );
        return;
      }
      ...
      console.log(
        `[AmbientVoiceService] 📝 Command transcribed: "${transcription.text}"`
      );
      ...
      console.error(
        `[AmbientVoiceService] ❌ Command processing failed:`,
        error.message
      );
    } finally {
  ...
  concatenateRecentAudio(session, durationMs) {
    ...
    const recentChunks = session.buffers.audioBuffer.filter(
    ...
    const totalStats = sessions.reduce(
      (acc, session) => ({
        audioChunksProcessed:
          acc.audioChunksProcessed + session.stats.audioChunksProcessed,
        wakeWordsDetected:
          acc.wakeWordsDetected + session.stats.wakeWordsDetected,
        voiceActivationsDetected:
          acc.voiceActivationsDetected + session.stats.voiceActivationsDetected,
        transcriptionsGenerated:
          acc.transcriptionsGenerated + session.stats.transcriptionsGenerated,
      }),
      {
        audioChunksProcessed: 0,
        wakeWordsDetected: 0,
