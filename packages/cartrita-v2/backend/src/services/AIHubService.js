...
class AIHubService {
  constructor(unifiedInferenceService, knowledgeHubService, hfRouterService) {
    ...
    console.log(
      '[AIHubService] 🤖 AI Hub service initialized with unified inference and knowledge integration'
    );
  }
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async span => {
        try {
          ...
          console.error(
            '[AIHubService] ❌ Error getting model catalog:',
            error
          );
  ...
  async buildModelCatalog() {
    if (
      this.catalogLastUpdated &&
      Date.now() - this.catalogLastUpdated < this.cacheTTL.catalog * 1000
    ) {
    ...
    if (this.hfRouter) {
      try {
        ...
        console.error(
          '[AIHubService] ⚠️ Error getting HF Router models:',
          error
        );
      }
    }
    ...
    console.log(
      `[AIHubService] ✅ Model catalog built with ${this.modelCatalog.length} models`
    );
  }
  ...
  filterModels(models, filters) {
    ...
    if (filters.capability && filters.capability !== 'all') {
      filtered = filtered.filter(
        m => m.capabilities && m.capabilities.includes(filters.capability)
      );
    }
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async span => {
        try {
          ...
          const searchResult = await this.knowledgeHub.searchEntries(
            userId,
            query,
            {
              limit,
          ...
          if (enhanceWithAI && enhancedResults.length > 0) {
            try {
              ...
              const summaryPrompt =
                `Based on these knowledge entries, provide a concise summary and key insights for the query "${query}":\n\n` +
                enhancedResults
                  .map(
                    entry =>
                      `- ${entry.title}: ${entry.content?.substring(0, 200) || entry.summary || ''}`
                  )
                  .join('\n');
              ...
              console.error(
                '[AIHubService] ⚠️ AI enhancement failed:',
                aiError
              );
          ...
          console.error(
            '[AIHubService] ❌ Error searching knowledge hub:',
            error
          );
          span?.recordException(error);
          throw error;
