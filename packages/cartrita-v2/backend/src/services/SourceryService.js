...
export default class SourceryService {
  constructor() {
    ...
    this.token =
      process.env.SOURCERY_TOKEN ||
      'user_jgWWmo1BwazNufvKZEmx6k3P3rAPFQQdZykTooF2ZEg9gJyrnrjQBO25GGw';
    this.metrics = {
      totalAnalyses: 0,
      issuesFound: 0,
      fixesApplied: 0,
  ...
  async initialize() {
    try {
      console.log(
        '[SourceryService] 🔍 Initializing Sourcery code quality service...'
      );
      ...
      if (this.token) {
        try {
          ...
          console.warn(
            '[SourceryService] ⚠️ Authentication may have failed, continuing...'
          );
        }
      }
      ...
      console.log(
        '[SourceryService] ✅ Sourcery service initialized successfully'
      );
      return true;
    } catch (error) {
      console.error(
        '[SourceryService] ❌ Failed to initialize Sourcery service:',
        error
      );
      this.initialized = false;
      return false;
  ...
  async analyzeCodeQuality(targetPath, options = {}) {
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async () => {
        try {
          console.log(
            `[SourceryService] 🔍 Analyzing code quality for: ${targetPath}`
          );
          ...
          const analysisResult = this.parseAnalysisOutput(
            output,
            options.output
          );
          ...
          console.error(
            `[SourceryService] Analysis failed for ${targetPath}:`,
            error
          );
          throw error;
        }
  ...
  async applyAutoRefactoring(targetPath, options = {}) {
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async () => {
        try {
          console.log(
            `[SourceryService] 🛠️ Applying refactoring to: ${targetPath}`
          );
          ...
          const output = await this.executeSourceryCommand(args);
          const refactorResult = this.parseRefactorOutput(
            output,
            options.dryRun
          );
          ...
          console.error(
            `[SourceryService] Refactoring failed for ${targetPath}:`,
            error
          );
          throw error;
        }
  ...
  async generateQualityReport(projectPath, options = {}) {
    ...
    try {
      console.log(
        `[SourceryService] 📊 Generating quality report for: ${projectPath}`
      );
      ...
      console.error(
        `[SourceryService] Failed to generate quality report:`,
        error
      );
      throw error;
    }
  ...
  parseAnalysisOutput(output, format = 'text') {
    try {
      ...
      console.error(
        '[SourceryService] Failed to parse analysis output:',
        error
      );
  ...
  calculateQualityMetrics(analysis) {
    ...
    return {
      ...
      filesAnalyzed,
      issuesPerFile:
        filesAnalyzed > 0 ? (totalIssues / filesAnalyzed).toFixed(2) : 0,
      issuesByType,
      issuesBySeverity,
  ...
  updateAnalysisMetrics(analysisResult) {
    ...
    if (analysisResult.complexityScore) {
      ...
      this.metrics.averageComplexityScore =
        (currentAvg * (totalAnalyses - 1) + analysisResult.complexityScore) /
        totalAnalyses;
    }
  }
