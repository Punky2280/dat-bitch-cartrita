...
class MultiProviderAIService {
  constructor() {
    ...
    console.log(
      '[MultiProviderAI] 🤖 Multi-Provider AI Service initialized with 14 providers'
    );
  ...
  validateApiKeys() {
    ...
    if (missing.length > 0) {
      console.warn(
        '[MultiProviderAI] ⚠️ Missing API keys:',
        missing.join(', ')
      );
    } else {
  ...
  routeTask(input) {
    ...
    if (processed.modality === 'image' && input.request === 'segment')
      return 'vision-segmentation';
    if (processed.modality === 'image' && input.request === 'detect')
      return 'vision-detection';
    if (input.candidate_labels) {
      return input.candidate_labels.length <= 10
        ? 'zero-shot'
        : 'hier-zero-shot';
    }
    if (input.question && input.context) return 'qa';
    if (/translate\s/i.test(input.prompt) || input.forceTranslate)
      return 'translation';
    if (input.intent === 'finance_sentiment')
      return 'text-classification-finance';
  ...
  async inference(request) {
    ...
    console.log(
      `[MultiProviderAI] 🎯 Routing ${request.task} to ${provider.name}`
    );
  ...
  async executeZeroShot(provider, request) {
    if (provider.name === 'Hugging Face') {
      return await this.hfJsonModel('facebook/bart-large-mnli', {
        inputs: request.input,
        parameters: {
    ...
    throw new Error(
      `Zero-shot classification not supported by ${provider.name}`
    );
  }
  ...
  async executeNER(provider, request) {
    if (provider.name === 'Hugging Face') {
      return await this.hfJsonModel('dslim/bert-base-NER', {
        inputs: request.input,
  ...
  async executeQA(provider, request) {
    if (provider.name === 'Hugging Face') {
      return await this.hfJsonModel('deepset/roberta-base-squad2', {
        inputs: {
  ...
  async executeGeneration(provider, request) {
    ...
    if (provider.name === 'Hugging Face') {
      return await this.hfJsonModel('google/gemma-2-2b-it', {
  ...
  async executeASR(provider, request) {
    if (provider.name === 'Hugging Face') {
      return await this.hfJsonModel('openai/whisper-large-v3', {
        inputs: request.input,
  ...
  async executeVisionDetection(provider, request) {
    if (provider.name === 'Hugging Face') {
      return await this.hfJsonModel('facebook/detr-resnet-50', {
        inputs: request.input,
      });
    }
  ...
  async executeVisionNSFW(provider, request) {
    if (provider.name === 'Hugging Face') {
      return await this.hfJsonModel('Falconsai/nsfw_image_detection', {
        inputs: request.input,
      });
    }
  ...
  getServiceStats() {
    return {
      ...
      availableProviders: Object.entries(this.providers).map(
        ([key, provider]) => ({
          key,
          name: provider.name,
