...
class HuggingFaceRouterService {
  constructor() {
    ...
    if (this.hfToken && this.hfToken.startsWith('hf_')) {
      ...
      console.warn(
        '[HuggingFaceRouterService] ⚠️ No valid HF token provided, service will return mock responses'
      );
    }
    ...
    console.log(
      '[HuggingFaceRouterService] 🚀 Service initialized with JavaScript fetch approach'
    );
  }
  ...
  async chatCompletion(params) {
    try {
      ...
      console.log(
        `[HuggingFaceRouterService] 💬 Chat completion: ${params.model || 'default'}`
      );
      ...
      console.log(
        `[HuggingFaceRouterService] ✅ Chat completion successful (${Date.now() - startTime}ms)`
      );
      ...
      console.error(
        '[HuggingFaceRouterService] ❌ Chat completion failed:',
        error.message
      );
      return {
        success: false,
  ...
  async textToImage(params) {
    try {
      const startTime = Date.now();
      console.log(
        `[HuggingFaceRouterService] 🎨 Text-to-image: "${params.prompt.substring(0, 50)}..."`
      );
      ...
      const model =
        ...
        params.model ||
        this.models.textToImage['stable-diffusion-xl'];

      const response = await fetch(
        `${this.baseURL}/hf-inference/models/${model}`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${this.hfToken}`,
      ...
      console.log(
        `[HuggingFaceRouterService] ✅ Image generated successfully (${Date.now() - startTime}ms)`
      );
      ...
      console.error(
        '[HuggingFaceRouterService] ❌ Image generation failed:',
        error.message
      );
      return {
        success: false,
  ...
  async createEmbeddings(params) {
    try {
      ...
      const model =
        ...
        params.model ||
        this.models.embeddings['multilingual-e5-large'];

      const response = await fetch(
        `${this.baseURL}/hf-inference/models/${model}/pipeline/feature-extraction`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${this.hfToken}`,
      ...
      console.log(
        `[HuggingFaceRouterService] ✅ Embeddings created (${Date.now() - startTime}ms)`
      );
      ...
      console.error(
        '[HuggingFaceRouterService] ❌ Embeddings failed:',
        error.message
      );
      return {
        success: false,
  ...
  async speechToText(params) {
    try {
      ...
      console.log(
        `[HuggingFaceRouterService] ✅ Transcription successful (${Date.now() - startTime}ms)`
      );
      ...
      console.error(
        '[HuggingFaceRouterService] ❌ Speech-to-text failed:',
        error.message
      );
      return {
        success: false,
