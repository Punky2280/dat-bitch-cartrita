...
export class MCPBridge {
  ...
  private readonly endpointMappings = new Map<
    string,
    {
      taskType: string;
  ...
  async handleLegacyRequest(request: LegacyRequest, reply: any): Promise<any> {
    ...
    try {
      ...
      const legacyResponse = await this.convertToLegacyResponse(
        taskResponse,
        endpoint
      );
      ...
      reply.header(
        ...
        'This API version is deprecated. Please migrate to v3'
      );
      reply.header(
        'X-Migration-Guide',
        'https://docs.cartrita.com/api/migration-v2-to-v3'
      );
  ...
  private getSocketEventMapping(
    eventName: string
  ): { taskType: string; parameterMapper: (data: any) => any } | null {
    const mappings: Record<
      string,
      { taskType: string; parameterMapper: (data: any) => any }
    > = {
      chat: {
        taskType: TaskTypes['langchain.chat.execute'],
        parameterMapper: data => ({
  ...
  private async handleUnmappedEndpoint(
    request: LegacyRequest,
    reply: any
  ): Promise<any> {
