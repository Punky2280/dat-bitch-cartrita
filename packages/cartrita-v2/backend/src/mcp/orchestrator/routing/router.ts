...
export class MCPRouter extends EventEmitter {
  ...
  private readonly tracer = trace.getTracer('mcp-router');
  ...
  async registerAgent(
    ...
    try {
      ...
      const validation = MCPValidator.safeValidate(
        data => MCPValidator.validateMessage(data),
        registration
      );
      if (!validation.success) {
        throw new Error(
          `Invalid agent registration: ${validation.errors.join(', ')}`
        );
      }
      ...
      const confirmMessage: MCPMessage = {
        id: uuidv4(),
      ...
      this.logger.error('Agent registration failed', error as Error, {
  ...
  async routeTask(
    ...
    try {
      ...
      const targetSupervisor =
        supervisorType || this.selectSupervisor(taskRequest.taskType);

      // Find available supervisor instance
      const supervisor = this.findAvailableSupervisor(targetSupervisor);
      if (!supervisor) {
        throw new Error(
          `No available supervisor for type: ${targetSupervisor}`
        );
      }
      ...
      const response = await this.sendTaskAndWaitForResponse(
        message,
        supervisor
      );
      ...
      span.recordException(error as Error);
      this.logger.error('Task routing failed', error as Error, {
  ...
  private initializeDefaultSupervisors(): void {
    const defaultSupervisors: SupervisorInfo[] = [
      {
  ...
  private selectSupervisor(taskType: string): string {
    // Use MCPValidator to determine supervisor
    return MCPValidator.getSupervisorForTask(taskType as any);
  }

  private findAvailableSupervisor(
    supervisorType: string
  ): SupervisorInfo | null {
    // Find supervisors that can handle this type
    const candidates = Array.from(this.supervisors.values()).filter(
      s =>
        s.id.includes(supervisorType) &&
        s.isHealthy &&
  ...
  private async sendTaskAndWaitForResponse(
    message: MCPMessage,
  ...
  private async handleTaskResponse(message: MCPMessage): Promise<void> {
    const taskId = message.correlationId;
    if (!taskId) {
      this.logger.warn('Task response without correlation ID', {
        messageId: message.id,
      });
      return;
    }
