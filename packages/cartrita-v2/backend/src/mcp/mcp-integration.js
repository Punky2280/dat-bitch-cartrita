...
export class MCPSystemManager {
  ...
  async initialize(existingServices = {}, dbPool = null, redisClient = null) {
    ...
    try {
      this.logger.log(
        '[MCP] Initializing Master Control Program with full infrastructure...'
      );
  ...
  async initializeMCPSchema() {
    if (!this.pool) {
      this.logger.warn(
        '[MCP] No database pool provided, skipping schema initialization'
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(
          '[MCP] pgvector extension not available, skipping embedding index'
        );
      }
  ...
  async initializeEnhancedMCP(existingServices) {
        ...
        switch (taskType) {
          ...
          case 'research.github.search':
            if (existingServices.researcherAgent) {
              return await existingServices.researcherAgent.processTask(
                parameters
              );
            }
            break;
          ...
          case 'writer.blog.post':
            if (existingServices.writerAgent) {
              return await existingServices.writerAgent.generateContent(
                parameters
              );
            }
            break;
          ...
          case 'codewriter.refactor.code':
            if (existingServices.codeWriterAgent) {
              return await existingServices.codeWriterAgent.generateCode(
                parameters
              );
            }
            break;
          ...
          case 'analytics.report.generate':
            if (existingServices.analyticsAgent) {
              return await existingServices.analyticsAgent.processData(
                parameters
              );
            }
            break;
          ...
          case 'multimodal.audio.transcribe':
            if (existingServices.multiModalFusionAgent) {
              return await existingServices.multiModalFusionAgent.processMultiModal(
                parameters
              );
            }
            break;
          ...
          default:
            ...
            if (existingServices.coreAgent) {
              return await existingServices.coreAgent.processRequest(
                parameters
              );
            }
    ...
    this.intelligenceSupervisor = {
      ...
      getStats() {
        return {
          activeAgents: Object.keys(existingServices).filter(key =>
            key.endsWith('Agent')
          ).length,
  ...
  getStatus() {
    return {
      ...
      supervisors: {
        intelligence: this.intelligenceSupervisor?.getStats() || {
          status: 'inactive',
        },
        multimodal: this.multiModalSupervisor?.getStats() || {
          status: 'inactive',
        },
      },
      supportedTaskTypes: [
