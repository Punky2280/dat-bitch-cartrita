...
export class SensorFusion {
   ...
   */
  private async weightedAverageFusion(
    sensors: any[],
    sensorsUsed: string[]
  ): Promise<any> {
   ...
   */
  private async kalmanFilterFusion(
    sensors: any[],
    sensorsUsed: string[]
  ): Promise<any> {
      ...
      if (typeof estimate === 'object' && typeof measurement === 'object') {
        for (const key in measurement) {
          if (
            ...
            typeof estimate[key] === 'number'
          ) {
            estimate[key] =
              estimate[key] + kalmanGain * (measurement[key] - estimate[key]);
          }
        }
      }
  ...
  private calculateConfidence(fusedData: any, originalSensors: any[]): number {
    ...
    if (sensorConfidences.length > 0) {
      const avgConfidence =
        sensorConfidences.reduce((sum, conf) => sum + conf, 0) /
        sensorConfidences.length;
