...
export class DeepgramAgent {
  ...
  private readonly tracer = trace.getTracer('deepgram-agent');
  private deepgramClient: any;
  private isInitialized = false;

  constructor(private config: DeepgramAgentConfig = {}) {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.deepgramClient = createClient(
        this.config.apiKey || process.env.DEEPGRAM_API_KEY
      );

      this.isInitialized = true;
      this.logger.info('Deepgram Agent initialized successfully');
    } catch (error) {
      this.logger.error('Failed to initialize Deepgram Agent', error as Error);
      throw error;
    }
  }
  ...
  async execute(request: TaskRequest, context: any): Promise<TaskResponse> {
    ...
    try {
      ...
      switch (request.taskType) {
        ...
        default:
          throw new Error(
            `Unsupported Deepgram task type: ${request.taskType}`
          );
      }
      ...
      this.logger.error('Deepgram task failed', error as Error, {
        taskId: request.taskId,
        taskType: request.taskType,
      });
   ...
   */
  private async transcribeFile(parameters: any): Promise<any> {
    ...
    let response: any;
    if (audioUrl) {
      response = await this.deepgramClient.listen.prerecorded.transcribeUrl(
        { url: audioUrl },
        transcriptionOptions
      );
    } else {
      response = await this.deepgramClient.listen.prerecorded.transcribeFile(
        audioBuffer,
        transcriptionOptions
      );
    }
   ...
   * Live transcription
   */
  private async transcribeLive(parameters: any): Promise<any> {
   ...
   * Run live agent
   */
  private async runLiveAgent(parameters: any): Promise<any> {
  ...
  private estimateCost(taskType: string, result: any): number {
    ...
    const baseCost = baseCosts[taskType as keyof typeof baseCosts] || 0.0059;
  ...
  private estimateTokens(result: any): number {
    if (!result?.transcript && !result?.finalTranscript) return 0;
