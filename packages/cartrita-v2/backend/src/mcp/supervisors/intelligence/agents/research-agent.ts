...
const { ResearcherAgent } = await import(
  '../../../../backend/src/agi/agents/ResearcherAgent.js'
);
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.researcherAgent = new ResearcherAgent({
        ...
        tavilyApiKey:
          this.config.searchEngines?.tavilyApiKey || process.env.TAVILY_API_KEY,
        serpApiKey:
          this.config.searchEngines?.serpApiKey || process.env.SERPAPI_API_KEY,
        gnewsApiKey:
          this.config.searchEngines?.gnewsApiKey || process.env.GNEWS_API_KEY,
      });
  ...
  async execute(request: TaskRequest, context: any): Promise<TaskResponse> {
    ...
    try {
      ...
      switch (request.taskType) {
        ...
        default:
          throw new Error(
            `Unsupported research task type: ${request.taskType}`
          );
      }
