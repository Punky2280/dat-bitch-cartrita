...
const { AnalyticsAgent: BackendAnalyticsAgent } = await import(
  '../../../../backend/src/agi/agents/AnalyticsAgent.js'
);
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.analyticsAgent = new BackendAnalyticsAgent({
        database: {
          host:
            this.config.database?.host || process.env.DB_HOST || 'localhost',
          port:
  ...
  async execute(request: TaskRequest, context: any): Promise<TaskResponse> {
    ...
    try {
      ...
      switch (request.taskType) {
        ...
        default:
          throw new Error(
            `Unsupported analytics task type: ${request.taskType}`
          );
      }
