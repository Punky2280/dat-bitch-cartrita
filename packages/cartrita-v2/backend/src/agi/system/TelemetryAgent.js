...
export default class TelemetryAgent extends BaseAgent {
  ...
  async initialize() {
    try {
      ...
      this.agentMetrics = {
        operationsExecuted: this.meter.createCounter(
          'telemetry_agent_operations_total',
          {
            description: 'Total telemetry operations executed by the agent',
          }
        ),
        tracesGenerated: this.meter.createCounter(
          'telemetry_agent_traces_total',
          {
            description: 'Total traces generated by the telemetry agent',
          }
        ),
        metricsCollected: this.meter.createCounter(
          'telemetry_agent_metrics_total',
          {
            description: 'Total metrics collected by the telemetry agent',
          }
        ),
        executionTime: this.meter.createHistogram(
          'telemetry_agent_execution_duration_ms',
          {
      ...
      console.log(
        '[TelemetryAgent] ✅ Telemetry Agent initialized successfully'
      );
  ...
  async processMessage(message, context = {}) {
    ...
    return await this.tracer.startActiveSpan(
      ...
      async span => {
        try {
          const result = await this.executeTelemetryOperation(
            operationType,
            message,
            context
          );
  ...
  classifyTelemetryRequest(message) {
    ...
    if (lowerMessage.includes('trace') || lowerMessage.includes('span')) {
      ...
      lowerMessage.includes('histogram')
    ) {
      return 'metrics';
    } else if (
  ...
  async handleTracingRequest(message, context) {
    ...
    switch (operation.type) {
      case 'create_trace':
        return await this.createCustomTrace(
          operation.name,
      ...
      default:
        return {
          ...
          type: 'tracing_help',
          message:
            'Available tracing operations: create custom trace, analyze recent traces, demo trace scenario',
          examples: [
  ...
  async handleMetricsRequest(message, context) {
    ...
    switch (operation.type) {
      case 'create_counter':
        return await this.createCustomCounter(
          operation.name,
          operation.description
        );
      case 'create_histogram':
        return await this.createCustomHistogram(
          operation.name,
          operation.description
        );
      case 'record_metric':
        return await this.recordCustomMetric(
      ...
      default:
        return {
          ...
          type: 'metrics_help',
          message:
            'Available metrics operations: create counter, create histogram, record metric, show current metrics',
          examples: [
        ...
        try {
          ...
          span.setAttributes({
            'performance.memory_usage_mb':
              performanceData.memoryUsage.heapUsed / 1024 / 1024,
      ...
      async span => {
        ...
        span.setAttributes({
          'analysis.traces_analyzed':
            analysis.trace_statistics.total_traces || 0,
  ...
  async createCustomTrace(name, attributes = {}, context = {}) {
    return await this.baseTelemetry.traceOperation(
      name,
      {
        kind: SpanKind.INTERNAL,
        attributes: {
  ...
  recordOperationMetrics(operationType, duration, success, error = null) {
    try {
      ...
      if (operationType === 'tracing' && this.agentMetrics.tracesGenerated) {
        this.agentMetrics.tracesGenerated.add(1, {
          success: success.toString(),
        });
      }

      if (operationType === 'metrics' && this.agentMetrics.metricsCollected) {
        this.agentMetrics.metricsCollected.add(1, {
          success: success.toString(),
        });
      }
      ...
      console.warn(
        '[TelemetryAgent] Failed to record operation metrics:',
        metricsError
      );
    }
  }
  ...
  extractTracingOperation(message) {
    ...
    if (lowerMessage.includes('create') && lowerMessage.includes('trace')) {
      const nameMatch =
        message.match(/trace (?:for |named |called )?['"](.*?)['"]/) ||
  ...
  extractMetricsOperation(message) {
    ...
    if (lowerMessage.includes('create counter')) {
      const nameMatch =
        message.match(/counter (?:named |called )?['"](.*?)['"]/) ||
      ...
      const nameMatch =
        message.match(/histogram (?:named |called )?['"](.*?)['"]/) ||
  ...
  async getTelemetryManual() {
    return {
      overview: {
        ...
        description:
          'Complete guide to using the Telemetry Agent for advanced observability operations',
        version: '1.0.0',
      ...
      metrics: {
        ...
        description:
          'Create and manage custom metrics for business and technical monitoring',
        operations: [
          {
          ...
          },
          {
            ...
            description:
              'Creates a histogram metric for measuring distributions',
          },
          {
        ...
        operations: [
          {
            ...
            description:
              'Provides comprehensive performance analysis including memory, CPU, and response times',
          },
          {
      ...
      analysis: {
        ...
        description:
          'Comprehensive analysis of telemetry data with insights and recommendations',
        operations: [
          {
            command: 'Analyze telemetry data',
            description:
              'Provides system health, trace statistics, and performance trends analysis',
          },
          {
            command: 'Generate telemetry report',
            description:
              'Creates detailed telemetry report with actionable insights',
          },
        ],
      },
      status: {
        ...
        operations: [
          {
            ...
            description:
              'Shows status of all telemetry components and services',
          },
          {
  ...
  extractDescription(message) {
    const descMatch =
      message.match(/(?:description|desc|for) ['"](.*?)['"]/) ||
      message.match(/(?:description|desc|for) (.*?)(?:\s*$)/);
    return descMatch
      ? descMatch[1].trim()
      : 'Custom metric created by Telemetry Agent';
  }
  ...
  generatePerformanceRecommendations(analysis) {
    ...
    if (analysis.memory_analysis.heap_utilization_percent > 80) {
      recommendations.push({
        ...
        priority: 'high',
        message:
          'High memory utilization detected. Consider investigating memory leaks or increasing heap size.',
      });
    }
    ...
    if (recommendations.length === 0) {
      recommendations.push({
        ...
        priority: 'low',
        message:
          'System performance appears healthy. Continue regular monitoring.',
      });
    }
  ...
  calculateHealthScore(status) {
    ...
    const memoryUsage =
      status.system_metrics.memory.heapUsed /
      status.system_metrics.memory.heapTotal;
  ...
  extractMetricName(message) {
    return 'extracted_metric';
  }
  extractValue(message) {
    return 1;
  }
  extractLabels(message) {
    return {};
  }
  extractFilter(message) {
    return null;
  }
  extractDocumentationTopic(message) {
    return 'overview';
  }
  ...
  async getTraceStatistics() {
    return { total_traces: 150 };
  }
  async getMetricSummary() {
    return { active_metrics: 25 };
  }
