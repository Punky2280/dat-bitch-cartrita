...
export class ToolEmbeddingService {
  ...
  buildInputPayload(tool) {
    ...
    return [
      ...
      tool.category || '',
      tool.schema
        ? JSON.stringify(tool.schema._def ? tool.schema._def : tool.schema)
        : '',
    ]
      .join('\n')
  ...
  async generateEmbedding(tool) {
    ...
    try {
      ...
      return (
        '[' +
        vector
          .map(v => (typeof v === 'number' ? v.toFixed(6) : '0.000000'))
          .join(',') +
        ']'
      );
    } catch (e) {
      console.warn(
        '[ToolEmbeddingService] embedding generation failed',
        e.message
      );
      return null;
    }
  ...
  async upsertEmbedding(toolName, embeddingText) {
    ...
    try {
      ...
      await db.query(
        ...
        VALUES ($1,$2, now())
        ON CONFLICT (tool_id) DO UPDATE SET embedding=EXCLUDED.embedding, updated_at=now()`,
        [toolId, embeddingText]
      );
    } catch (e) {
