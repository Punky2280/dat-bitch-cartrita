...
export async function registerAgentTools(registry) {
    ...
    func: async ({ image_url, analysis_type }) => {
      ...
      try {
        ...
        const analysisPrompts = {
          ...
          style: 'Analyze style',
          composition: 'Analyze composition',
          technical: 'Analyze technical quality',
          ...
          messages: [
            {
              role: 'user',
              content: [
                { type: 'text', text: analysisPrompts[analysis_type] },
                {
                  type: 'image_url',
        ...
        return { success: false, error: e.message, image_url, analysis_type };
      }
    },
  });
  ...
  try {
    ...
    calc.category = 'coding';
    registry.tools.set('calculator', calc);
  } catch (e) {}
  ...
  registry.registerTool({
    ...
    func: async ({ code, language, focus = 'all' }) => {
      if (process.env.SKIP_OPENAI === '1' || !process.env.OPENAI_API_KEY)
        return { skipped: true, reason: 'OPENAI disabled', language };
      const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
      ...
      try {
        ...
        parsed = { summary: resp.choices[0].message.content };
      }
