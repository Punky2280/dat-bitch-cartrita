  ...
  registry.registerTool({
    ...
    func: async ({ query, user_id, limit = 10 }) => {
      try {
        ...
        const result =
          db && db.query ? await db.query(dbQuery, params) : { rows: [] };
        ...
        if (result.rows.length === 0 && process.env.OPENAI_API_KEY) {
          try {
            ...
            console.warn(
              '[utilityRegistry] semantic fallback failed:',
              e.message
            );
          }
        }
        ...
        const unique = allResults.filter(
          (r, i, a) => i === a.findIndex(t => t.id === r.id)
        );
        return {
          success: true,
          query,
          ...
          search_type:
            unique.length > (result.rows || []).length ? 'semantic' : 'keyword',
