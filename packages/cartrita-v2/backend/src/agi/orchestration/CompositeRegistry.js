...
export class CompositeRegistry {
  ...
  async initialize() {
    const maxPhase =
      process.env.REGISTRY_PHASE_MAX !== undefined
        ? Number(process.env.REGISTRY_PHASE_MAX)
        : Infinity;
    const include = process.env.REGISTRY_INCLUDE
      ? new Set(process.env.REGISTRY_INCLUDE.split(',').map(s => s.trim()))
      : null;
    const timeoutMs = process.env.REGISTRY_TIMEOUT_MS
      ? Number(process.env.REGISTRY_TIMEOUT_MS)
      : 3000;

    const phases = [...new Set(this.registerFns.map(r => r.phase))].sort(
      (a, b) => a - b
    );
    for (const phase of phases) {
      ...
      const group = this.registerFns.filter(
        r => r.phase === phase && (!include || include.has(r.name))
      );
      ...
      await Promise.all(
        group.map(async reg => {
          ...
          let status = 'ok',
            toolCountBefore = this.tools.size;
          try {
            let timeoutHandle;
            const race = await Promise.race([
              (async () => {
                await reg.fn(this);
                if (
                  process.env.REGISTRY_PERSIST === '1' &&
          ...
          phaseSummary.registries.push({
            name: reg.name,
            status,
            durationMs: Math.round(performance.now() - start),
            added: this.tools.size - toolCountBefore,
          });
        })
      );
      ...
      func: async input => {
        try {
          ...
          return JSON.stringify(
            { success: false, error: e.message, tool: def.name },
            null,
            2
          );
        }
  ...
  async persistNewTools() {
    ...
    for (const [name, def] of this.tools) {
      try {
        await db.query(
          `INSERT INTO tool_definitions(name, category, version, description, schema_json)
          VALUES ($1,$2,$3,$4,$5)
          ...
          [
            ...
            JSON.stringify(
              def.schema?._def ? def.schema._def : def.schema || {}
            ),
          ]
        );
        ...
        console.warn(
          '[CompositeRegistry] persistence error for tool',
          name,
          e.message
        );
      }
