...
class EnhancedGPT5SupervisorAgent {
  ...
  async initialize() {
    try {
      console.log(
        '[EnhancedGPT5Supervisor] 🚀 Initializing V2 hierarchical multi-agent system...'
      );
      ...
      console.log(
        `[EnhancedGPT5Supervisor] 🎯 Using optimal model: ${supervisorModel.model} (reasoning: ${supervisorModel.reasoning})`
      );
      ...
      console.log(
        '[EnhancedGPT5Supervisor] 🛠️ Tool registry initialized with advanced capabilities'
      );
      ...
      console.log(
        '[EnhancedGPT5Supervisor] ✅ V2 Enhanced supervisor ready with GPT-5 capabilities'
      );

      return true;
    } catch (error) {
      console.error(
        '[EnhancedGPT5Supervisor] ❌ Initialization failed:',
        error
      );
      this.isInitialized = false;
      return false;
  ...
  loadAgentModelAssignments() {
    ...
    console.log(
      `[EnhancedGPT5Supervisor] 📊 Loaded ${this.modelAssignments.size} agent model assignments`
    );
  }
  ...
  async loadSubAgentsWithOptimalModels() {
    const agentFileNames = [
      ...
      'IntegrationAgent.js',
      'ArtistAgent.js',
      'KnowledgeCuratorAgent.js',
      'SecurityAgent.js',
      'WorkflowMaestroAgent.js',
      'FileOrganizerAgent.js',
      'ResearcherAgent.js',
      'ComputerUseAgent.js', // V2: Added Computer Use Agent
    ...
    console.log(
      `[EnhancedGPT5Supervisor] 📦 Loading ${agentFileNames.length} sub-agents with V2 enhancements...`
    );
    ...
    const loadPromises = agentFileNames.map(async fileName => {
      try {
        ...
        console.log(
          `[EnhancedGPT5Supervisor] ✅ ${agentName} loaded with ${modelConfig.model} (${modelConfig.reasoning} reasoning)`
        );
      } catch (error) {
        const agentName = fileName.replace('.js', '');
        console.warn(
          `[EnhancedGPT5Supervisor] ⚠️ Failed to load ${agentName}:`,
          error.message
        );
      }
    });
    ...
    console.log(
      `[EnhancedGPT5Supervisor] 🎉 Loaded ${this.subAgents.size} sub-agents with V2 optimizations`
    );
  }
  ...
  async buildEnhancedStateGraph() {
    ...
    for (const [agentName, agentData] of this.subAgents) {
      workflow.addNode(
        agentName,
        this.createEnhancedSubAgentNode(agentName, agentData)
      );
    }

    // Enhanced routing with intelligent model selection
    workflow.addConditionalEdges('cartrita', this.createEnhancedRouter(), [
      'END',
      ...Array.from(this.subAgents.keys()),
    ]);
    ...
    console.log(
      '[EnhancedGPT5Supervisor] 🔗 Enhanced state graph built with V2 routing'
    );
  }
        ...
        async () => {
          try {
            ...
            const processingConfig =
              this.determineProcessingConfig(requestAnalysis);
  ...
  async generateEnhancedResponse(state, processingConfig) {
    ...
    const enhancedResponse =
      await this.gpt5Service.generateWithAdvancedFeatures(
  ...
  analyzeRequestComplexity(state) {
    ...
    const hasComplexRequests =
      /create|build|implement|develop|analyze|optimize/i.test(content);
...
Remember: Always maintain Cartrita's personality while leveraging V2's advanced capabilities for optimal user experience.`;
  ...
  parseEnhancedDecision(responseContent) {
    try {
      ...
      console.warn(
        '[EnhancedGPT5Supervisor] Failed to parse JSON decision, using fallback'
      );
    }
  ...
  createEnhancedRouter() {
    return state => {
      ...
      if (
        !lastMessage.content ||
        lastMessage.content.includes('"action": "respond"')
      ) {
        return 'END';
      }
      ...
      try {
        ...
        console.warn(
          '[EnhancedGPT5Supervisor] Router decision parsing failed:',
          error
        );
      }
  ...
  optimizeAgentModelIfNeeded(agentName) {
    ...
    if (metrics.successRate < 90 && !assignment.model.includes('gpt-5')) {
      console.log(
        `[EnhancedGPT5Supervisor] 🎯 Optimizing model for ${agentName}: upgrading for better performance`
      );
    ...
    if (metrics.averageResponseTime > 2000 && assignment.model === 'gpt-5') {
      console.log(
        `[EnhancedGPT5Supervisor] ⚡ Optimizing model for ${agentName}: switching to faster model`
      );
      ...
      return await OpenTelemetryTracing.traceAgentOperation(
        ...
        async () => {
          try {
            ...
            console.error(
              `[EnhancedGPT5Supervisor] Agent ${agentName} execution failed:`,
              error
            );
  ...
  async processEnhancedRequest(messages, userId = null) {
    ...
    try {
      ...
      console.log(
        `[EnhancedGPT5Supervisor] ✅ Enhanced request processed in ${responseTime}ms with V2 features`
      );
      ...
      console.error(
        '[EnhancedGPT5Supervisor] Enhanced request processing failed:',
        error
      );
      throw error;
    }
  ...
  createErrorResponse(error, state) {
    return {
      messages: [
        new AIMessage({
          content:
            'I apologize, mi amor, but I encountered an issue processing your request. My V2 systems are working to resolve this. Please try again in a moment. 💫',
