...
that connect with audiences and drive real business results with style and impact.`,
  ...
  async invoke(state) {
    ...
    try {
      ...
      const creativeAnalysis = await this.analyzeCreativeRequest(
        creativeRequest,
        state
      );
      ...
      let response = await this.generateCreativeResponse(
        creativeAnalysis,
        creativeStrategy,
        state
      );
      ...
      response = await this.enhanceWithDesignPrinciples(
        response,
        creativeAnalysis
      );

      // Add creative personality with Miami flair
      response = this.enhanceWithCreativePersonality(
        response,
        creativeAnalysis
      );

      // Execute creative tools
      const toolResults = await this.executeCreativeTools(
        creativeAnalysis,
        state
      );
  ...
  async analyzeCreativeRequest(request, state) {
    ...
    const requestPatterns = {
      ...
      marketing_creative:
        /campaign|ad|banner|marketing|promotional|conversion/i,
    ...
    analysis.designRecommendations =
      this.generateDesignRecommendations(analysis);
  ...
  generateDesignRecommendations(analysis) {
    ...
    if (analysis.requestType === 'brand_development') {
      recommendations.push({
        ...
        suggestion:
          'Develop a cohesive brand identity system with consistent visual elements',
        priority: 'high',
    ...
    if (
      analysis.requestType === 'web_design' ||
      analysis.requestType === 'user_experience'
    ) {
      recommendations.push({
        category: 'user_experience',
        suggestion:
          'Implement mobile-first responsive design with intuitive navigation',
        priority: 'high',
    ...
    if (analysis.requestType === 'marketing_creative') {
      recommendations.push({
        ...
        suggestion:
          'Design with clear call-to-actions and persuasive visual hierarchy',
        priority: 'high',
    ...
    if (analysis.requestType === 'content_creation') {
      recommendations.push({
        ...
        suggestion:
          'Develop content templates and visual consistency across platforms',
        priority: 'medium',
  ...
  generateBrandGuidelines(analysis) {
    ...
    if (analysis.designElements.includes('color_scheme')) {
      guidelines.push({
        ...
        guideline:
          'Primary, secondary, and accent colors with hex codes and usage rules',
    ...
    if (analysis.designElements.includes('typography')) {
      guidelines.push({
        ...
        guideline:
          'Heading, body, and accent fonts with size scales and usage hierarchy',
    ...
    if (analysis.designElements.includes('logo')) {
      guidelines.push({
        element: 'Logo Usage',
        guideline:
          'Logo variations, minimum sizes, clear space, and usage restrictions',
  ...
  createCreativeIntroduction(analysis) {
    const intros = {
      brand_development:
        "Time to build a brand that absolutely slays! Let me craft an identity that's both memorable and profitable:",
      web_design:
        "Website design mode activated! I'm creating a digital experience that converts visitors into customers:",
      marketing_creative:
        "Campaign creative that actually works? That's my specialty! Here's how we make it irresistible:",
      content_creation:
        "Content that stops the scroll and drives action? I've got the formula down to a science:",
      visual_design:
        'Visual storytelling time! Let me create graphics that speak louder than words:',
      default:
        "Creative challenge accepted! Here's how I'm going to make this design work harder for your business:",
    };
  ...
  generateDesignPrinciplesGuidance(analysis) {
    ...
    for (const [principle, description] of Object.entries(
      ...
      if (Math.random() < 0.6) {
        // Show subset to avoid overwhelming
    ...
    guidance += '\n**Next Steps:**\n';
    guidance +=
      'Ready to bring this creative vision to life? I can start with concept development or dive straight into the design execution - whatever gets your brand looking amazing faster! 🎨';
