...
class ComputerUseAgent extends BaseAgent {
  constructor() {
    super(
      'computer_use',
      'sub',
      ['computer_control', 'automation', 'web_browsing', 'application_control'],
    ...
    this.pythonAgentPath = path.resolve(
      __dirname,
      '../../../../cartrita-v2/src/agents/computer_use_agent.py'
    );
    ...
    console.log(
      '[ComputerUseAgent] 🖥️ OpenAI Computer Use Agent initialized with hierarchical supervision'
    );
  }
  ...
  async invoke(state) {
    ...
    return OpenTelemetryTracing.traceAgentOperation(
      ...
      async span => {
        try {
          console.log(
            '[ComputerUseAgent] 🖥️ Processing computer use request with supervision'
          );
          ...
          const supervisorAuth = await this.requestSupervisorAuthorization(
            taskDescription,
            state
          );
          ...
          const vaultAccess = await this.requestVaultAccess(
          ...
          console.error(
            '[ComputerUseAgent] ❌ Computer use task failed:',
            error
          );
          this.computerMetrics.failed_tasks++;
  ...
  async requestSupervisorAuthorization(taskDescription, state) {
    ...
    console.log(
      `[ComputerUseAgent] 🔐 Requesting supervisor authorization: ${transactionId}`
    );
    ...
    if (
      safetyCheck.risk_level === 'high' ||
      safetyCheck.risk_level === 'critical'
    ) {
      return {
        approved: false,
  ...
  async requestVaultAccess(transactionId) {
    console.log(
      `[ComputerUseAgent] 🗝️ Requesting vault access for transaction: ${transactionId}`
    );
  ...
  async executeComputerTask(taskDescription, transactionId, apiKeyHash, state) {
    console.log(
      `[ComputerUseAgent] 🚀 Executing computer task: ${transactionId}`
    );
    ...
    const taskFile = path.join(
      __dirname,
      `../../../temp/computer_task_${transactionId}.json`
    );
  ...
  async executePythonAgent(taskFile) {
    return new Promise((resolve, reject) => {
      console.log(
        `[ComputerUseAgent] 🐍 Launching Python agent with task file: ${taskFile}`
      );

      const pythonProcess = spawn(
        'python3',
        [this.pythonAgentPath, '--task-file', taskFile],
        {
      ...
      pythonProcess.on('close', code => {
        console.log(
          `[ComputerUseAgent] 🐍 Python agent exited with code: ${code}`
        );
        ...
        try {
          ...
          console.log(
            `[ComputerUseAgent] ✅ Python agent completed successfully`
          );
          resolve(result);
        } catch (parseError) {
          console.error(
            '[ComputerUseAgent] ❌ Failed to parse Python agent output:',
            parseError
          );
          console.error('Raw output:', stdout);
          reject(
            new Error(
              `Failed to parse Python agent output: ${parseError.message}`
            )
          );
        }
      });
      ...
      pythonProcess.on('error', error => {
        console.error(
          '[ComputerUseAgent] ❌ Python agent process error:',
          error
        );
      ...
      setTimeout(() => {
        if (!pythonProcess.killed) {
          console.warn(
            '[ComputerUseAgent] ⏰ Python agent timeout, killing process'
          );
  ...
  formatTaskResponse(taskResult) {
    ...
    const duration = taskResult.duration_seconds
      ? ` in ${taskResult.duration_seconds.toFixed(1)}s`
      : '';
    const iterations = taskResult.iterations
      ? ` (${taskResult.iterations} steps)`
      : '';
  ...
  checkPythonAgentAvailable() {
    try {
      ...
      console.warn(
        `[ComputerUseAgent] ⚠️ Python agent not accessible: ${error.message}`
      );
      return false;
    }
  }
