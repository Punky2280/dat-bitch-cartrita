...
class CartritaSupervisorAgent {
  ...
  async ensureHFBridgeAgents() {
    const required = [
      {
        name: 'VisionMaster',
        path: '../../integrations/huggingface/bridge/HFVisionAgent.js',
      },
      {
        name: 'AudioWizard',
        path: '../../integrations/huggingface/bridge/HFAudioAgent.js',
      },
      {
        name: 'LanguageMaestro',
        path: '../../integrations/huggingface/bridge/HFLanguageAgent.js',
      },
      {
        name: 'MultiModalOracle',
        path: '../../integrations/huggingface/bridge/HFMultimodalAgent.js',
      },
      {
        name: 'DataSage',
        path: '../../integrations/huggingface/bridge/HFDataAgent.js',
      },
    ];
    for (const spec of required) {
      ...
      try {
        console.log(
          `[CartritaSupervisor] ➕ Adding missing HF bridge agent: ${spec.name}`
        );
        ...
        if (HFBridge) {
          ...
          console.log(
            `[CartritaSupervisor] ✅ HF bridge agent registered (fallback): ${instance.config.name}`
          );
        }
      } catch (e) {
        console.warn(
          `[CartritaSupervisor] ⚠️ Failed fallback registration for ${spec.name}: ${e.message}`
        );
      }
    }
  }
  ...
  async registerSubAgents() {
    ...
    const agentFileNames = [
      // NEW: 15 Sophisticated Task Agents with Advanced Prompt Engineering
      'CartritaCoreAgent.js', // Primary interface agent with Miami personality
      'CodeMaestroAgent.js', // Advanced development expert
      'DataScienceWizardAgent.js', // Analytics and intelligence specialist
      'CreativeDirectorAgent.js', // Design and content excellence expert
      'ProductivityMasterAgent.js', // Task and project management expert
      'SecurityGuardianAgent.js', // Cybersecurity and privacy expert
      'BusinessStrategyAgent.js', // Market intelligence and strategic planning
      'ResearchIntelligenceAgent.js', // Information gathering and analysis expert
      'CommunicationExpertAgent.js', // Messaging and relationship management
      'MultimodalFusionAgent.js', // Vision, audio, text integration expert
      'PersonalizationExpertAgent.js', // User experience personalization specialist
      'IntegrationMasterAgent.js', // System connectivity and API expert
      'QualityAssuranceAgent.js', // Testing, validation, and reliability expert
      'EmergencyResponseAgent.js', // Crisis management and rapid response
      'AutomationArchitectAgent.js', // Workflow and process optimization expert

      // Computer Use Agent - OpenAI Computer-Use-Preview Integration
      'ComputerUseAgent.js', // OpenAI computer use with hierarchical supervision

      // Legacy Core consciousness agents (14)
      'CodeWriterAgent.js',
      'ResearcherAgent.js',
      'ArtistAgent.js',
      'WriterAgent.js',
    ...
    for (const fileName of agentFileNames) {
      try {
        ...
        if (importPath.includes('huggingface')) {
          console.log(
            `[CartritaSupervisor] 🔍 Importing HF bridge agent from: ${importPath}`
          );
        }
        ...
        console.error(
          `[CartritaSupervisor] ❌ Failed to load ${fileName}: ${error.message}`
        );
  ...
  async createSupervisorNode(state) {
    ...
    for (const { agent, patterns } of modalityMap) {
      ...
      if (matched) {
        try {
          global.otelCounters?.fastDelegations?.add(1, {
            agent,
            available: this.subAgents.has(agent) ? 'yes' : 'no',
          });
        } catch (_) {}
        try {
          if (global.debugMetrics) global.debugMetrics.fast_path_delegations++;
        } catch (_) {}
        if (this.subAgents.has(agent)) {
          console.log(
            `[CartritaSupervisor] ⚡ Fast-path delegating to ${agent} based on keyword match`
          );
          return {
            messages: [
              new AIMessage(`Routing to ${agent} for specialized processing.`),
            ],
            next_agent: agent,
          ...
          console.log(
            `[CartritaSupervisor] ⚡ Fast-path (no subAgent) recognized ${agent} modality; continuing inline`
          );
