...
and deliver insights that drive real business decisions with Cartrita's confidence.`,
  ...
  async invoke(state) {
    ...
    try {
      ...
      let response = await this.generateAnalyticalResponse(
        dataAnalysis,
        analysisStrategy,
        state
      );
      ...
      const toolResults = await this.executeAnalyticalTools(
        dataAnalysis,
        state
      );
  ...
  async analyzeDataRequest(request, state) {
    ...
    analysis.businessRecommendations =
      this.generateBusinessRecommendations(analysis);
  ...
  generateBusinessRecommendations(analysis) {
    ...
    if (analysis.dataTypes.includes('customer_data')) {
      recommendations.push({
        ...
        recommendation:
          'Implement customer segmentation for personalized experiences',
        impact: 'high',
        effort: 'medium',
      });
    }
    ...
    if (analysis.dataTypes.includes('sales_data')) {
      recommendations.push({
        ...
        recommendation:
          'Develop dynamic pricing model based on demand patterns',
        impact: 'high',
        effort: 'high',
      });
    }
    ...
    if (analysis.requestType === 'predictive') {
      recommendations.push({
        ...
        recommendation:
          'Establish automated forecasting pipeline for business planning',
        impact: 'medium',
        effort: 'medium',
      });
    }
  ...
  createAnalyticalIntroduction(analysis) {
    const intros = {
      exploratory:
        'Time to dive deep into this data! Let me uncover the patterns and stories hidden in these numbers:',
      predictive:
        "Prediction mode activated! I'm going to build some powerful models to forecast what's coming:",
      descriptive:
        "Let's make this data tell its story! I'll create visualizations that make the insights pop:",
      prescriptive:
        "Strategic analysis time! I'll analyze the data and give you actionable recommendations:",
      diagnostic:
        "Detective work begins! Let me investigate what's driving these patterns:",
      default:
        "Data science magic incoming! Here's how I'm going to extract maximum value from this data:",
    };
  ...
  generateBusinessInsights(analysis) {
    ...
    insights += '\n**Next Steps:**\n';
    insights +=
      'Ready to dive into the analysis? I can start with data exploration or jump straight into the modeling - whatever gets you the insights faster!';
