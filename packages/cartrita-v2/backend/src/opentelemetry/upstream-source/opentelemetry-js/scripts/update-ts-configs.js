...
const packageJsonDependencyFields = [
  'dependencies',
  'peerDependencies',
  'devDependencies',
];
const tsConfigMergeKeys = ['compilerOptions', 'include', 'files'];
...
function main() {
  ...
  for (const packageMeta of workspacePackages.values()) {
    generateTsConfig(
      projectRoot,
      workspacePackages,
      path.join(projectRoot, packageMeta.dir),
      false,
      packageMeta
    );
  }
...
function generateTsConfig(
  ...
  const dependenciesDir = resolveDependencyDirs(
    workspacePackages,
    otelDependencies
  );
  const references = dependenciesDir
    .map(it => path.relative(pkgRoot, path.join(projectRoot, it)))
    .sort();
...
function writeRootTsConfigJson(pkgRoot, projectRoot, workspacePackages) {
  ...
  const references = Array.from(workspacePackages.values())
    ...
    .map(it => toPosix(path.relative(pkgRoot, path.join(projectRoot, it.dir))))
    .sort();
  ...
  for (const tsconfigName of ['tsconfig.esm.json', 'tsconfig.esnext.json']) {
    ...
    const references = Array.from(workspacePackages.values())
      ...
      .map(it =>
        toPosix(path.relative(pkgRoot, path.join(projectRoot, it.dir)))
      )
      .sort();
...
function getOtelDependencies(packageJson) {
  ...
  for (const type of packageJsonDependencyFields) {
    ...
    Object.keys(packageJson[type])
      .filter(it => it.startsWith('@opentelemetry'))
...
function resolveWorkspacePackages(projectRoot) {
  ...
  for (const pkgDefinition of packageJson.workspaces) {
    ...
    if (pkgDefinition.endsWith('*')) {
      ...
      const pkgs = fs
        .readdirSync(path.join(projectRoot, relDir))
