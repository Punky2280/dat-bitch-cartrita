...
function isDeprecated(src, k) {
  ...
  if (!match) {
    throw new Error(
      `could not find the "${k}" export in semconv build/esnext/ source files`
    );
  }
  ...
  while (idx >= 0) {
    if (src[idx] === '/' && src[idx + 1] === '*') {
      ...
      const deprecatedMsgMatch = /^\s*\*\s*@deprecated\s+(.*)$/m.exec(
        blockComment
      );
...
function summarizeChanges({ prev, curr, prevSrc, currSrc }) {
    ...
    if (!prevNames.has(k)) {
      ...
      if (!isPrevDeprecated && isCurrDeprecated) {
        changes.push({
          ...
          k,
          v: curr[k],
          deprecatedResult: isCurrDeprecated,
        });
  ...
  if (removed.length) {
    execSummaryFromChangeType.removed = `${removed.length} removed export${
      removed.length === 1 ? '' : 's'
    }`;
    if (summary.length) {
      summary.push('');
    ...
    removed.forEach(ch => {
      if (last && ch.ns !== last.ns) {
        summary.push('');
      }
      const cindent = ' '.repeat(longest - ch.k.length + 1);

      const prevVRepr = ch.prevV.includes('_VALUE_')
        ? JSON.stringify(ch.prevV)
        : ch.prevV;
  ...
  if (changed.length) {
    execSummaryFromChangeType.changed = `${changed.length} exported value${
      changed.length === 1 ? '' : 's'
    } changed`;
    if (summary.length) {
      summary.push('');
    ...
    changed.forEach(ch => {
      ...
      const prevVRepr = ch.k.includes('_VALUE_')
        ? JSON.stringify(ch.prevV)
        : ch.prevV;
  ...
  if (deprecated.length) {
    execSummaryFromChangeType.deprecated = `${
      deprecated.length
    } newly deprecated export${deprecated.length === 1 ? '' : 's'}`;
    if (summary.length) {
      summary.push('');
    }
    let last;
    const longest = deprecated.reduce(
      (acc, ch) => Math.max(acc, ch.k.length),
      0
    );
  ...
  if (undeprecated.length) {
    execSummaryFromChangeType.undeprecated = `${
      undeprecated.length
    } newly undeprecated export${undeprecated.length === 1 ? '' : 's'}`;
    if (summary.length) {
      summary.push('');
    }
    let last;
    const longest = undeprecated.reduce(
      (acc, ch) => Math.max(acc, ch.k.length),
      0
    );
  ...
  if (added.length) {
    execSummaryFromChangeType.added = `${added.length} added export${
      added.length === 1 ? '' : 's'
    }`;
    let last, lastAttr;
...
function semconvChangelogGen(aVer = undefined, bVer = undefined) {
  ...
  if (!fs.existsSync(aDir)) {
    console.log(
      `Downloading and extracting @opentelemetry/semantic-conventions@${aVer}`
    );
  ...
  if (!bVer) {
    ...
    if (!specVerMatch) {
      throw new Error(
        `could not determine current semconv SPEC_VERSION: ${specVerRe} did not match in ${generateShPath}`
      );
    }
    ...
    console.log(
      `Downloading and extracting @opentelemetry/semantic-conventions@${bVer}`
    );
  ...
  const stableChInfo = summarizeChanges({
    ...
    prev: Object.assign(
      ...globSync(path.join(aDir, 'build/src/stable_*.js')).map(require)
    ),
    curr: Object.assign(
      ...globSync(path.join(bDir, 'build/src/stable_*.js')).map(require)
    ),
  ...
  const unstableChInfo = summarizeChanges({
    prev: Object.assign(
      ...globSync(path.join(aDir, 'build/src/experimental_*.js')).map(require)
    ),
    curr: Object.assign(
      ...globSync(path.join(bDir, 'build/src/experimental_*.js')).map(require)
    ),
...
* feat: update semantic conventions to ${bSemconvVer} [#NNNN]
  * Semantic Conventions ${bSemconvVer}: [changelog](https://github.com/open-telemetry/semantic-conventions/blob/main/CHANGELOG.md#${slugify(
    bSemconvVer
  )}) | [latest docs](https://opentelemetry.io/docs/specs/semconv/)
  * \`@opentelemetry/semantic-conventions\` (stable) changes: *${execSummaryFromChInfo(
    stableChInfo
  )}*
  * \`@opentelemetry/semantic-conventions/incubating\` (unstable) changes: *${execSummaryFromChInfo(
    unstableChInfo
  )}*
`,
  ];
...
function main() {
  ...
  console.log(
    'The following could be added to the top "Enhancement" section of "semantic-conventions/CHANGELOG.md":'
  );
  console.log('\n- - -');
  console.log(s);
  console.log('- - -');
