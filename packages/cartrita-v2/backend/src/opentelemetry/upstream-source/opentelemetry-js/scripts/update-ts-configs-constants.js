...
function getDefaultTsConfig(pkgRoot, projectRoot) {
  return {
    extends: toPosix(
      path.relative(pkgRoot, path.join(projectRoot, 'tsconfig.base.json'))
    ),
    compilerOptions: {
      rootDir: '.',
      outDir: 'build',
...
function getEsmTsConfig(pkgRoot, projectRoot) {
  return {
    extends: toPosix(
      path.relative(pkgRoot, path.join(projectRoot, 'tsconfig.base.esm.json'))
    ),
    compilerOptions: {
      rootDir: 'src',
      outDir: 'build/esm',
...
function getEsnextTsConfig(pkgRoot, projectRoot) {
  return {
    extends: toPosix(
      path.relative(
        pkgRoot,
        path.join(projectRoot, 'tsconfig.base.esnext.json')
      )
    ),
    compilerOptions: {
      rootDir: 'src',
