...
class ApiRateLimiter {
  ...
  async queueRequest(apiCall, estimatedTokens = 1000, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      console.log(
        `[ApiRateLimiter] 📝 Queued request ${request.id} (${this.requestQueue.length} in queue)`
      );
      this.processQueue();
    });
  }
  ...
  async processQueue() {
    ...
    if (this.requestTokens < 1) {
      console.log(
        '[ApiRateLimiter] ⏳ Insufficient request tokens, waiting...'
      );
      return;
    }
    ...
    if (this.tokenTokens < nextRequest.estimatedTokens) {
      console.log(
        `[ApiRateLimiter] ⏳ Insufficient token capacity (need ${nextRequest.estimatedTokens}, have ${Math.floor(this.tokenTokens)})`
      );
      return;
    }
    ...
    console.log(
      `[ApiRateLimiter] 🚀 Executing request ${request.id} (${this.activeRequests} active)`
    );
  ...
  async executeWithRetry(request) {
    ...
    try {
      ...
      console.log(
        `[ApiRateLimiter] ✅ Request ${id} completed (${actualTokens} tokens)`
      );
      return result;
    } catch (error) {
      ...
      if (isRateLimitError && retries < maxRetries) {
        ...
        console.log(
          `[ApiRateLimiter] 🔄 Rate limit hit, retrying ${id} in ${delay}ms (attempt ${retries + 1}/${maxRetries + 1})`
        );
  ...
  cleanupHistory() {
    ...
    if (
      beforeReq > this.requestHistory.length ||
      beforeTok > this.tokenHistory.length
    ) {
      console.log(
        `[ApiRateLimiter] 🧹 Cleaned up history: ${beforeReq - this.requestHistory.length} requests, ${beforeTok - this.tokenHistory.length} tokens`
      );
    }
  }
