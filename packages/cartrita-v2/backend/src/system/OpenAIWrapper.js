...
class OpenAIWrapper {
  constructor() {
    ...
    if (this.apiKey && this.apiKey.startsWith('sk-')) {
      ...
      this.openai = null;
      console.warn(
        '[OpenAIWrapper] ⚠️ No valid API key provided, wrapper will return mock responses'
      );
    }
  ...
  async createChatCompletion(params, options = {}) {
    ...
    return ApiRateLimiter.queueRequest(
      async () => {
        try {
          console.log(
            `[OpenAIWrapper] 💬 Creating chat completion (model: ${enhancedParams.model})`
          );
          const response =
            await this.openai.chat.completions.create(enhancedParams);

          console.log(
            `[OpenAIWrapper] ✅ Chat completion successful (${response.usage?.total_tokens || 'unknown'} tokens)`
          );
          return response;
        } catch (error) {
          console.error(
            `[OpenAIWrapper] ❌ Chat completion failed:`,
            error.message
          );
          throw error;
        }
  ...
  async createSpeech(params, options = {}) {
    ...
    return ApiRateLimiter.queueRequest(
      async () => {
        try {
          console.log(
            `[OpenAIWrapper] 🔊 Creating speech (voice: ${enhancedParams.voice})`
          );
          const response =
            await this.openai.audio.speech.create(enhancedParams);

          console.log(`[OpenAIWrapper] ✅ Speech generation successful`);
          return response;
        } catch (error) {
          console.error(
            `[OpenAIWrapper] ❌ Speech generation failed:`,
            error.message
          );
          throw error;
        }
      ...
      100,
      options
    ); // TTS uses fewer tokens but still counts as API call
  }
  ...
  async createTranscription(params, options = {}) {
    ...
    return ApiRateLimiter.queueRequest(
      async () => {
        try {
          console.log(
            `[OpenAIWrapper] 🎤 Creating transcription (model: ${enhancedParams.model})`
          );
          const response =
            await this.openai.audio.transcriptions.create(enhancedParams);

          console.log(`[OpenAIWrapper] ✅ Transcription successful`);
          return response;
        } catch (error) {
          console.error(
            `[OpenAIWrapper] ❌ Transcription failed:`,
            error.message
          );
          throw error;
        }
      ...
      200,
      options
    ); // Moderate token estimate for transcription
  }
  ...
  async createImage(params, options = {}) {
    ...
    return ApiRateLimiter.queueRequest(
      async () => {
        try {
          console.log(
            `[OpenAIWrapper] 🎨 Creating image (model: ${enhancedParams.model}, size: ${enhancedParams.size})`
          );
          ...
          console.error(
            `[OpenAIWrapper] ❌ Image generation failed:`,
            error.message
          );
          throw error;
        }
      ...
      500,
      options
    ); // Image generation is token-intensive
  }
  ...
  async createEmbeddings(params, options = {}) {
    ...
    return ApiRateLimiter.queueRequest(
      async () => {
        try {
          console.log(
            `[OpenAIWrapper] 📊 Creating embeddings (model: ${enhancedParams.model})`
          );
          const response = await this.openai.embeddings.create(enhancedParams);

          console.log(
            `[OpenAIWrapper] ✅ Embeddings successful (${response.usage?.total_tokens || 'unknown'} tokens)`
          );
          return response;
        } catch (error) {
  ...
  estimateTokensForChat(params) {
    ...
    const tokensPerChar =
      this.tokenEstimator[model] || this.tokenEstimator.default;
  ...
  getMockChatResponse(params) {
    const message =
      params.messages?.[params.messages.length - 1]?.content || 'Hello';
    return {
      id: 'mock-' + Date.now(),
  ...
  getMockAudioResponse() {
    return {
      ...
      blob: () =>
        Promise.resolve(new Blob(['mock audio data'], { type: 'audio/mpeg' })),
    };
  }
  ...
  getStats() {
    return {
      ...
      apiKey: this.apiKey
        ? `${this.apiKey.substring(0, 8)}...${this.apiKey.substring(-4)}`
        : null,
      baseURL: this.baseURL,
