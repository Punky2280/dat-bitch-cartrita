...
if (process.env.DB_SKIP === '1' || process.env.NODE_ENV === 'test') {
  ...
  pool.on('connect', () => {
    console.log(
      `✅ Database connected (${baseConfig.host}:${baseConfig.port})`
    );
  });
  pool.on('error', err => {
  ...
  (async () => {
    try {
      ...
      if (
        (process.env.DB_PORT_AUTOFALLBACK === '1' ||
        ...
        try {
          console.warn(
            '[db] Primary port 5432 failed, attempting fallback port 5435'
          );
          ...
          console.error(
            '[db] ❌ Fallback port 5435 also failed:',
            fallbackErr.message
          );
        }
      } else {
        console.error(
          '[db] ❌ Primary database unreachable (continuing):',
          err.message
        );
      }
    }
