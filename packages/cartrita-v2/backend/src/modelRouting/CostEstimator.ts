...
export class CostEstimator extends EventEmitter {
  ...
  private estimateFromCostProfile(
    ...
    const overheadFactor =
      profile.overhead_factor || context.overhead_factor || 1.0;
   ...
   */
  private estimateFromHardwareMetrics(
    context: CostContext
  ): CostEstimationResult {
    ...
    const efficiencyAdjustment =
      (finalCostPer1k - adjustedCostPer1k) * (totalTokens / 1000);
   ...
   */
  private estimateFromFallbackMethod(
    context: CostContext
  ): CostEstimationResult {
  ...
  async estimateBatchCost(
    ...
    const totalTokens = requests.reduce(
      (sum, req) => sum + req.tokens_in + req.tokens_out,
      0
    );
  ...
  async generateOptimizationSuggestions(
    ...
    const avgCost =
      usage.reduce((sum, event) => sum + event.cost_usd, 0) / usage.length;
    const avgTokens =
      usage.reduce((sum, event) => sum + event.total_tokens!, 0) / usage.length;
  ...
  private projectBudgetExhaustion(
    budget: BudgetTracking,
    currentSpend: number
  ): Date {
  ...
  private analyzeTradeOffs(
    currentModel: string,
    alternativeModel: ModelRegistryEntry
  ): string[] {
    const tradeOffs: string[] = [];

    if (
      alternativeModel.parameters_billion &&
