...
export class ModelRegistryService extends EventEmitter {
  ...
  async selectModel(
    ...
    const estimatedCost = await this.estimateBasicCost(
      criteria.task_type,
      estimatedTokens
    );
  ...
  async executeInference(
    ...
    if (options.safety_enabled !== false) {
      ...
      if (!safetyResult.is_safe) {
        throw new Error(
          `Content rejected by safety pipeline: ${safetyResult.explanation}`
        );
      }
    }
    ...
    try {
      ...
      const costResult = await this.costEstimator.estimateCost(
        {
          ...
          measured_tokens_per_second:
            model.cost_profile.estimated_tokens_per_hour / 3600,
          tokens_in: tokensIn,
      ...
      if (options.safety_enabled !== false && inferenceResult.text) {
        ...
        if (!postSafetyResult.is_safe) {
          const redactAction = postSafetyResult.suggested_actions.find(
            a => a.action === 'redact'
          );
   ...
   * Get cost statistics
   */
  async getCostStatistics(timeRangeHours: number = 24): Promise<
    APIResponse<{
  ...
  private ensureInitialized(): void {
    if (!this.isInitialized) {
      throw new Error(
        'Model Registry Service not initialized. Call initialize() first.'
      );
    }
  }
  ...
  private async estimateBasicCost(
    taskType: TaskType,
    tokens: number
  ): Promise<number> {
