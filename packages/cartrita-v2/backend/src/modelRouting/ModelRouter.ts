...
export class ModelRouter extends EventEmitter {
  ...
  async selectModel(
    ...
    try {
      ...
      const strategy =
        this.routingStrategies.get(strategyName) ||
      ...
      if (candidates.length === 0) {
        throw new Error(
          `No models available for task type: ${criteria.task_type}`
        );
      }
      ...
      const result: ModelSelectionResult = {
        ...
        fallback_available:
          strategy.fallback_enabled && rankedModels.length > 1,
  ...
  private async getCandidateModels(
    ...
    return allModels.filter(model => {
      ...
      if (context.safety_requirements?.max_risk_level) {
        ...
        const maxRiskIndex = riskLevels.indexOf(
          context.safety_requirements.max_risk_level
        );
  ...
  private async checkAvailabilityAndBalance(
    ...
    const currentLoad =
      this.loadBalancer.get(primaryChoice.model.model_id) || 0;
  ...
  private async estimateRequestCost(
    ...
    return (
      model.cost_profile.estimated_cost_per_1k_tokens_usd * (avgTokens / 1000)
    );
  }
