...
export class BenchmarkHarness extends EventEmitter {
    ...
    isRunning = false;
    results = [];
    constructor(modelInferenceService, // HuggingFace or other inference service
                ...
                for (const hardwareConfig of config.hardware_configs) {
                    ...
                    for (const contextLength of config.context_lengths) {
                        ...
                        for (const temperature of config.temperature_settings) {
                            if (this.abortController.signal.aborted)
                                break;
                            const result = await this.benchmarkModelConfiguration(modelId, hardwareConfig, contextLength, temperature, config);
     ...
     */
    async benchmarkModelConfiguration(modelId, hardwareConfig, contextLength, temperature, config) {
        ...
        for (let i = 0; i < config.measurement_runs; i++) {
            ...
            const result = await this.measureSingleInference(modelId, prompt, temperature, config.timeout_ms);
     ...
     */
    async measureSingleInference(modelId, prompt, temperature, timeoutMs) {
     ...
     */
    async runWarmup(modelId, prompts, temperature) {
     ...
     */
    generateTestPrompts(contextLength, count) {
        const prompts = [];
     ...
     */
    adjustPromptLength(prompt, targetLength) {
     ...
     */
    async runQualityBenchmarks(modelId, temperature) {
        const results = {};
        ...
        for (const sample of mmluSamples) {
            try {
                ...
                this.logger.warn('Quality benchmark question failed', {
                    modelId,
                    error,
                });
            }
     ...
     */
    calculateAverageResourceUsage(usages) {
        if (usages.length === 0) {
    ...
    generateSummary() {
        ...
        for (const [modelId, results] of modelSummaries) {
            const avgTokensPerSecond = results.reduce((sum, r) => sum + r.metrics.tokens_per_second, 0) /
                results.length;
            const avgLatencyP95 = results.reduce((sum, r) => sum + r.metrics.latency_p95_ms, 0) /
                results.length;
            summary[modelId] = {
                total_runs: results.length,
                avg_tokens_per_second: avgTokensPerSecond,
                avg_latency_p95_ms: avgLatencyP95,
                avg_memory_usage_gb: results.reduce((sum, r) => sum + r.metrics.memory_usage_gb, 0) /
                    results.length,
