...
const MODEL_CATALOG: ModelEntry[] = JSON.parse(
  fs.readFileSync(catalogPath, 'utf-8')
);
const CATALOG_BY_ID: Record<string, ModelEntry> = Object.fromEntries(
...
function recordOutcome(id: string, ok: boolean) {
  ...
  try {
    if (ok)
      (global as any).otelCounters?.hfRoutingSuccess?.add(1, { model: id });
...
async function safetyFilter(
  text: string
): Promise<{ safe: boolean; risk_categories: string[]; confidence: number }> {
        ...
        for (const m of ordered) {
          try {
            ...
            if (
              ...
              usedFallbacks++;
              (global as any).otelCounters?.hfRoutingFallbacks?.add(1, {
                reason: 'low_conf',
                model: m.repo_id,
              });
              continue;
