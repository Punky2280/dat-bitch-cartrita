...
router.get('/metrics/security', flexibleAuth, async (req, res) => {
  try {
    ...
    // Get token usage stats from oauth_tokens (closest equivalent)
    const revealStats = await db.query(`
      SELECT 
        COUNT(*) as total_tokens_created,
        COUNT(*) FILTER (WHERE expires_at IS NOT NULL AND expires_at > NOW()) as tokens_active,
        COUNT(*) FILTER (WHERE expires_at < NOW()) as tokens_expired,
        AVG(EXTRACT(EPOCH FROM (updated_at - created_at))) as avg_usage_time_seconds
      FROM oauth_tokens 
      WHERE created_at >= NOW() - INTERVAL '24 hours'
    `);
    ...
    res.json({
      ...
      data: {
        ...
        total_users_with_preferences: preferenceStats.rows.reduce(
  ...
  try {
    ...
    const apiKeyStats = await db.query(`
      SELECT 
        ...
        COUNT(*) FILTER (WHERE updated_at >= NOW() - INTERVAL '7 days') as recently_used,
...
router.get('/metrics/performance', flexibleAuth, async (req, res) => {
  try {
    ...
    const tableSizes = await db.query(`
      ...
      WHERE relname IN ('users', 'security_events', 'api_keys', 'oauth_tokens')
  ...
  try {
    ...
    const recentApiUsage = await db.query(
      ...
      SELECT 
        'api_usage' as type,
        'key_used' as action,
        jsonb_build_object('key_name', service_name) as metadata,
