...
function ensureMetrics() {
  ...
  try {
    ...
    global.otelCounters.rotationPolicyCrud = OpenTelemetryTracing.createCounter(
      'rotation_policy_crud_total',
      'Rotation policy CRUD ops (labels: action, result)'
    );
    global.otelCounters.rotationEventsTotal =
      OpenTelemetryTracing.createCounter(
        'rotation_events_total',
        'Credential rotation events recorded (labels: result)'
      );
...
router.get('/policies', authenticateToken, async (req, res) => {
  const span = OpenTelemetryTracing.traceOperation(
    'vault.rotation.listPolicies'
  );
  try {
...
router.post('/policies', authenticateToken, async (req, res) => {
  ...
  if (!credential_id)
    return res
      .status(400)
      .json({ success: false, error: 'credential_id_required' });
  try {
    const userId = req.user.id;
    const now = new Date();
    ...
    global.otelCounters.rotationPolicyCrud.add(1, {
      action: 'upsert',
      result: 'success',
    });
...
router.delete('/policies/:id', authenticateToken, async (req, res) => {
  const span = OpenTelemetryTracing.traceOperation(
    'vault.rotation.deletePolicy'
  );
  try {
    const userId = req.user.id;
    const { id } = req.params;
    const result = await pool.query(
      `DELETE FROM credential_rotation_policies WHERE id=$1 AND user_id=$2 RETURNING id`,
      [id, userId]
    );
    if (result.rowCount === 0)
      return res
        .status(404)
        .json({ success: false, error: 'policy_not_found' });
    global.otelCounters.rotationPolicyCrud.add(1, {
      action: 'delete',
      result: 'success',
    });
