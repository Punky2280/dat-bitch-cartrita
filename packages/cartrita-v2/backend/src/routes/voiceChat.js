  ...
  try {
    ...
    if (!global.otelCounters?.voiceCredentials) {
      try {
        ...
        global.otelCounters.voiceCredentials =
          OpenTelemetryTracing.createCounter(
            'voice_credentials_requests_total',
            'Voice credentials issuance requests'
          );
      } catch (_) {}
    }
    try {
      global.otelCounters?.voiceCredentials?.add(1);
    } catch (_) {}

    res.status(200).json({
      provider: 'Deepgram',
      token: status.deepgram.available
        ? 'ephemeral-token-not-implemented'
        : 'mock-deepgram-jwt-token-for-client',
      ephemeral: status.deepgram.available,
      expires_in: 300,
      message: status.deepgram.available
        ? 'Ephemeral voice session granted (scoped).'
        : 'Mock voice session granted (no Deepgram key set).',
...
router.post('/synthesize', authenticateToken, async (req, res) => {
  try {
    ...
    if (!text || !text.trim()) {
      return res
        .status(400)
        .json({ error: 'Provide some text for synthesis.' });
    }
    ...
    if (!global.otelCounters?.voiceTTS) {
      try {
        global.otelCounters = global.otelCounters || {};
        global.otelCounters.voiceTTS = OpenTelemetryTracing.createCounter(
          'voice_tts_requests_total',
          'Voice TTS synthesis requests'
        );
      } catch (_) {}
    }
    try {
      global.otelCounters?.voiceTTS?.add(1, { style: voice_style });
    } catch (_) {}
  ...
  async (req, res) => {
    try {
      ...
      if (!global.otelCounters?.voiceASR) {
        try {
          global.otelCounters = global.otelCounters || {};
          global.otelCounters.voiceASR = OpenTelemetryTracing.createCounter(
            'voice_asr_requests_total',
            'Voice ASR transcription requests'
          );
        } catch (_) {}
      }
      try {
        global.otelCounters?.voiceASR?.add(1);
      } catch (_) {}
      ...
      if (req.file) {
        ...
        const b64 = req.body.audio_base64.replace(
