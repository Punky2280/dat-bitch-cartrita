...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const result = await keyVault.rotate({
        userId: req.user.id,
        serviceName,
        newRawKey: newKey,
        reason,
...
router.get(
  '/keys/:serviceName/metadata',
  authenticateToken,
  requireAdmin,
  async (req, res) => {
    try {
      const meta = await keyVault.getMetadata(
...
router.get(
  '/keys/:serviceName/versions',
  authenticateToken,
  requireAdmin,
  async (req, res) => {
    try {
      const versions = await keyVault.listVersions(
        req.params.serviceName,
        req.user.id
      );
...
router.post(
  '/keys/:serviceName/validate',
  authenticateToken,
  requireAdmin,
  async (req, res) => {
    try {
      const { candidate } = req.body;
      const result = await keyVault.validateCandidate({
        userId: req.user.id,
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      console.log(
        '[KeyVaultRoute] Restore attempt for service',
        req.params.serviceName,
        'user',
        req.user?.id
      );
      const result = await keyVault.restore({
        userId: req.user.id,
        serviceName: req.params.serviceName,
      });
