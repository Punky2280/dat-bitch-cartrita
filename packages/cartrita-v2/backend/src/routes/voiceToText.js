    ...
    try {
      ...
      if (!hasFile && !urlSource) {
        return res.status(400).json({
          success: false,
          error:
            'No audio provided. Use multipart form-data with field "audio" or JSON { url }.',
        });
      }
      ...
      const model =
        body.model || q.model || process.env.DEEPGRAM_MODEL || 'nova-3';
      ...
      const result = await voiceErrorHandler.transcribeAudio(
        audioSource,
