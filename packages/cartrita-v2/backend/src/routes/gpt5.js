...
router.post('/verbosity', async (req, res) => {
  try {
    ...
    const result = await gpt5Service.getVerbosityAdjustedResponse(
      prompt,
      verbosity,
      options
    );
...
router.post('/cfg', async (req, res) => {
  try {
    ...
    const result = await gpt5Service.generateWithCFGConstraints(
      prompt,
      grammar,
      options
    );
...
router.post('/minimal', async (req, res) => {
  try {
    ...
    const result = await gpt5Service.getMinimalReasoningResponse(
      prompt,
      options
    );
