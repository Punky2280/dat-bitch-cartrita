...
knowledgeHub.initialize().then(success => {
  if (success) {
    ...
    console.warn(
      '[Knowledge] ⚠️ Knowledge Hub disabled or optional initialization skipped'
    );
  } else {
    console.error(
      '[Knowledge] ❌ Enhanced Knowledge Hub initialization failed (fatal)'
    );
  }
});
  ...
  return await traceOperation('knowledge.advanced_rag.query', async () => {
    try {
      ...
      if (result.success && result.confidence > 0.8) {
        console.log(
          `[Knowledge] 🧠 High-quality RAG response (confidence: ${result.confidence})`
        );
      }
