...
function ensureVaultMetrics() {
  ...
  try {
    ...
    global.otelCounters.vaultKeysTotal = OpenTelemetryTracing.createCounter(
      'vault_keys_total',
      'Total API keys (labels: provider, status)'
    );
    global.otelCounters.vaultValidationAttempts =
      OpenTelemetryTracing.createCounter(
        'vault_validation_attempts_total',
        'Validation attempts (labels: provider, result)'
      );
    global.otelCounters.vaultRotationTotal = OpenTelemetryTracing.createCounter(
      'vault_rotation_total',
      'Rotation operations (labels: provider, result)'
    );
    global.otelCounters.vaultSecurityEvents =
      OpenTelemetryTracing.createCounter(
        'vault_security_events_total',
        'Security events emitted (labels: type, severity)'
      );
    ...
    if (vaultMetricsInit && global.otelCounters?.vaultKeysTotal) {
      try {
        result.rows.forEach(r => {
          global.otelCounters.vaultKeysTotal.add(1, {
...
router.post('/keys', authenticateToken, async (req, res) => {
  try {
    ...
    if (providerResult.rows.length === 0) {
      return res
        .status(400)
        .json({ success: false, error: 'Invalid or inactive provider' });
    }
    ...
    const result = await pool.query(
      ...
      RETURNING id, key_name, created_at, updated_at, checksum`,
      [
        userId,
        provider_id,
        key_name,
        ...
        rotation_interval_days,
        nextRotationAt,
        rotation_policy ? JSON.stringify(rotation_policy) : null,
        visibility,
        category,
        checksum,
    ...
    await pool.query(
      `INSERT INTO api_security_events (user_id, event_type, description, metadata) VALUES ($1,$2,$3,$4)`,
      [
        userId,
        'key_created',
    ...
    if (vaultMetricsInit && global.otelCounters?.vaultSecurityEvents) {
      global.otelCounters.vaultSecurityEvents.add(1, {
        type: 'key_created',
        severity: 'info',
      });
    }
...
router.post('/keys/:keyId/validate', authenticateToken, async (req, res) => {
  try {
    ...
    const keyResult = await pool.query(
      `SELECT uak.key_data, ap.name as provider_name FROM user_api_keys uak JOIN api_providers ap ON uak.provider_id = ap.id WHERE uak.id = $1 AND uak.user_id = $2`,
      [keyId, userId]
    );
    ...
    await pool.query(
      `INSERT INTO api_security_events (user_id, event_type, description, metadata, severity) VALUES ($1,$2,$3,$4,$5)`,
      [
        userId,
        'key_validated',
        `API key '${keyId}' validated`,
        JSON.stringify({
          key_id: keyId,
          provider: provider_name,
          valid: result.valid,
    ...
    if (vaultMetricsInit && global.otelCounters?.vaultValidationAttempts) {
      global.otelCounters.vaultValidationAttempts.add(1, {
        provider: provider_name,
        result: result.valid ? 'success' : 'fail',
      });
      global.otelCounters.vaultSecurityEvents.add(1, {
        type: 'key_validated',
...
router.post('/keys/:keyId/rotate', authenticateToken, async (req, res) => {
  try {
    ...
    if (vaultMetricsInit && global.otelCounters?.vaultRotationTotal) {
      global.otelCounters.vaultRotationTotal.add(1, {
        provider: provider_id?.toString() || 'unknown',
        result: 'success',
      });
      global.otelCounters.vaultSecurityEvents.add(1, {
        type: 'key_rotated',
