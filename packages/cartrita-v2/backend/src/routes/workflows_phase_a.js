...
router.put('/:id', requireServices, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, description, trigger_type, steps, settings, is_active } =
      req.body;

    await OpenTelemetryTracing.traceOperation(
      'workflow.api.update_workflow',
      {},
      async span => {
        const result = await db.query(
          `
...
router.post('/expressions/evaluate', requireServices, async (req, res) => {
  try {
    ...
    await OpenTelemetryTracing.traceOperation(
      'workflow.api.evaluate_expression',
      {},
      async span => {
...
router.post('/expressions/validate', requireServices, async (req, res) => {
  try {
    ...
    await OpenTelemetryTracing.traceOperation(
      'workflow.api.validate_expression',
      {},
      async span => {
