  ...
  return await OpenTelemetryTracing.traceOperation(
    ...
    async span => {
      try {
        ...
        console.log(
          '[CameraTestingAPI] Creating testing session for user:',
          userId
        );

        const result = await service.createTestingSession(
          userId,
          sessionConfig
        );
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async span => {
        try {
          ...
          console.log(
            '[CameraTestingAPI] Analyzing frame for session:',
            sessionId,
            {
          ...
          const frameMetadata = {
            ...
            client_info: req.body.client_info
              ? JSON.parse(req.body.client_info)
              : {},
          };
          ...
          span.setAttributes({
            ...
            'analysis.is_black_frame': result.success
              ? result.analysis?.isBlackFrame
              : false,
          });
  ...
  return await OpenTelemetryTracing.traceOperation(
    ...
    async span => {
      try {
        ...
        console.log('[CameraTestingAPI] Performing multi-frame analysis:', {
          sessionId,
          frameCount,
        });

        const result = await service.performMultiFrameAnalysis(
          sessionId,
          frameCount
        );
      ...
      async span => {
        try {
          ...
          if (result.success) {
            ...
            res.set(
              'Content-Disposition',
              `inline; filename="resized.${options.format}"`
            );
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async span => {
        try {
          ...
          console.log(
            '[CameraTestingAPI] Generating diagnostic report for session:',
            sessionId
          );
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async span => {
        try {
          ...
          if (results.length >= 2) {
            try {
              const multiResult = await service.performMultiFrameAnalysis(
                sessionId,
                results.length
              );
              ...
              console.warn(
                '[CameraTestingAPI] Multi-frame analysis failed:',
                error.message
              );
            }
          }
          ...
          span.setAttributes({
            ...
            /* The above code is using a ternary operator to check if the
          variable `multiFrameAnalysis` is truthy. If
          `multiFrameAnalysis` is truthy, the expression will evaluate
          to `true`, otherwise it will evaluate to `false`. */
            'batch.multi_frame_analysis': multiFrameAnalysis ? true : true,
          });
