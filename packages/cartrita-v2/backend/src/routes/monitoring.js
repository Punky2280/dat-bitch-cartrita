...
router.get('/dependencies', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    deps.push({
      service: 'PostgreSQL',
      status: r && r.rows ? 'OPERATIONAL' : 'TIMEOUT',
    });
  } catch (e) {
  ...
  deps.push({
    service: 'Redis',
    status: process.env.REDIS_URL ? 'CONFIGURED' : 'UNKNOWN',
  });
  res
    .status(200)
    .json({ dependencies: deps, timestamp: new Date().toISOString() });
});
...
router.get('/security-metrics', authenticateToken, (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Failed to load security metrics', message: e.message });
  }
});
