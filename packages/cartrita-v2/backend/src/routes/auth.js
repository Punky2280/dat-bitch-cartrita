...
router.post('/register', async (req, res) => {
  try {
    ...
    const token = jwt.sign(baseClaims, process.env.JWT_SECRET, {
      expiresIn: '24h',
    });
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      try {
        global.openTelemetryIntegration?.meter?.createCounter &&
    ...
    try {
      if (global.otelCounters?.authFailure)
        global.otelCounters.authFailure.add(1, { stage: 'exception' });
    } catch (_) {}
    res.status(500).json({
...
router.get('/users', async (req, res) => {
  if (
    ...
    process.env.ENABLE_DEV_AUTH_ENDPOINTS !== '1'
  ) {
    return res.status(403).json({ error: 'Forbidden' });
  }
  try {
    const result = await db.query(
      'SELECT id, name, email, created_at FROM users ORDER BY id ASC LIMIT 500'
    );
...
router.post('/validate-users', async (req, res) => {
  ...
  try {
    const result = await db.query(
      'SELECT id, name, email FROM users ORDER BY id ASC LIMIT 200'
    );
...
router.post('/admin/validate-all', requireAdmin, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT id, name, email FROM users ORDER BY id ASC LIMIT 5000'
    );
    const issues = [];
    const summary = [];
...
router.post('/emergency-login', (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: 'Emergency login failed', details: error.message });
  }
});
