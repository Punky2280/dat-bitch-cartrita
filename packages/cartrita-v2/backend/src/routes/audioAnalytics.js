...
const requireAudioService = (req, res, next) => {
  if (!audioAnalyticsService || !audioAnalyticsService.isInitialized) {
    return res.status(503).json({
      ...
      message:
        'The audio analytics service is not initialized or not available',
    });
  }
  next();
};
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `[AudioAnalytics] Starting analysis for ${req.file.originalname}`
      );

      // Perform analysis
      const results = await audioAnalyticsService.analyzeAudio(
        req.file.path,
        options
      );
    ...
    try {
      ...
      if (isNaN(start) || isNaN(end) || start >= end || start < 0) {
        return res.status(400).json({
          ...
          message:
            'startTime and endTime must be valid numbers with startTime < endTime',
        });
      }
      ...
      console.log(
        `[AudioAnalytics] Analyzing segment ${start}-${end}s from ${req.file.originalname}`
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const queuePromise = audioAnalyticsService.queueAnalysis(
        req.file.path,
        options
      );
      ...
      try {
        await queuePromise;
        console.log(
          `[AudioAnalytics] Queued analysis completed for ${req.file.originalname}`
        );
      } catch (error) {
        console.error(
          `[AudioAnalytics] Queued analysis failed for ${req.file.originalname}:`,
          error
        );
      } finally {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const history = await audioAnalyticsService.getAnalysisHistory(
        req.user.id,
        limit
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      const speakers = await audioAnalyticsService.getSpeakerProfiles(
        req.user.id
      );
