    ...
    try {
      ...
      if (global.hfBinaryStore.size >= MAX_IN_MEMORY_ITEMS) {
        return res
          ...
          .json({
            success: false,
            error: 'In-memory store item limit reached',
          });
      }
      ...
      if (projected > MAX_TOTAL_MEMORY_BYTES) {
        ...
        try {
          ...
          return res
            .status(500)
            .json({ success: false, error: 'Failed to persist file to disk' });
        }
      }
      const id = uuidv4();
...
router.get('/list', authenticateToken, async (req, res) => {
  try {
    ...
    const mimeFilter = req.query.mime
      ? String(req.query.mime).toLowerCase()
      : null;
    const expiredMode = ['include', 'only', 'exclude'].includes(
      req.query.expired
    )
      ? req.query.expired
      : 'exclude';
    ...
    for (const [id, entry] of global.hfBinaryStore.entries()) {
      ...
      if (mimeFilter && (entry.type || '').toLowerCase() !== mimeFilter)
        continue;
      all.push({
        token: `hfbin:${id}`,
    ...
    all.sort((a, b) => {
      const aTime =
        global.hfBinaryStore.get(a.token.substring(6))?.uploadedAt || 0;
      const bTime =
        global.hfBinaryStore.get(b.token.substring(6))?.uploadedAt || 0;
      return bTime - aTime;
    });
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Failed to list tokens' });
  }
});
...
router.delete('/revoke/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (!global.hfBinaryStore.has(id))
    ...
    if (entry.userId && String(entry.userId) !== String(userId)) {
      ...
      if (global.otelCounters?.hfTokenMisuse) {
        try {
          global.otelCounters.hfTokenMisuse.add(1, { action: 'revoke' });
        } catch (_) {}
      }
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Failed to revoke token' });
  }
});
