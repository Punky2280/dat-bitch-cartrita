...
router.post('/policies', async (req, res) => {
  try {
    const policy = await RotationSchedulingService.saveRotationPolicy(
      req.user.id,
      req.body
    );
    res.json({
...
router.delete('/policies/:id', async (req, res) => {
  try {
    const result = await RotationSchedulingService.deleteRotationPolicy(
      req.user.id,
...
router.post('/policies/test', async (req, res) => {
  try {
    const result = await RotationSchedulingService.testRotationPolicy(
      req.user.id,
      req.body
    );
    res.json({
...
router.get('/keys/needing-rotation', async (req, res) => {
  try {
    const keys = await RotationSchedulingService.getKeysNeedingRotation(
      req.user.id
    );
    res.json({
      success: true,
...
router.get('/history', async (req, res) => {
  try {
    ...
    const history = await RotationSchedulingService.getRotationHistory(
      req.user.id,
      limit
    );
    res.json({
...
router.get('/analytics', async (req, res) => {
  try {
    const analytics = await RotationSchedulingService.getRotationAnalytics(
      req.user.id
    );
    res.json({
      success: true,
...
router.get('/status', async (req, res) => {
  try {
    ...
    const overdueCount = overdueKeys.filter(
      k => k.rotation_status === 'overdue'
    ).length;
    const warningCount = overdueKeys.filter(
      k => k.rotation_status === 'warning'
    ).length;

    const status =
      overdueCount > 0 ? 'critical' : warningCount > 0 ? 'warning' : 'healthy';
    ...
    res.json({
      ...
      summary: {
        ...
        healthy_keys:
          (parseInt(analytics.summary.total_keys) || 0) -
          overdueCount -
          warningCount,
