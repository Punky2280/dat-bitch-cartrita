...
function recordOutcome(id, ok) {
    ...
    try {
        if (ok)
            global.otelCounters?.hfRoutingSuccess?.add(1, { model: id });
        else
            global.otelCounters?.hfRoutingErrors?.add(1, { model: id });
    }
    catch (_) { }
}
...
function qualityPrior(m) {
    ...
    if (s.includes('70B') || s.includes('72B'))
        return 0.95;
    if (['33B', '34B', '35B'].some(x => s.includes(x)))
        return 0.85;
    if (['20B', '22B', '32B'].some(x => s.includes(x)))
        return 0.8;
    if (['14B', '15B', '13B', '12B', '10.7B'].some(x => s.includes(x)))
        return 0.75;
    if (['9B', '8B', '7B', '6B'].some(x => s.includes(x)))
        return 0.65;
    if (['3.3B', '3B', '2.7B', '2B', '1.6B', '1.3B', '1.1B'].some(x => s.includes(x)))
        return 0.55;
    if (['small', 'nano', 'MiniLM', '124M'].some(x => s.includes(x)))
        return 0.5;
    return 0.55;
}
function costScore(m) {
    const s = m.approx_params;
    if (['70B', '72B', '33B', '34B', '35B'].some(x => s.includes(x)))
        return 0.3;
    if (['14B', '15B', '13B', '12B', '10.7B'].some(x => s.includes(x)))
        return 0.55;
    if (['9B', '8B', '7B', '6B'].some(x => s.includes(x)))
        return 0.7;
    if (['3.3B', '3B', '2.7B', '2B', '1.6B', '1.3B', '1.1B', 'small', 'nano'].some(x => s.includes(x)))
        return 0.85;
    return 0.6;
}
...
async function probe(repo, category) {
    ...
    if (['vision_language', 'audio'].includes(category))
        return true;
...
async function rerank(model, query, docs) {
    ...
    const result = data
        .map((s, i) => {
        const score = typeof s === 'object' && s.score !== undefined
            ? s.score
            : typeof s === 'number'
                ? s
                : 0;
...
export class HuggingFaceRouterService {
    async route(prompt, opts = {}) {
        ...
        return OpenTelemetryTracing.traceOperation('hf.route_inference', { attributes: spanAttrs }, async () => {
            ...
            if (opts.budget_tier) {
                candidates = candidates.filter(m => m.cost_tier === opts.budget_tier ||
            ...
            if (opts.context_length_needed) {
                candidates = candidates.filter(m => (m.context_length || 4096) >= opts.context_length_needed);
            }
            // Multilingual filtering
            if (opts.multilingual) {
                const multilingualCandidates = candidates.filter(m => m.category === 'multilingual' ||
            ...
            for (const m of ordered) {
                try {
                    ...
                    const conf = typeof output === 'string'
                    ...
                    if (opts.max_cost_per_1k_tokens &&
