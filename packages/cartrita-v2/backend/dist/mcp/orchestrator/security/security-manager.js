...
export class MCPSecurityManager {
  logger;
  jwtConfig;
  tracer = trace.getTracer('mcp-security');
  ...
  async authorize(permission) {
    return async (request, reply) => {
      ...
      try {
        ...
        span.recordException(error);
        this.logger.warn('Authorization failed', error, {
  ...
  getStats() {
    return {
      ...
      sessionDetails: Array.from(this.activeSessions.entries()).map(
        ([id, session]) => ({
          id: id.substring(0, 8) + '...',
  ...
  async createSecurityContext(user, request) {
