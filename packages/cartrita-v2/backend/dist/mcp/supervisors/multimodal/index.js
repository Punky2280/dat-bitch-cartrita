...
export class MultiModalSupervisor extends EventEmitter {
  ...
  sensorFusion;
  mediaProcessor;
  tracer = trace.getTracer('multimodal-supervisor');
  metrics = getMetrics();
  isInitialized = false;
  activeTasks = new Map();
  ...
  constructor(config = {}) {
    ...
    this.config = {
      ...
      openai: {
        ...
        organization:
          config.openai?.organization ?? process.env.OPENAI_ORGANIZATION,
      },
      deepgram: {
    ...
    this.logger.info('Multi-Modal Supervisor created', {
  ...
  async initialize() {
    ...
    try {
      ...
      this.transport.registerHandler(
      ...
      this.logger.error('Failed to initialize Multi-Modal Supervisor', error);
      throw error;
    } finally {
      span.end();
    }
  ...
  async handleTaskRequest(message) {
    ...
    try {
      ...
      if (!validation.success) {
        throw new Error(
          `Invalid task request: ${validation.errors.join(', ')}`
        );
      }
      ...
      const errorResponse = {
        ...
        metrics: {
          processingTimeMs:
            Date.now() -
            (this.activeTasks.get(taskRequest.taskId)?.startTime || Date.now()),
          queueTimeMs: 0,
          retryCount: 0,
          costUsd: 0,
          tokensUsed: 0,
      ...
      span.recordException(error);
      this.logger.error('Multi-modal task execution failed', error, {
        taskId: taskRequest.taskId,
      });
    } finally {
   ...
   */
  async executeTask(request, mcpContext) {
    ...
    try {
      ...
      if (!agent) {
        throw new Error(
          `No available agent for task type: ${request.taskType}`
        );
      }
      ...
      if (
        ...
        this.requiresSensorFusion(request.taskType)
      ) {
   ...
   */
  async executeWithSensorFusion(request, agent, mcpContext) {
    ...
    try {
      ...
      span.recordException(error);
      throw new Error(`Sensor fusion failed: ${error.message}`);
    } finally {
      span.end();
    }
  }
  ...
  async handleStreamData(message) {
    ...
    if (
      session.contentType?.includes('audio') &&
      session.dataChunks.length % 10 === 0
    ) {
   ...
   */
  async postprocessResult(result, taskType) {
   ...
   */
  async processStreamingAudio(session) {
    try {
      ...
      this.logger.error('Real-time audio processing failed', error, {
        streamId: session.streamId,
      });
    }
  }
   ...
   */
  requiresSensorFusion(taskType) {
    return (
      taskType === TaskTypes['multimodal.fuse'] ||
   ...
   */
  requiresMediaProcessing(taskType) {
    return (
   ...
   */
  async sendTaskResponse(originalMessage, response) {
