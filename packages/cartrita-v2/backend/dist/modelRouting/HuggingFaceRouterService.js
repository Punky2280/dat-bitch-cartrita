...
const OpenTelemetryTracing =
  require('../system/OpenTelemetryTracing.js').default;
// Load enhanced catalog with cost tiers and quality scores
// Try enhanced catalog first, fallback to original
let catalogPath = path.join(
  process.cwd(),
  'packages/backend/src/modelRouting/hfModelCatalogEnhanced.json'
);
if (!fs.existsSync(catalogPath)) {
  catalogPath = path.join(
    process.cwd(),
    'packages/backend/src/modelRouting/hfModelCatalog.json'
  );
}
if (!fs.existsSync(catalogPath)) {
  ...
  const alt = path.join(
    process.cwd(),
    'src/modelRouting/hfModelCatalogEnhanced.json'
  );
  if (fs.existsSync(alt)) {
    catalogPath = alt;
  } else {
    const altOrig = path.join(
      process.cwd(),
      'src/modelRouting/hfModelCatalog.json'
    );
...
const CATALOG_BY_ID = Object.fromEntries(
  MODEL_CATALOG.map(m => [m.repo_id, m])
);
...
function qualityPrior(m) {
  ...
  if (['14B', '15B', '13B', '12B', '10.7B'].some(x => s.includes(x)))
    return 0.75;
  if (['9B', '8B', '7B', '6B'].some(x => s.includes(x))) return 0.65;
  if (
    ['3.3B', '3B', '2.7B', '2B', '1.6B', '1.3B', '1.1B'].some(x =>
      s.includes(x)
    )
  )
    return 0.55;
...
function costScore(m) {
  ...
  if (
    ['3.3B', '3B', '2.7B', '2B', '1.6B', '1.3B', '1.1B', 'small', 'nano'].some(
      x => s.includes(x)
    )
  )
    return 0.85;
...
function shortlist(task, max = 6) {
  let candidates = MODEL_CATALOG.filter(m => {
    if (task === 'general')
      return ['general', 'lightweight'].includes(m.category);
    if (task === 'embedding')
      return ['embedding', 'rerank'].includes(m.category);
    if (task === 'rerank') return m.category === 'rerank';
    return m.category === task;
  });
  if (candidates.length === 0) {
    candidates = MODEL_CATALOG.filter(m =>
      ['general', 'lightweight'].includes(m.category)
    );
  }
  return candidates.sort((a, b) => composite(b) - composite(a)).slice(0, max);
}
async function probe(repo, category) {
...
function confidenceHeuristic(output, taskType) {
  if (!output) return 0;
  ...
  const refusalPatterns =
    /sorry|cannot|unable|don't know|not sure|unclear|uncertain/i;
  ...
  if (taskType === 'code') {
    ...
    const hasMathContent =
      /\d+|[+\-*/=<>]|\b(equation|formula|calculate)\b/i.test(output);
  ...
  const baseConfidence =
    0.4 * lengthScore + 0.3 * diversity + 0.3 * (1 - refusalPenalty);
...
async function generate(model, prompt, params = {}) {
  const body = {
    inputs: prompt,
    parameters: {
  ...
  try {
    ...
    if (Array.isArray(res.data) && res.data[0]?.generated_text)
      return String(res.data[0].generated_text);
...
async function rerank(model, query, docs) {
  ...
  const result = data
    .map((s, i) => {
      const score =
        typeof s === 'object' && s.score !== undefined
          ? s.score
          : typeof s === 'number'
            ? s
            : 0;
...
async function translate(model, text, src, tgt) {
  const meta = CATALOG_BY_ID[model];
  if (
    meta?.category === 'multilingual' &&
    meta.primary_tasks.includes('translation')
  ) {
    const res = await SESSION.post(`/models/${model}`, { inputs: text });
    recordOutcome(model, res.status === 200);
    const d = res.data;
    if (Array.isArray(d) && d[0]?.translation_text)
      return d[0].translation_text;
...
export class HuggingFaceRouterService {
  async route(prompt, opts = {}) {
    const spanAttrs = { 'hf.route.prompt_chars': prompt.length };
      ...
      async () => {
        ...
        if (opts.require_safety_filter) {
          ...
          if (!safetyResult.safe) {
            throw new Error(
              `Safety filter blocked request: ${safetyResult.risk_categories.join(', ')}`
            );
          }
        }
        ...
        if (opts.budget_tier) {
          candidates = candidates.filter(
            m =>
              ...
              (opts.budget_tier === 'premium' &&
                ['standard', 'economy'].includes(m.cost_tier || 'standard'))
          );
        }
        // Context length filtering
        if (opts.context_length_needed) {
          candidates = candidates.filter(
            m => (m.context_length || 4096) >= opts.context_length_needed
          );
        }
        // Multilingual filtering
        if (opts.multilingual) {
          const multilingualCandidates = candidates.filter(
            m =>
              m.category === 'multilingual' ||
              m.primary_tasks.includes('multilingual') ||
              m.repo_id.includes('multilingual') ||
              ['Qwen', 'aya', 'nllb', 'm2m'].some(prefix =>
                m.repo_id.includes(prefix)
              )
          );
        ...
        for (const m of ordered) {
          try {
            ...
            if (task === 'embedding') {
              ...
              output = await generate(m.repo_id, prompt, {
                temperature: opts.temperature,
                max_new_tokens: opts.max_new_tokens,
              });
              actualOutputTokens =
                typeof output === 'string' ? Math.ceil(output.length / 4) : 0;
            } else if (
              task === 'multilingual' &&
            ...
            const conf =
              typeof output === 'string'
                ? confidenceHeuristic(output, task)
                : 0.9;
            ...
            if (
              opts.max_cost_per_1k_tokens &&
              estimatedCost > opts.max_cost_per_1k_tokens
            ) {
              console.warn(
                `Model ${m.repo_id} estimated cost ${estimatedCost} exceeds limit ${opts.max_cost_per_1k_tokens}`
              );
              if (!opts.enable_fallback) {
                throw new Error(
                  `Cost limit exceeded: ${estimatedCost} > ${opts.max_cost_per_1k_tokens}`
                );
              }
              usedFallbacks++;
              continue;
            ...
            if (
              ...
              console.log(
                `Model ${m.repo_id} confidence ${conf} below threshold ${minConfidence}, trying fallback`
              );
              usedFallbacks++;
              global.otelCounters?.hfRoutingFallbacks?.add(1, {
                reason: 'low_conf',
                model: m.repo_id,
              });
              continue;
            ...
            if (opts.require_safety_filter && typeof output === 'string') {
              ...
              if (!outputSafetyResult.safe) {
                console.warn(
                  `Output from ${m.repo_id} failed safety filter: ${outputSafetyResult.risk_categories.join(', ')}`
                );
                ...
                throw new Error(
                  `Output safety filter failed: ${outputSafetyResult.risk_categories.join(', ')}`
                );
              }
            }
        ...
        throw new Error(
          `All candidates failed after ${usedFallbacks} attempts. Last error: ${lastErr?.message || 'Unknown'}`
        );
      }
    );
  }
  ...
  shortlist(prompt, taskHint, max = 6) {
