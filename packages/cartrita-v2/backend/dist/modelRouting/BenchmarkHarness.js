...
export class BenchmarkHarness extends EventEmitter {
  ...
  results = [];
  constructor(
    modelInferenceService, // HuggingFace or other inference service
   ...
   */
  async benchmarkModelConfiguration(
    modelId,
    hardwareConfig,
    contextLength,
    temperature,
    ...
    const testPrompts = this.generateTestPrompts(
      contextLength,
    ...
    await this.runWarmup(
      modelId,
      testPrompts.slice(0, config.warmup_runs),
      temperature
    );
    // Measure performance
    const latencies = [];
  ...
  adjustPromptLength(prompt, targetLength) {
    ...
    if (estimatedTokens < targetLength) {
      ...
      const padding = ' '.repeat(
        Math.max(0, (targetLength - estimatedTokens) * 4)
      );
  ...
  calculateAverageResourceUsage(usages) {
    ...
    const sum = usages.reduce(
      (acc, usage) => ({
        ...
        gpuUtilizationPercent:
          (acc.gpuUtilizationPercent || 0) + (usage.gpuUtilizationPercent || 0),
        gpuMemoryUsageMB:
          (acc.gpuMemoryUsageMB || 0) + (usage.gpuMemoryUsageMB || 0),
      }),
      {
        cpuPercent: 0,
        memoryUsageMB: 0,
    ...
    return {
      ...
      gpuUtilizationPercent:
        sum.gpuUtilizationPercent / usages.length || undefined,
  ...
  generateSummary() {
    ...
    for (const [modelId, results] of modelSummaries) {
      const avgTokensPerSecond =
        results.reduce((sum, r) => sum + r.metrics.tokens_per_second, 0) /
        results.length;
      const avgLatencyP95 =
        results.reduce((sum, r) => sum + r.metrics.latency_p95_ms, 0) /
        results.length;
      summary[modelId] = {
        ...
        avg_memory_usage_gb:
          results.reduce((sum, r) => sum + r.metrics.memory_usage_gb, 0) /
          results.length,
