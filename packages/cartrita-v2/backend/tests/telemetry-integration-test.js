...
async function runTelemetryIntegrationTest() {
  ...
  try {
    ...
    console.log(
      `   ✅ MCP Communication Tracing: ${mcpResult.mcp_trace === 'success'}`
    );
    ...
    const agentStatusResult = await telemetryAgent.processMessage(
      'Check telemetry status'
    );

    console.log(
      `   ✅ Enhanced Service Status: ${enhancedStatus.enhanced.initialized ? 'Healthy' : 'Degraded'}`
    );
    console.log(
      `   ✅ Base Service Status: ${baseStatus.initialized ? 'Healthy' : 'Degraded'}`
    );
    console.log(
      `   ✅ Agent Status Check: ${agentStatusResult.success !== false ? 'Healthy' : 'Degraded'}`
    );
    ...
    const upstreamFeatures = {
      ...
      'Resource Attributes':
        enhancedStatus.enhanced.upstream_integration.opentelemetry_js ===
        'active',
    };
