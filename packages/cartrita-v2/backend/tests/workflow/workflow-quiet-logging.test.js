...
describe('Workflow Engine Quiet Logging', () => {
  ...
  beforeAll(() => {
    if (!shouldQuietLogs()) {
      ...
      console.warn(
        'Environment is not quiet; skipping quiet logging assertions'
      );
      return;
    }
  ...
  it('does not emit handler not implemented warnings in quiet mode', () => {
    ...
    const hasHandlerWarnings = warnings.some(
      w => w.includes('Handler') && w.includes('not implemented')
    );
  ...
  it('does not emit scheduler DB errors in quiet mode', () => {
    ...
    const hasSchedulerErrors = errors.some(e =>
      e.includes('Error processing scheduled workflows')
    );
