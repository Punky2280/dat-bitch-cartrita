...
class HFJobsTestSuite {
    ...
    try {
      const jobResult = await this.hfJobs.runJob({
        command: [
          'python',
          '-c',
          'import sys; print(f"Hello from HF Jobs! Python {sys.version}")',
        ],
        image: 'python:3.12',
        flavor: 'cpu-basic',
        jobType: 'test_basic',
  ...
  async testConfiguration() {
    ...
    console.log(
      `  📊 Min file size for cloud: ${(config.minFileSizeForCloud / 1024 / 1024).toFixed(1)}MB`
    );
    ...
    const shouldUseCloud = this.config.shouldUseCloud(mockAudioInfo, 'full', {
      userId: testUserId,
    });
    console.log(
      `  🤖 Decision for 15MB full analysis: ${shouldUseCloud ? 'cloud' : 'local'}`
    );
  ...
  async testHealthCheck() {
    ...
    try {
      ...
      console.log(
        '  ⚠️  Health check failed - this may be expected without Pro subscription'
      );
  ...
  printSummary() {
    ...
    if (failed === 0) {
      ...
      console.log(
        '🚀 You can now run audio analysis jobs on HF infrastructure.'
      );
    } else {
