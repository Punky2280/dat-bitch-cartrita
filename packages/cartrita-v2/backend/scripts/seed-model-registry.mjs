...
const INITIAL_MODELS = [
  ...
  // Safety/Moderation
  {
    ...
    status: 'active',
    risk_notes:
      'Specialized for content moderation - not for general text generation.',
  },
];
...
async function seedModelRegistry() {
  try {
    console.log(
      '[Model Registry Seeder] 🌱 Starting model registry seeding...'
    );
    ...
    for (const model of INITIAL_MODELS) {
      try {
        console.log(
          `[Model Registry Seeder] 📝 Seeding model: ${model.model_id}`
        );
        ...
        if (result.success) {
          successCount++;
          console.log(
            `[Model Registry Seeder] ✅ Successfully seeded: ${model.model_id}`
          );
        } else {
          failureCount++;
          console.error(
            `[Model Registry Seeder] ❌ Failed to seed ${model.model_id}:`,
        ...
        failureCount++;
        console.error(
          `[Model Registry Seeder] ❌ Error seeding ${model.model_id}:`,
          error.message
        );
      }
    }
    ...
    try {
      ...
      console.warn(
        '[Model Registry Seeder] ⚠️ Failed to update efficiency scores:',
        error.message
      );
    }
    ...
    try {
      ...
      for (const criteria of testSelections) {
        ...
        console.log(
          `[Model Registry Seeder] 🎯 ${criteria.task_type}: ${selection.selected} (${selection.reason})`
        );
      }
    } catch (testError) {
      console.warn(
        '[Model Registry Seeder] ⚠️ Model selection test failed:',
        testError.message
      );
    }
    ...
    try {
      ...
      console.warn(
        '[Model Registry Seeder] ⚠️ Database disconnect error:',
        dbError.message
      );
    }
  }
