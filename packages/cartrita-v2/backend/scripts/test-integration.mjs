...
async function testIntegration() {
  ...
  try {
    ...
    console.log(
      `   - Capabilities: ${statusBefore.capabilities.length} listed`
    );
    ...
    const { default: EnhancedWorkflowEngine } = await import(
      '../src/services/EnhancedWorkflowEngine.js'
    );
    ...
    console.log(
      `  ✅ Workflow stats accessible: ${Object.keys(stats).length} metrics`
    );
    ...
    try {
      ...
      const { default: EnhancedKnowledgeHub } = await import(
        '../src/services/EnhancedKnowledgeHub.js'
      );
      ...
      console.log(
        `  ⚠️ Knowledge Hub construction failed (expected without API key): ${error.message}`
      );
    }
    ...
    try {
      ...
      console.error(
        '  ❌ Model registry structure test failed:',
        error.message
      );
    }
    ...
    const testMethods = [
      { name: 'executeWorkflow', args: ['test-wf', 1, {}, {}] },
      {
        ...
        args: [1, { originalname: 'test.txt' }, 'path', {}],
      },
    ...
    for (const { name, args } of testMethods) {
      try {
        ...
        if (
          ...
          console.log(
            `  ⚠️ Method ${name} failed with unexpected error: ${error.message}`
          );
        }
      }
    }
