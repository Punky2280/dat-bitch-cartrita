...
async function customerServiceExample() {
    ...
    ...trainingDataTemplates.customerService,
    {
      messages: [
        { role: 'user', content: "My account is locked and I can't log in." },
        {
          role: 'assistant',
          content:
            "I understand how frustrating that must be! I can help you unlock your account right away. For security purposes, I'll need to verify your identity first. Could you please provide me with the email address associated with your account?",
        },
      ],
    },
    {
      messages: [
        { role: 'user', content: 'I want to cancel my subscription.' },
        {
          role: 'assistant',
          content:
            "I'm sorry to hear you're considering canceling your subscription. I'd love to help address any concerns you might have first. Could you tell me what's prompting this decision? There might be a solution that works better for you.",
        },
      ],
    },
    {
      messages: [
        { role: 'user', content: 'The app keeps crashing on my phone.' },
        {
          role: 'assistant',
          content:
            "That sounds really frustrating! App crashes can usually be resolved quickly. Let's troubleshoot this together. First, could you tell me what type of phone you're using and which version of our app you have installed?",
        },
      ],
    },
  ];
  ...
  try {
    ...
    console.log('');
    console.log(
      '⚠️  Note: Actual fine-tuning job creation is commented out to avoid charges.'
    );
    console.log(
      '   Uncomment the execution code below to run the full workflow.'
    );
...
async function monitoringExample() {
  ...
  try {
    ...
    if (error.message.includes('API key')) {
      console.log(
        '   Make sure OPENAI_FINETUNING_API_KEY is set in your .env file'
      );
    }
  }
}
...
async function main() {
  ...
  console.log(
    `   OPENAI_FINETUNING_API_KEY: ${process.env.OPENAI_FINETUNING_API_KEY ? '✅ Set' : '❌ Missing'}`
  );
  console.log('');

  if (!process.env.OPENAI_FINETUNING_API_KEY) {
    console.log(
      '⚠️  OPENAI_FINETUNING_API_KEY is required for fine-tuning operations.'
    );
  ...
  console.log(
    '1. Uncomment execution code in customerServiceExample() to run actual fine-tuning'
  );
  ...
  console.log(
    '4. Check /api/fine-tuning endpoints for web interface integration'
  );
}
