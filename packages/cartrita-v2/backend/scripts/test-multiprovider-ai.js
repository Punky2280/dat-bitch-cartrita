...
const TEST_CASES = [
  {
    ...
    expected: result =>
      result.payload &&
      result.payload[0] &&
      result.payload[0][0].label === 'POSITIVE',
  },
  {
    ...
    payload: {
      task: 'zero-shot',
      input:
        'This is a great educational resource for learning machine learning',
      params: {
        candidate_labels: [
          'education',
          'technology',
    ...
    expected: result =>
      result.payload &&
      result.payload.labels &&
      result.payload.labels[0] === 'education',
  },
  {
  ...
  },
  {
    ...
    payload: {
      ...
      input: {
        ...
        context:
          'France is a country in Western Europe. Its capital and largest city is Paris, which is located in the north-central part of the country.',
      },
    },
    expected: result =>
      result.payload &&
      result.payload.answer &&
      result.payload.answer.toLowerCase().includes('paris'),
  },
  {
    name: 'Task Routing',
...
async function runBasicEndpointTests() {
  ...
  for (const endpoint of ENDPOINTS) {
    ...
    if (result.success) {
      log(
        colors.green,
        `✅ ${endpoint.method} ${endpoint.path} - Status: ${result.status}`
      );
    } else {
      log(
        colors.red,
        `❌ ${endpoint.method} ${endpoint.path} - Status: ${result.status}, Error: ${result.error}`
      );
    }
  }
}
    ...
    try {
      ...
      if (result.success && result.data.success) {
        ...
        if (isValid) {
          ...
          // Log key results
          if (
            testCase.name === 'Text Classification' &&
          ...
          if (
            testCase.name === 'Zero-Shot Classification' &&
            result.data.payload.scores
          ) {
          ...
          if (
            testCase.name === 'Question Answering' &&
            ...
            log(
              colors.yellow,
              `   📊 Answer: "${answer}" (${(score * 100).toFixed(1)}%)`
            );
          }
        } else {
        ...
        log(
          colors.red,
          `❌ ${testCase.name} - API Error: ${result.data?.error || result.error}`
        );
      }
    } catch (error) {
...
async function generateSummaryReport() {
  ...
  if (healthResult.success && healthResult.data.success) {
    ...
    log(
      colors.blue,
      `📈 Success Rate: ${(stats.metrics.successRate * 100).toFixed(1)}%`
    );
    log(
      colors.blue,
      `⏱️  Avg Latency: ${stats.metrics.avgLatency.toFixed(2)}ms`
    );
