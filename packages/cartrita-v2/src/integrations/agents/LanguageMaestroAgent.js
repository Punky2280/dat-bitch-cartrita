...
export default class LanguageMaestroAgent {
  constructor() {
    ...
    this.personality = 'Eloquent linguist with deep understanding of human language and communication';
  ...
  async initialize() {
    try {
      ...
      console.log('[LanguageMaestro] 📚 Language intelligence agent initialized');
      return true;
    } catch (error) {
      console.error('[LanguageMaestro] ❌ Initialization failed:', error.message);
      return false;
    }
  }
  ...
  async summarizeText(text, options = {}) {
    ...
    return {
      ...
      compressionRatio: text.length / (result[0]?.summary_text?.length || result.summary_text?.length || 1),
      timestamp: new Date().toISOString()
    };
  }
  ...
  async analyzeText(text, analysisType = 'comprehensive', options = {}) {
    ...
    try {
      switch (analysisType) {
        case 'sentiment':
          results.results.sentiment = await this.classifyText(text, null, options);
          break;
        ...
        case 'comprehensive':
          ...
          analyses.push(this.classifyText(text, null, options).catch(() => null));
          
          // Entity extraction for longer texts
          if (text.length > 50) {
            analyses.push(this.extractEntities(text, options).catch(() => null));
          }
          ...
          const [sentiment, entities, summary] = await Promise.allSettled(analyses);
          
          results.results.sentiment = sentiment.status === 'fulfilled' ? sentiment.value : null;
          results.results.entities = entities.status === 'fulfilled' ? entities.value : null;
          results.results.summary = summary.status === 'fulfilled' ? summary.value : null;
          break;
