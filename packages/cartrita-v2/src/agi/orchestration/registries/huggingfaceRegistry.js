...
export async function registerHuggingFaceTools(registry) {
  ...
  try {
    ...
    registry.registerTool({
      name: 'hf_text_classification', description:'Classify text with HF route', category:'huggingface', schema: z.object({ text: z.string()}), func: async ({ text }) => orchestrator.classifyText?.(text) || { error:'not implemented'}
    });
  } catch (e) {
    registry.registerTool({ name:'hf_status', description:'HF init error', category:'huggingface', schema: z.object({}), func: async ()=>({ integrated:false, error:e.message }) });
  }
}
