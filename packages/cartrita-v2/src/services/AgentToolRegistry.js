...
import { pool as db } from '../database/connection.js';
...
Format as structured JSON with clear categories.`;
    ...
    this.registerTool({
      ...
      description: 'Take a screenshot of the current computer screen for analysis',
    ...
    this.registerTool({
      ...
      schema: z.object({
        ...
        description: z.string().optional().describe('Description of what to click')
      }),
      func: async ({ x, y, button = 'left', description = 'screen click' }) => {
        try {
          console.log(`[ComputerTools] 🖱️ Clicking at (${x}, ${y}) with ${button} button: ${description}`);
    ...
    this.registerTool({
      ...
      schema: z.object({
        ...
        description: z.string().optional().describe('Description of what is being typed')
      }),
    ...
    this.registerTool({
      ...
      schema: z.object({
        ...
        scroll_x: z.number().optional().default(0).describe('Horizontal scroll amount'),
        scroll_y: z.number().describe('Vertical scroll amount (positive = down, negative = up)'),
        description: z.string().optional().describe('Description of scroll action')
      }),
      func: async ({ x, y, scroll_x = 0, scroll_y, description = 'scroll action' }) => {
        try {
          console.log(`[ComputerTools] 📜 Scrolling at (${x}, ${y}) by (${scroll_x}, ${scroll_y}): ${description}`);
    ...
    this.registerTool({
      ...
      category: 'computer_control',
      schema: z.object({
        keys: z.array(z.string()).describe('Array of keys to press (e.g., ["ctrl", "c"] for copy)'),
        description: z.string().optional().describe('Description of key action')
      }),
      func: async ({ keys, description = 'key press' }) => {
        try {
          console.log(`[ComputerTools] ⌨️ Pressing keys: ${keys.join('+')} - ${description}`);
    ...
    this.registerTool({
      ...
      description: 'Execute a complete computer use task with supervision and safety checks',
      category: 'computer_control',
      schema: z.object({
        task: z.string().describe('Description of the computer task to perform'),
        max_iterations: z.number().optional().default(10).describe('Maximum number of action iterations'),
        justification: z.string().describe('Justification for why this computer access is needed'),
        safety_override: z.boolean().optional().default(false).describe('Override safety checks (admin only)')
      }),
      func: async ({ task, max_iterations = 10, justification, safety_override = false }) => {
        try {
          console.log(`[ComputerTools] 🚀 Executing supervised computer task: ${task}`);
    ...
    this.registerTool({
      ...
      category: 'computer_control',
      schema: z.object({
        action_description: z.string().describe('Description of the action to check'),
        context: z.string().optional().describe('Additional context for the safety check')
      }),
      func: async ({ action_description, context = '' }) => {
        try {
          console.log(`[ComputerTools] ⚠️ Safety check for: ${action_description}`);
          
          // Simple safety classification
          const dangerousPatterns = [
            /delete/i, /remove/i, /format/i, /sudo/i, /admin/i, 
            /password/i, /credit.*card/i, /banking/i, /install.*software/i
          ];
          
          const isDangerous = dangerousPatterns.some(pattern => 
            pattern.test(action_description) || pattern.test(context)
          );
          ...
          const result = {
            ...
            safety_concerns: isDangerous ? ['Potentially dangerous operation detected'] : [],
    ...
    this.registerTool({
      ...
      schema: z.object({
        ...
        status: z.enum(['started', 'completed', 'failed', 'denied']).describe('Transaction status'),
        metadata: z.object({}).optional().describe('Additional metadata for the transaction')
      }),
      func: async ({ transaction_id, action, status, metadata = {} }) => {
        try {
          console.log(`[ComputerTools] 📝 Logging transaction: ${transaction_id} - ${action} (${status})`);
