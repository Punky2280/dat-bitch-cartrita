...
export default class JournalService {
  ...
  async initialize() {
    try {
      console.log('[JournalService] 🚀 Initializing journal management service...');
      
      // Test database connection
      await this.dbPool.query('SELECT 1');
      
      console.log('[JournalService] ✅ Journal service initialized successfully');
      return true;
    } catch (error) {
  ...
  async getEntries(userId, filters = {}) {
    return await OpenTelemetryTracing.traceOperation(
      ...
      async (span) => {
        try {
          const { limit = 20, offset = 0, startDate, endDate, tags, mood } = filters;
    ...
    return await OpenTelemetryTracing.traceOperation(
      'lifeos.journal.create',
      {
        attributes: {
          ...
          'entry.word_count': entryData.content ? entryData.content.split(' ').length : 0
        }
      },
      async (span) => {
        try {
          ...
          const result = await this.dbPool.query(query, [
            userId, title, content, mood_score, emotions, tags,
            weather, location, is_private, word_count
          ]);
          ...
          console.log(`[JournalService] ✅ Created entry: ${newEntry.id} for user ${userId}`);
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async (span) => {
        try {
          ...
          const result = await this.dbPool.query(query, [
            title, content, mood_score, emotions, tags,
            weather, location, is_private, word_count, entryId, userId
          ]);
      ...
      for (const match of matches) {
        ...
        if (taskText.length > 10 && taskText.length < 100) {
          tasks.push({
            ...
            priority: this.determinePriority(taskText, mood_score, sentiment_score),
  ...
  determinePriority(taskText, moodScore, sentimentScore) {
    ...
    const importantWords = ['important', 'crucial', 'vital', 'key', 'significant'];
    
    const hasUrgent = urgentWords.some(word => taskText.toLowerCase().includes(word));
    const hasImportant = importantWords.some(word => taskText.toLowerCase().includes(word));
    
    if (hasUrgent || (moodScore && moodScore <= 3)) return 'high';
    if (hasImportant || (sentimentScore && sentimentScore < -0.1)) return 'medium';
    return 'low';
  }
  ...
  scheduleEnrichment(entryId) {
    ...
    console.log(`[JournalService] 📋 Scheduled enrichment for entry: ${entryId}`);
  }
