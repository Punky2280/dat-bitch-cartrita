...
class SecurityAuditLogger {
  ...
  async ensureLogDirectory() {
    try {
      ...
      console.error('[SecurityAuditLogger] Failed to create log directory:', error);
    }
  }
  ...
  async logSecurityEvent(eventType, details = {}) {
    ...
    try {
      ...
      console.log(`[SecurityAuditLogger] ${eventType} logged with severity: ${logEntry.severity}`);
      return logEntry;
    } catch (error) {
      console.error('[SecurityAuditLogger] Failed to log security event:', error);
      throw error;
    }
  }
  ...
  async runSecurityChecks(options = {}) {
    ...
    if (vulnerabilities.length > 0) {
      recommendations.push('Address identified vulnerabilities in order of severity');
    }
  ...
  checkEnvironmentSecurity() {
    ...
    const sensitiveEnvVars = ['API_KEY', 'SECRET', 'PASSWORD', 'TOKEN', 'PRIVATE'];
    const envKeys = Object.keys(process.env);
    
    sensitiveEnvVars.forEach(sensitive => {
      const matches = envKeys.filter(key => key.toUpperCase().includes(sensitive));
      matches.forEach(key => {
  ...
  async checkFilePermissions() {
    ...
    try {
      ...
      console.error('[SecurityAuditLogger] File permission check failed:', error);
    }
  ...
  determineSeverity(eventType, details) {
    const highSeverityEvents = ['unauthorized_access', 'security_breach', 'vulnerability_scan_failed'];
  ...
  checkAlertThresholds(eventType, details) {
    ...
    if (threshold && current >= threshold) {
      console.warn(`[SecurityAuditLogger] ALERT: ${eventType} threshold exceeded (${current}/${threshold})`);
    }
  }
  ...
  async getRecentEvents(limit = 50) {
    try {
      ...
      console.error('[SecurityAuditLogger] Failed to read security events:', error);
      return [];
    }
  }
}
