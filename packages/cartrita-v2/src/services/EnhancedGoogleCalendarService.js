...
class EnhancedGoogleCalendarService {
    ...
    async initialize(dbPool) {
        ...
        try {
            console.log('[GoogleCalendar] Initializing Enhanced Google Calendar Service...');
            
            this.dbPool = dbPool;
            
            // Validate required environment variables
            const requiredVars = ['GOOGLE_API_KEY', 'GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET'];
            ...
            console.log('[GoogleCalendar] ✅ Enhanced Google Calendar Service initialized successfully');
        } catch (error) {
    ...
    async testConnection() {
        try {
            ...
            if (error.code === 401) {
                console.warn('[GoogleCalendar] ⚠️ Authentication required - will need user authorization');
                return false;
            }
            throw error;
        }
    ...
    async findFreeTime(calendarId, duration, timeRange = {}) {
        try {
            ...
            const freeBusyRequest = {
                ...
                timeMax: timeRange.end || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
    ...
    startPeriodicSync() {
        ...
        this.syncInterval = setInterval(async () => {
            try {
                ...
                for (const calendar of calendars.slice(0, 5)) { // Limit to first 5 calendars
