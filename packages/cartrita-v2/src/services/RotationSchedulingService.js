...
class RotationSchedulingService {
      ...
      if (id) {
        ...
        result = await db.query(
          `UPDATE rotation_policies 
           ...
           RETURNING *`,
          [id, provider_name, policy_name, interval_days, auto_rotate, grace_days, enabled, userId]
        );
      } else {
        // Create new policy
        result = await db.query(
          `INSERT INTO rotation_policies 
           ...
           RETURNING *`,
          [userId, provider_name, policy_name, interval_days, auto_rotate, grace_days, enabled]
        );
      }
  ...
  async applyPolicyToKeys(userId, policy) {
    try {
      ...
      console.log(`[RotationService] Applied policy ${policy.policy_name} to keys`);
    } catch (error) {
  ...
  async getKeysNeedingRotation(userId) {
    try {
      ...
      console.error('[RotationService] Error getting keys needing rotation:', error);
    ...
    try {
      ...
      const eventResult = await db.query(
        `INSERT INTO rotation_events 
         ...
         RETURNING *`,
        [keyId, 'manual', reason, this.hashKey(apiKey.encrypted_key), performedBy || userId]
      );
      ...
      return {
        affected_keys: result.rows.map(key => ({
          ...
          days_until_new_rotation: Math.ceil(
            (new Date(key.new_rotation_due) - new Date()) / (1000 * 60 * 60 * 24)
          )
        }))
      };
    } catch (error) {
