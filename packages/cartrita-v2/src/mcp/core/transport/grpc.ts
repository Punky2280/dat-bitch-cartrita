...
import { Server, ServerCredentials, Client, loadPackageDefinition, InterceptingCall, ListenerBuilder } from '@grpc/grpc-js';
...
export class MCPGRPCTransport extends EventEmitter {
  ...
  private loadProtoDefinition(): void {
    try {
      ...
      this.serviceDefinition = loadPackageDefinition(packageDefinition) as GRPCServiceDefinition;
  ...
  private createClientCredentials(): any {
    ...
    try {
      ...
      this.logger.error('Failed to create client TLS credentials', error as Error);
      const { credentials } = require('@grpc/grpc-js');
      return credentials.createInsecure();
    }
  }
 ...
 */
export function createGRPCTransport(options: GRPCTransportOptions): MCPGRPCTransport {
