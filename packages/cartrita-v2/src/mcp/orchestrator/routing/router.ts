...
export class MCPRouter extends EventEmitter {
   ...
   */
  async registerAgent(registration: AgentRegistration): Promise<AgentRegistration> {
   ...
   */
  async routeTask(taskRequest: TaskRequest, supervisorType?: string): Promise<TaskResponse> {
  ...
  private findAvailableSupervisor(supervisorType: string): SupervisorInfo | null {
    // Find supervisors that can handle this type
    const candidates = Array.from(this.supervisors.values()).filter(s => 
