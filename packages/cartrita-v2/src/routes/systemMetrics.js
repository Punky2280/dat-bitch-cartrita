...
router.get('/metrics', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Metrics collection failed' });
  }
});
...
router.get('/health', (req, res) => {
  ...
  const basicHealth = {
    ...
    services: lastMetrics?.services || { database: { status: 'unknown' }, redis: { status: 'unknown' }, agents: { status: 'unknown' } }
  };
