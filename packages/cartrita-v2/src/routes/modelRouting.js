...
async function ensureRouterLoaded() {
  ...
  const compiledPath = path.resolve(process.cwd(), 'packages/backend/dist/router/HuggingFaceRouterService.js');
  const jsPath = path.resolve(process.cwd(), 'packages/backend/src/modelRouting/HuggingFaceRouterService.js');
  const tsPath = path.resolve(process.cwd(), 'packages/backend/src/modelRouting/HuggingFaceRouterService.ts');
  ...
  try {
    ...
    if (isDev) {
      ...
      try {
        ...
        const r = spawnSync('npm', ['run', 'build:router'], { cwd: path.resolve(process.cwd(), 'packages/backend'), stdio: 'inherit' });
      ...
      try {
        ...
        routerService = modTs.default || modTs.HuggingFaceRouterService || modTs;
        return routerService;
      } catch (e2) {
        routerInitError = e2;
      }
...
router.get('/catalog', authenticateToken, async (req, res) => {
  ...
  if (!svc) {
    return res.status(503).json({ success: false, error: 'Model router unavailable', detail: routerInitError?.message });
  }
  try {
...
router.post('/route', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (global.otelCounters?.hfRoutingLatency) {
      global.otelCounters.hfRoutingLatency.record(result.timing_ms, { model: result.model_id, task: result.task });
    }
...
router.post('/classify', authenticateToken, async (req, res) => {
  ...
  const svc = await ensureRouterLoaded();
  if (!svc) return res.status(503).json({ success: false, error: 'Model router unavailable', detail: routerInitError?.message });
  try {
    const task = svc.classify(prompt);
    const shortlist = svc.getCatalog().filter(m => m.category === task).slice(0,6).map(m=>m.repo_id);
