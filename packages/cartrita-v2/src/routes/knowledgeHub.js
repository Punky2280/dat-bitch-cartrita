...
const CreateEntrySchema = z.object({
    ...
    content_type: z.enum(['text', 'markdown', 'code', 'url', 'file']).default('text'),
    category: z.string().max(100).default('general'),
    subcategory: z.string().max(100).optional(),
    tags: z.array(z.string()).default([]),
    importance_score: z.number().min(0).max(1).default(0.5),
    source_type: z.enum(['user_created', 'imported', 'ai_generated', 'web_scraped']).default('user_created'),
...
const CreateClusterSchema = z.object({
    ...
    cluster_type: z.enum(['semantic', 'topical', 'hierarchical']).default('semantic'),
...
router.post('/entries', async (req, res) => {
    try {
        ...
        const entry = await KnowledgeHubService.createEntry(validatedData, req.user.id);
...
router.put('/entries/:id', async (req, res) => {
    try {
        ...
        const entry = await KnowledgeHubService.updateEntry(entryId, validatedData, req.user.id);
...
router.post('/clusters', async (req, res) => {
    try {
        ...
        const cluster = await KnowledgeHubService.createCluster(validatedData, req.user.id);
...
router.post('/relationships', async (req, res) => {
    try {
        const { source_entry_id, target_entry_id, relationship_type, strength } = req.body;
        
        if (!source_entry_id || !target_entry_id || !relationship_type) {
            return res.status(400).json({ 
                error: 'source_entry_id, target_entry_id, and relationship_type are required' 
            });
        }
