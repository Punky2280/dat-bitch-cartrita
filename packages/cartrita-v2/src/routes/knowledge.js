...
import AdvancedRAGPipeline from '../services/AdvancedRAGPipeline.js';
...
const traceOperation = (name, fn) => {
  if (OpenTelemetryTracing && OpenTelemetryTracing.traceOperation) {
    ...
    console.log(`[Knowledge] Executing: ${name}`);
    return fn();
  }
};

const traceAgentOperation = (name, fn) => {
  if (OpenTelemetryTracing && OpenTelemetryTracing.traceAgentOperation) {
    return OpenTelemetryTracing.traceAgentOperation(name, {}, fn);
  } else {
    console.log(`[Knowledge] Agent operation: ${name}`);
    return fn();
  }
};
...
const initializeRAG = () => {
  if (!advancedRAG && knowledgeHub.isInitialized()) {
    advancedRAG = new AdvancedRAGPipeline(knowledgeHub);
    console.log('🧠 Advanced RAG Pipeline initialized');
  }
  return advancedRAG;
};

// Knowledge Hub root endpoint
router.get('/', authenticateToken, (req, res) => {
  const ragStatus = initializeRAG();
  res.json({
    success: true,
    service: 'Enhanced Knowledge Hub',
    status: knowledgeHub.isInitialized() ? 'operational' : 'initializing',
    version: '3.1',
    features: [
      ...
      'Advanced RAG Pipeline with multi-stage retrieval',
      'Chain-of-Thought reasoning',
      'Context optimization and conflict resolution',
      'Quality assessment and feedback loops',
      'Conversation-aware memory'
    ],
    endpoints: [
      '/search', '/categories', '/collections', '/documents', '/entries', 
      '/clusters', '/graph', '/analytics', '/processing-status', 
      '/relationships', '/extractions', '/stats',
      '/advanced-rag', '/rag-metrics', '/rag-config', '/rag-memory'
    ],
    advancedRAG: {
      status: ragStatus ? 'operational' : 'initializing',
      features: ragStatus ? [
        'Multi-stage retrieval (dense + sparse + rerank)',
        'Query expansion and intent classification',
        'Context compression and conflict detection',
        'Chain-of-Thought reasoning',
        'Quality assessment with confidence scoring',
        'Conversation memory (5 turns)',
        'Fallback generation for failed queries'
      ] : []
    },
    statistics: knowledgeHub.processingStats,
    timestamp: new Date().toISOString()
  });
});

// Knowledge Hub health endpoint
router.get('/health', (req, res) => {
  res.json({
    success: true,
    status: knowledgeHub.isInitialized() ? 'healthy' : 'initializing',
    service: 'Enhanced Knowledge Hub',
...
router.post('/advanced-rag', authenticateToken, async (req, res) => {
  return await traceOperation('knowledge.advanced_rag.query', async () => {
    try {
      const rag = initializeRAG();
      if (!rag) {
        return res.status(503).json({ 
          success: false, 
          error: 'Advanced RAG pipeline not ready',
          retry_after_ms: 3000
        });
      }
      ...
      if (!query) {
        return res.status(400).json({
          success: false,
          error: 'Query is required for advanced RAG processing'
        });
      }

      const result = await rag.generateAdvancedResponse(userId, query, options);
      
      // Log successful high-quality responses
      if (result.success && result.confidence > 0.8) {
        console.log(`[Knowledge] 🧠 High-quality RAG response (confidence: ${result.confidence})`);
      }

      res.json(result);
    } catch (error) {
      console.error('[Knowledge] ❌ Advanced RAG error:', error);
      res.status(500).json({
        success: false,
        error: 'Failed to generate advanced RAG response',
...
router.get('/rag-metrics', authenticateToken, async (req, res) => {
  try {
    ...
    console.error('[Knowledge] ❌ RAG metrics error:', error);
    res.status(500).json({
      success: false,
...
router.post('/rag-config', authenticateToken, async (req, res) => {
  try {
    const rag = initializeRAG();
    if (!rag) {
      return res.status(503).json({ 
    ...
    res.json({
      success: true,
      message: 'RAG pipeline configuration updated',
      newConfig: rag.getMetrics().config
    });
  } catch (error) {
    console.error('[Knowledge] ❌ RAG config error:', error);
    res.status(500).json({
      success: false,
...
router.delete('/rag-memory/:userId', authenticateToken, async (req, res) => {
  try {
    const rag = initializeRAG();
    if (!rag) {
      return res.status(503).json({ 
    ...
    // Only allow users to clear their own memory, unless admin
    if (targetUserId !== req.user.id.toString() && req.user.role !== 'admin') {
      return res.status(403).json({
        success: false,
        error: 'Unauthorized to clear memory for other users'
      });
    }

    rag.clearConversationMemory(parseInt(targetUserId));
    
    res.json({
      success: true,
      message: `Conversation memory cleared for user ${targetUserId}`
    });
  } catch (error) {
    console.error('[Knowledge] ❌ RAG memory clear error:', error);
    res.status(500).json({
      success: false,
...
router.delete('/rag-memory', authenticateToken, async (req, res) => {
  try {
    const rag = initializeRAG();
    if (!rag) {
      return res.status(503).json({ 
    ...
    rag.clearConversationMemory(req.user.id);
    
    res.json({
      success: true,
      message: `Conversation memory cleared for user ${req.user.id}`
    });
  } catch (error) {
    console.error('[Knowledge] ❌ RAG memory clear error:', error);
    res.status(500).json({
