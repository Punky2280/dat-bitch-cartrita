...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      version: '3.0.0',
      description: 'Production-grade computer vision with TensorFlow.js integration and comprehensive ML capabilities',
      capabilities: [
      ...
      models: {
        classification: status.models?.available?.includes('classification') ? 'TensorFlow.js Basic' : 'Available',
        detection: status.models?.available?.includes('detection') ? 'TensorFlow.js Detection' : 'Available',
      ...
      technical_specs: {
        supported_formats: status.supportedFormats || ['image/jpeg', 'image/png', 'image/webp', 'image/bmp'],
    ...
    async (span) => {
      try {
        ...
        span.setAttributes({
          ...
          'cv.classifications_count': result.success ? result.classifications.length : 0
        });
    ...
    async (span) => {
      try {
        ...
        span.setAttributes({
          ...
          'cv.objects_detected': result.success ? result.detections.length : 0
        });
 ...
 * @body    multipart/form-data with 'image' field
 */
router.post('/embeddings', authenticateToken, upload.single('image'), async (req, res) => {
  return await OpenTelemetryTracing.traceOperation(
    ...
    async (span) => {
      try {
        ...
        span.setAttributes({
          ...
          'cv.embedding_dimensions': result.success ? result.dimensions?.length || 512 : 0
        });
    ...
    async (span) => {
      try {
        ...
        span.setAttributes({
          ...
          'cv.has_color_analysis': result.success ? !!result.analysis?.colors : false
        });
    ...
    async (span) => {
      try {
        ...
        span.setAttributes({
          ...
          'cv.text_length': result.success ? result.results?.text?.length || 0 : 0
        });
  ...
  return await OpenTelemetryTracing.traceOperation(
    ...
    async (span) => {
      try {
        ...
        const validTasks = ['classification', 'detection', 'embedding', 'ocr', 'analysis'];
        ...
        span.setAttributes({
          ...
          'batch.task': task,
          'batch.total_size_bytes': req.files.reduce((sum, file) => sum + file.size, 0)
        });
