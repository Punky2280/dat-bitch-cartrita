    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes('MessageBus') && i > 0) {
        ...
        if (
          prevLines.some(
            prevLine =>
              prevLine.includes('MessageBus') && prevLine.includes('require')
          )
        ) {
          ...
          if (!line.trim().startsWith('//')) {
            ...
            modified = true;
            console.log(
              `Commented duplicate MessageBus in ${filePath}:${i + 1}`
            );
          }
        }
      }
      ...
      if (line.trim().startsWith('case ') && i < lines.length - 1) {
        ...
        if (
          nextLine &&
          !nextLine.includes('break') &&
          !nextLine.includes('return') &&
          lines[i + 2] &&
          (lines[i + 2].trim().startsWith('case ') ||
            lines[i + 2].trim().startsWith('default'))
        ) {
...
console.log('\n🔧 Running ESLint --fix...');
try {
  execSync('npx eslint packages/backend/src --ext .js --fix', {
    stdio: 'inherit',
  });
