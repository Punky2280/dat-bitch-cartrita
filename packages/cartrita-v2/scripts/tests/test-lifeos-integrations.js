...
async function testCalendarIntegration() {
  ...
  if (
    ...
    syncResult.error.code === 'CREDENTIALS_MISSING'
  ) {
    log.success(
      'Calendar sync endpoint working (credentials missing as expected)'
    );
...
async function testEmailIntegration() {
  ...
  if (
    ...
    log.success(
      'Email sync endpoint working (credentials missing as expected)'
    );
  } else if (syncResult.success) {
  ...
  const messagesResult = await makeRequest(
    'GET',
    '/api/email/messages?limit=10'
  );

  if (messagesResult.success) {
    log.success(
      `Retrieved ${messagesResult.data.messages.length} email messages`
    );
  } else {
...
async function testContactIntegration() {
  ...
  if (
    ...
    log.success(
      'Contact sync endpoint working (credentials missing as expected)'
    );
  } else if (syncResult.success) {
  ...
  const createResult = await makeRequest('POST', '/api/contacts', {
    ...
    last_name: 'Contact',
    email_addresses: [
      { email: 'test@example.com', type: 'work', primary: true },
    ],
...
async function testNotificationSystem() {
  ...
  if (notificationsResult.success) {
    log.success(
      `Retrieved ${notificationsResult.data.notifications.length} notifications`
    );
  } else {
  ...
  const prefsResult = await makeRequest(
    'GET',
  ...
  if (prefsResult.success) {
    log.success(
      `Retrieved ${prefsResult.data.preferences.length} notification preferences`
    );
  } else {
  ...
  const statsResult = await makeRequest(
    'GET',
    '/api/notifications/stats?days=7'
  );

  if (statsResult.success) {
    log.success('Notification statistics retrieved successfully');
    log.info(
      `Total notifications: ${statsResult.data.stats.total_notifications}`
    );
  } else {
...
async function testPrivacyControls() {
  ...
  if (consentResult.success) {
    log.success(
      `Retrieved ${consentResult.data.consent.length} consent records`
    );
  } else {
  ...
  if (retentionResult.success) {
    log.success(
      `Retrieved ${retentionResult.data.retention_policies.length} retention policies`
    );
  } else {
  ...
  if (dataTypesResult.success) {
    log.success(
      `Retrieved ${dataTypesResult.data.data_types.length} available data types`
    );
  } else {
...
async function testApiKeyVault() {
  ...
  if (providersResult.success) {
    log.success(
      `Retrieved ${providersResult.data.providers.length} API providers`
    );
  } else {
