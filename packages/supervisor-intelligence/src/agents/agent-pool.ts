...
import { Logger } from '@cartrita/mcp-core';
...
export interface AgentInfo {
  ...
  execute: (request: any, context: any) => Promise<any>;
}

export interface IntelligenceAgentPoolConfig {
  openai?: {
    apiKey?: string;
    organization?: string;
  };
...
export class IntelligenceAgentPool extends EventEmitter {
   ...
   */
  async initialize(config: IntelligenceAgentPoolConfig): Promise<void> {
   ...
   */
  getAgentsByCapability(taskType: string): AgentInfo[] {
    return Array.from(this.agents.values()).filter(agent =>
      agent.capabilities.includes(taskType) && agent.isHealthy
    );
  }
   ...
   */
  getAvailableAgent(taskType: string): AgentInfo | null {
  ...
  private async initializeLangChainAgents(config: IntelligenceAgentPoolConfig): Promise<void> {
  ...
  private async initializeHuggingFaceAgents(config: IntelligenceAgentPoolConfig): Promise<void> {
  ...
  private async initializeSpecializedAgents(config: IntelligenceAgentPoolConfig): Promise<void> {
    // Research Agent
