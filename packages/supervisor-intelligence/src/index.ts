...
sharedInProcessTransport.subscribe('supervisor-intelligence', msg => {
  // debug
  if (process.env.MCP_DEBUG) console.log('[supervisor-intelligence] received', msg.message_type, msg.id, 'to', msg.recipient);
  if (msg.recipient !== 'supervisor-intelligence' || msg.message_type !== 'TASK_REQUEST') return;
  let taskReq; try { taskReq = TaskRequestSchema.parse(msg.payload); } catch { return; }
  ...
  if (process.env.MCP_DEBUG) console.log('[supervisor-intelligence] publishing response for', taskReq.task_id);
  sharedInProcessTransport.publish(response);
});

export function startSupervisor() { return sharedInProcessTransport; }

if (import.meta.url === `file://${process.argv[1]}`) startSupervisor();
