...
import { IntelligenceAgentPool } from './agents/agent-pool.js';
...
export class IntelligenceSupervisor extends EventEmitter {
  ...
  private readonly agentPool: IntelligenceAgentPool;
  ...
  constructor(config: IntelligenceSupervisorConfig = {}) {
    ...
    this.agentPool = new IntelligenceAgentPool();
  ...
  private async executeTask(request: TaskRequest, mcpContext: any): Promise<TaskResponse> {
    ...
    try {
      ...
      const agent = await this.taskRouter.selectAgent(request.taskType, request.parameters);
