  ...
  const instance = {
    post: vi.fn(async (url, body) => {
      ...
      if (body && body.inputs && typeof body.inputs === 'string') {
        return { data: [{ generated_text: 'Mock output for: ' + body.inputs.slice(0, 20) }] };
      }
...
describe('HuggingFaceRouterService core logic', () => {
  ...
  beforeEach(async () => {
    ...
    if (typeof ServiceOrInstance === 'function') serviceInstance = new ServiceOrInstance();
    else serviceInstance = ServiceOrInstance;
  });

  it('classifies summarization / long context prompt', () => {
    const task = serviceInstance.classify?.('Summarize the following article about AI trends.');
    expect(task).toBeTypeOf('string');
    expect(['long_context','general','multilingual','embedding','rerank']).toContain(task);
  });

  it('shortlist returns scored models ordered by compositeScore desc', () => {
    const shortlist = serviceInstance.shortlist?.('Explain how attention works in transformers', 'text-generation', 5);
    expect(Array.isArray(shortlist)).toBe(true);
    expect(shortlist.length).toBeGreaterThan(0);
    let ordered = true;
    for (let i = 1; i < shortlist.length; i++) {
      if ((shortlist[i-1].compositeScore || 0) < (shortlist[i].compositeScore || 0)) ordered = false;
    }
    expect(ordered).toBe(true);
  });

  it('route produces output and confidence', async () => {
    const result = await serviceInstance.route('List three uses of embeddings in NLP.');
