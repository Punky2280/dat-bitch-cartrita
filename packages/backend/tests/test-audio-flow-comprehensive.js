...
class AudioTestingSuite {
  ...
  async runAllTests() {
    console.log('🎵 Task 16: Audio Testing Flow Repair - Comprehensive Test Suite\\n');
  ...
  async testVoiceServiceEndpoints() {
    ...
    const endpoints = [
      { method: 'GET', path: '/api/voice-chat/status', name: 'Voice Chat Status' },
      { method: 'GET', path: '/api/voice-to-text/status', name: 'Voice-to-Text Status' },
      { method: 'POST', path: '/api/voice-chat/transcribe', name: 'Voice Chat Transcription' },
      { method: 'POST', path: '/api/voice-to-text/transcribe', name: 'Voice-to-Text Transcription' },
      { method: 'POST', path: '/api/voice-chat/synthesize', name: 'Text-to-Speech Synthesis' },
      { method: 'POST', path: '/api/voice-chat/credentials', name: 'Voice Chat Credentials' },
    ];
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (response.status === 200) {
          ...
          this.log(`⚠️ ${endpoint.name} - Available but requires proper authentication/data (status: ${response.status})`, 'warning');
        } else {
          this.log(`❌ ${endpoint.name} - Unexpected status: ${response.status}`, 'error');
        }
        ...
        if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
          ...
          this.log(`⚠️ ${endpoint.name} - Network error: ${error.message}`, 'warning');
        }
      }
    }
  }
  ...
  async testAudioFormatSupport() {
    ...
    for (const format of supportedFormats) {
      try {
        ...
        if (response.status === 200) {
          this.log(`✅ ${format.mime} format - Supported and processed`, 'success');
        } else if ([400, 422].includes(response.status)) {
          this.log(`⚠️ ${format.mime} format - Rejected (validation issue)`, 'warning');
        } else {
          this.log(`⚠️ ${format.mime} format - Authentication required`, 'warning');
        }

      } catch (error) {
        this.log(`❌ ${format.mime} format - Test failed: ${error.message}`, 'error');
      }
    }
  }

  async testTextToSpeechFlow() {
    this.log('Testing Text-to-Speech functionality with quality assessment...', 'test');

    const testPhrases = [
      { text: 'Hello, this is a test of the text to speech system.', context: 'Basic phrase' },
      { text: 'The quick brown fox jumps over the lazy dog.', context: 'Pangram test' },
      { text: 'Testing numbers: 123, 456, 789 and symbols: @#$%.', context: 'Special characters' },
    ];
    ...
    for (const testCase of testPhrases) {
      try {
        ...
        if (response.status === 200) {
          ...
          this.log(`✅ TTS ${testCase.context} - Success (${audioSize} bytes, ${latency}ms)`, 'success');
          
          // Quality assessment
          if (audioSize > 1000) {
            this.log(`✅ TTS Quality - Audio content generated (${audioSize} bytes)`, 'success');
          } else {
            this.log(`⚠️ TTS Quality - Unusually small audio file`, 'warning');
          }
          
          if (latency < 5000) {
            this.log(`✅ TTS Performance - Good latency (${latency}ms)`, 'success');
          } else {
            this.log(`⚠️ TTS Performance - High latency (${latency}ms)`, 'warning');
          }
        } else {
          this.log(`⚠️ TTS ${testCase.context} - Status ${response.status}`, 'warning');
        }

      } catch (error) {
        this.log(`❌ TTS ${testCase.context} - Failed: ${error.message}`, 'error');
      }
    }
  }

  async testSpeechToTextFlow() {
    this.log('Testing Speech-to-Text functionality with various inputs...', 'test');

    // Create test audio scenarios
    const testScenarios = [
      { name: 'Silent Audio', data: Buffer.alloc(2048, 0), expected: 'silence' },
      { name: 'White Noise', data: Buffer.alloc(2048).map(() => Math.random() * 255), expected: 'noise' },
      { name: 'Minimal Audio', data: Buffer.alloc(512, 128), expected: 'minimal' }
    ];
    ...
    for (const scenario of testScenarios) {
      try {
        ...
        formData.append('audio', audioBlob, `${scenario.name.toLowerCase().replace(' ', '_')}.wav`);
        ...
        if (response.status === 200) {
          ...
          this.log(`✅ STT ${scenario.name} - Success: "${transcript}" (${latency}ms)`, 'success');
          
          // Validate response structure
          if (response.data.confidence !== undefined) {
            this.log(`✅ STT Response Structure - Confidence score provided: ${response.data.confidence}`, 'success');
          }
          
          if (response.data.language !== undefined) {
            this.log(`✅ STT Language Detection - Language detected: ${response.data.language}`, 'success');
          }
        } else if (response.status === 400) {
          this.log(`⚠️ STT ${scenario.name} - Bad request (expected for test audio)`, 'warning');
        } else {
          this.log(`⚠️ STT ${scenario.name} - Status ${response.status}`, 'warning');
        }

      } catch (error) {
        if (error.response && error.response.status === 400) {
          this.log(`⚠️ STT ${scenario.name} - Bad request (expected for synthetic audio)`, 'warning');
        } else {
          this.log(`❌ STT ${scenario.name} - Failed: ${error.message}`, 'error');
        }
      }
    }
  }
  ...
  async testVoiceSessionLifecycle() {
    ...
    try {
      ...
      if (startResponse.status === 200) {
        ...
        const sessionId = startResponse.data.sessionId || startResponse.data.session_id;
        if (sessionId) {
          this.log(`✅ Voice Session - Session ID provided: ${sessionId}`, 'success');
          ...
          if (stopResponse.status === 200) {
            ...
            this.log(`⚠️ Voice Session - Stop session status: ${stopResponse.status}`, 'warning');
          }
        }
      } else {
        this.log(`⚠️ Voice Session - Start session status: ${startResponse.status}`, 'warning');
      }

    } catch (error) {
      this.log(`❌ Voice Session Lifecycle - Failed: ${error.message}`, 'error');
    }
  }
    ...
    return new Promise((resolve) => {
      ...
      ws.on('message', (data) => {
        try {
          ...
          this.log(`✅ WebSocket Audio - Received message type: ${message.type}`, 'success');
        } catch (error) {
      ...
      ws.on('error', (error) => {
        this.log(`❌ WebSocket Audio - Connection error: ${error.message}`, 'error');
        clearTimeout(timeout);
        resolve();
      });
      ...
      ws.on('close', () => {
        if (connectionSuccessful) {
          this.log('✅ WebSocket Audio - Connection closed gracefully', 'success');
        }
        clearTimeout(timeout);
        resolve();
      });
  ...
  async testAudioPostProcessing() {
    ...
    if (selectedAudio && selectedAudio.id === 2) {
      this.log('✅ Audio Selection Logic - Correctly selected highest quality audio', 'success');
    } else {
      this.log(`❌ Audio Selection Logic - Failed to select optimal audio (selected: ${selectedAudio?.id})`, 'error');
    }

    // Test duration filtering
    const longRecordings = mockAudioSelections.filter(audio => audio.duration > 3.0);
    if (longRecordings.length === 2) {
      this.log('✅ Audio Filtering - Duration filtering works correctly', 'success');
    } else {
      this.log('❌ Audio Filtering - Duration filtering failed', 'error');
    }

    // Test quality thresholding
    const highQualityRecordings = mockAudioSelections.filter(audio => audio.quality >= 0.8);
    if (highQualityRecordings.length === 2) {
      this.log('✅ Audio Quality Filter - Quality thresholding works correctly', 'success');
    } else {
      this.log('❌ Audio Quality Filter - Quality thresholding failed', 'error');
    }
  }
    ...
    for (const scenario of errorScenarios) {
      try {
        const config = {
          ...
          data: scenario.data,
          validateStatus: status => scenario.expectedStatus.includes(status) || status < 500,
          timeout: 5000
        };
        ...
        if (scenario.expectedStatus.includes(response.status)) {
          this.log(`✅ Error Handling - ${scenario.name}: Correct error response (${response.status})`, 'success');
        } else if (response.status < 500) {
          this.log(`⚠️ Error Handling - ${scenario.name}: Unexpected status ${response.status}`, 'warning');
        } else {
          this.log(`❌ Error Handling - ${scenario.name}: Server error ${response.status}`, 'error');
        }

      } catch (error) {
        if (error.response && scenario.expectedStatus.includes(error.response.status)) {
          this.log(`✅ Error Handling - ${scenario.name}: Correct error response`, 'success');
        } else {
          this.log(`❌ Error Handling - ${scenario.name}: Failed - ${error.message}`, 'error');
        }
      }
    }
  }
    ...
    for (const benchmark of benchmarks) {
      try {
        ...
        if (result.latency <= benchmark.sla) {
          this.log(`✅ Performance - ${benchmark.name}: ${result.latency}ms (within SLA)`, 'success');
        } else {
          this.log(`⚠️ Performance - ${benchmark.name}: ${result.latency}ms (exceeds SLA of ${benchmark.sla}ms)`, 'warning');
        }
      } catch (error) {
        this.log(`❌ Performance - ${benchmark.name}: Benchmark failed - ${error.message}`, 'error');
      }
    }
  }
  ...
  async benchmarkSTTProcessing() {
    ...
    try {
      const formData = new FormData();
      const audioBlob = new Blob([Buffer.alloc(1024, 128)], { type: 'audio/wav' });
  ...
  generateTestReport() {
    ...
    const statusIcon = this.failedTests === 0 ? '🎉' : this.failedTests < 3 ? '⚠️' : '❌';
    const status = this.failedTests === 0 ? 'EXCELLENT' : 
                   this.failedTests < 3 ? 'ACCEPTABLE' : 'NEEDS_ATTENTION';
    ...
    console.log('🎯 STT failure resolution and selection logic improvements validated');
  }
}
