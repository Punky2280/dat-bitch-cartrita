...
describe('Advanced Key Vault (lightweight)', () => {
  ...
  it('adds a key and fetches metadata without increment', async () => {
    const add = await request(server)
      ...
      .set(authHeader)
      .send({ serviceName: 'huggingface_api_token', key: 'hf_test_key_1234567890', scopes: ['read','write'], purposeTags: ['ai','ml'] });
  ...
  it('rotates key with version preservation and lists versions', async () => {
    const rotate = await request(server)
      ...
      .set(authHeader)
      .send({ serviceName: 'huggingface_api_token', newKey: 'hf_new_value_ABCDEFGHIJ', preserve_old: true, reason: 'test-rotation' });
