...
async function safeRequest(method, path, fn) {
  try {
    ...
    console.warn(`[availability] Skipped ${method.toUpperCase()} ${path}: backend not reachable`);
  }
}
...
describe('Endpoint availability', () => {
  ...
  it('POST /api/router is not accessible without auth (non-2xx)', async () => {
    try {
      ...
      console.warn('[availability] Skipped POST /api/router: backend not reachable');
    }
  });
  ...
  it('POST /api/hf/upload is auth-protected (non-2xx) and not 404', async () => {
    try {
      ...
      console.warn('[availability] Skipped POST /api/hf/upload: backend not reachable');
    }
  });
