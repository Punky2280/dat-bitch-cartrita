...
describe('Persona API', () => {
  it('GET returns persona with interaction_modes', async () => {
    ...
    expect(Object.values(res.body.persona.interaction_modes).some(Boolean)).toBe(true);
  });
  ...
  it('PUT clamps trait values and enforces interaction mode selection', async () => {
    const update = await request(server)
      ...
      .send({
        ...
        interaction_modes: { empathetic: false, witty: false, direct: true, analytical: false }
      });
    ...
    expect(Object.values(update.body.persona.interaction_modes).some(Boolean)).toBe(true);
