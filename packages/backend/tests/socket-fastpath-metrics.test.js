...
describe('Socket fast-path delegation metrics (soft)', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      await delay(800);
      try { resp = await request(BASE).get('/api/metrics/custom'); } catch(_) {}
    }
  ...
  it('increments fast-path counter when sending obvious vision request', async () => {
    ...
    socket = Client(BASE, { query: TOKEN ? { token: TOKEN } : {}, transports: ['websocket'] });
    await new Promise(res => socket.on('connect', res));
    socket.emit('user_message', { text: 'Please analyze image segmentation of this sample.' });
