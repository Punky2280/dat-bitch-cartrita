...
function selectBestAudioRecording(recordings) {
  ...
  const sorted = qualityFiltered.sort((a, b) => {
    ...
    console.log(`🔍 Comparing ${a.id} (q:${a.quality}) vs ${b.id} (q:${b.quality}), diff: ${qualityDiffRounded}`);
    ...
    console.log(`⏱️ Duration preference: ${a.id}(${a.duration}s, score:${aScore}) vs ${b.id}(${b.duration}s, score:${bScore})`);
...
async function testAudioSelectionLogic() {
  ...
  if (selectedAudio && selectedAudio.id === 2) {
    console.log('✅ PASS: Audio Selection Logic - Correctly selected highest quality audio');
    return true;
  } else {
    console.log(`❌ FAIL: Audio Selection Logic - Expected ID 2, got ID ${selectedAudio?.id}`);
    return false;
  }
}
...
async function testVoiceEndpoints() {
  ...
  const endpoints = [
    ...
    { method: 'POST', path: '/api/voice-chat/synthesize', name: 'Text-to-Speech' },
    { method: 'POST', path: '/api/voice-chat/transcribe', name: 'Voice Transcription' },
  ];
    ...
    try {
      ...
      if (endpoint.method === 'POST') {
        if (endpoint.path.includes('synthesize')) {
          ...
          const testAudio = new Blob([new ArrayBuffer(1024)], { type: 'audio/wav' });
      ...
      if (response.status === 200) {
        ...
        console.log(`⚠️ ${endpoint.name} - Available, needs valid data (${response.status})`);
        passCount++;
      } else if (response.status === 401) {
        console.log(`⚠️ ${endpoint.name} - Available, needs authentication (401)`);
...
async function main() {
  ...
  console.log(`✅ Audio Selection Logic: ${selectionTest ? 'FIXED' : 'STILL BROKEN'}`);
  console.log(`✅ Voice Endpoints: ${endpointTest ? 'WORKING' : 'ISSUES REMAIN'}`);
