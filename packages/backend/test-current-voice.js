...
async function testCurrentVoiceSetup() {
  ...
  try {
    ...
    // Test voice-chat routes (expecting 404 since server wasn't restarted)
    ...
    try {
      ...
      console.log('   📊 Services available:', Object.keys(statusResponse.data.services || {}));
    } catch (error) {
      console.log('   ❌ Service status endpoint error:', error.response?.status || error.message);
    }
    ...
    const routesMissing = routes.some(async (route) => {
      try {
        await axios.get(`${BASE_URL}${route}`, { validateStatus: () => true });
        return false;
      } catch (error) {
