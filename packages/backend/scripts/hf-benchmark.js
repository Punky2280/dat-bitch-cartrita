...
const prompts = [
  { task: 'text-generation', prompt: 'Explain retrieval augmented generation simply.' },
  { task: 'text-generation', prompt: 'List 5 security hardening measures for a Node.js API.' },
  { task: 'translation', prompt: 'Translate to French: The model router selected an efficient encoder.' },
  { task: 'embedding', prompt: 'Neural search vector representation importance.' },
  { task: 'rerank', prompt: 'Graph databases benefits for recommendation systems.' }
];
...
async function run() {
  ...
  for (const item of prompts) {
    ...
    process.stdout.write(`• ${item.task} -> ${results[results.length - 1].model} (${elapsed} ms)\n`);
  }
  ...
  const csv = [headers.join(',')].concat(
    results.map(r => headers.map(h => (r[h] !== null && r[h] !== undefined ? String(r[h]).replace(/,/g, ';') : '')).join(','))
  ).join('\n');
