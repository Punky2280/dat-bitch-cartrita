...
class SimpleHFJobsTest {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.orgs && data.orgs.length > 0) {
                    console.log(`🏢 Organizations: ${data.orgs.map(org => org.name).join(', ')}`);
                }
            } else {
        ...
        for (const modelId of modelsToTest) {
            try {
                ...
                if (response.ok) {
                    ...
                    console.log(`⚠️  ${modelId}: Access restricted (may require gated model approval)`);
                } else {
    ...
    showIntegrationSummary() {
        ...
        console.log('   - Jobs docs: https://huggingface.co/docs/huggingface_hub/guides/jobs');
