...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === '--temperature' || a === '-T') args.temperature = Number(argv[++i]);
...
async function run() {
  ...
  if (!prompt && args.prompt) {
    ...
    if (!fs.existsSync(filePath)) {
      console.error('Prompt file not found:', filePath); process.exit(1);
    }
  ...
  if (forceHttp) {
    ...
    try {
      const resp = await axios.post(`${base}/api/models/route`, {
        prompt,
        taskHint: args.task,
        maxCandidates: args.max,
        temperature: args.temperature,
      }, {
        headers: args.jwt ? { Authorization: `Bearer ${args.jwt}` } : undefined,
        timeout: 60_000,
      });
  ...
  try {
    const svcMod = await import('../src/modelRouting/HuggingFaceRouterService.ts').catch(async () => {
    ...
    const result = await instance.route({ prompt, taskHint: args.task, options: { maxCandidates: args.max, temperature: args.temperature } });
...
function output(payload, args) {
  ...
  if (Array.isArray(fallbacks) && fallbacks.length) console.log(`Fallbacks Used: ${fallbacks.length}`);
  if (args.verbose && Array.isArray(shortlist)) {
    console.log('\nShortlist:');
    shortlist.forEach((m, i) => {
      console.log(`  ${i+1}. ${m.repo_id || m.id || m.model} score=${m.compositeScore?.toFixed?.(3)}`);
    });
  }
  if (data.output) {
    console.log('\n--- Output ---');
    console.log(typeof data.output === 'string' ? data.output.trim() : JSON.stringify(data.output, null, 2));
  }
}
