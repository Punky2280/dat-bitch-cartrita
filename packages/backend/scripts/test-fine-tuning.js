...
const sampleTrainingData = [
    {
        messages: [
            ...
            { 
                role: "assistant", 
                content: "Machine learning is a subset of artificial intelligence (AI) that enables computers to learn and improve from experience without being explicitly programmed. It uses algorithms to analyze data, identify patterns, and make predictions or decisions." 
            }
        ]
    },
    {
        messages: [
            ...
            { 
                role: "assistant", 
                content: "Supervised learning works by training a model on labeled data, where both input features and correct outputs are provided. The algorithm learns to map inputs to outputs, then can make predictions on new, unseen data. Common examples include classification and regression tasks." 
            }
        ]
    },
    {
...
async function runTests() {
    ...
    results.forEach((result, index) => {
        ...
        console.log(`${status} Test ${index + 1}: ${result.test} (${result.status || 'Error'})`);
    });
    ...
    if (passed === total) {
        ...
        console.log('⚠️  Some tests failed. Check the API key configuration and server status.');
    }
