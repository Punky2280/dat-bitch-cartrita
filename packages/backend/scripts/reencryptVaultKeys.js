...
function decryptBlob(masterKey, blob){
  ...
  const decipher = crypto.createDecipheriv(ALGO, masterKey, iv, { authTagLength:16 });
...
function encryptBlob(masterKey, plaintext){
  ...
  const cipher = crypto.createCipheriv(ALGO, masterKey, iv, { authTagLength:16 });
...
async function main(){
  ...
  if(!oldPass || !newPass){
    console.error('❌ Set OLD_ENCRYPTION_MASTER_KEY and NEW_ENCRYPTION_MASTER_KEY');
    process.exit(1);
  }
  if(oldPass === newPass){
  ...
  try {
    ...
    const { rows } = await client.query('SELECT id, key_data, checksum FROM user_api_keys WHERE key_data IS NOT NULL');
    ...
    for(const row of rows){
      let plaintext;
      try { plaintext = decryptBlob(oldKey, row.key_data); } catch(e){
        console.error(`Row ${row.id} decrypt failed; aborting.`, e.message); throw e;
      }
      ...
      if(newChecksum !== row.checksum){
        console.warn(`Checksum mismatch for row ${row.id} (original stored ${row.checksum} vs recomputed ${newChecksum}) - updating to recomputed.`);
      }
      await client.query('UPDATE user_api_keys SET key_data=$1, checksum=$2, updated_at=CURRENT_TIMESTAMP WHERE id=$3',[newEncrypted, newChecksum, row.id]);
    }
    await client.query('COMMIT');
    console.log('✅ Re-encryption completed successfully.');
    console.log('⚠️  Update environment: set ENCRYPTION_MASTER_KEY to NEW value and restart services.');
  } catch(err){
