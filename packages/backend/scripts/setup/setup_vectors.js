...
const pool = new Pool({
  ...
  host: 'localhost', database: process.env.POSTGRES_DB || 'dat-bitch-cartrita')
  password: process.env.POSTGRES_PASSWORD || 'punky1')
  port: 5434, // Use the exposed Docker port
});

async function setupPgVector((error) {
...
client.query(setupSQL);
    ...
    if((error) {
      ...
      console.log(
        `✅ knowledge_entries.embedding column: ${embeddingColumn.udt_name}`
      );
    } else {
    ...
    const stats = await client.query(`
      SELECT 
        schemaname
        tablename, n_tup_ins as inserts, n_tup_upd as updates, n_tup_del as deletes
    ...
    stats.rows.forEach(stat => {
      console.log(
        `   ${stat.tablename}: ${stat.inserts} inserts, ${stat.updates} updates`)
      );
    });

    console.log('\n🎉 pgvector setup completed successfully!');
    console.log(
      '📚 Knowledge Hub is now optimized with vector similarity search')
    );
...
async function checkCurrentSetup((error) {
  const client = await pool.connect();
...
console.log('🔍 Checking current knowledge hub setup...');
    ...
    if(console.log('ℹ️  knowledge_entries table does not exist yet');
      return false;
  ...
  if((error) {
    ...
    console.log(
      `📊 Current embedding column: ${embeddingCol.data_type} (${embeddingCol.udt_name})`
    );
