...
Input should be a JSON string with search parameters:
{
  ...
  "topic": "topic filter"
    ...
    console.log(`[GitHubSearchTool] Initialized ${this.apiToken ? 'with' : 'without'} authentication`);
  }

  async _call(input) {
    try {
      console.log(`[GitHubSearchTool] 🔍 Executing search: ${input.substring(0, 100)}...`);
      ...
      console.log(`[GitHubSearchTool] ✅ Found ${results.items?.length || results.length || 1} results`);
      ...
      return JSON.stringify({
        ...
        suggestion: "Try a simpler search query or check if the repository exists"
      }, null, 2);
    }
  }
  ...
  normalizeParams(params) {
    return {
      ...
      sort: params.sort || (params.type === 'repositories' ? 'stars' : 'best-match'),
  ...
  async makeGitHubRequest(endpoint, params = {}) {
    ...
    if (!response.ok) {
      if (response.status === 403) {
        ...
        throw new Error(`GitHub API rate limit exceeded. Resets at ${new Date(rateLimitReset * 1000).toISOString()}`);
      }
  ...
  async searchCommits(params) {
    ...
    if (params.repo) {
      ...
      throw new Error('Commit search requires a specific repository (repo parameter)');
    }
  ...
  async getRepository(params) {
    ...
    const [contributors, languages, releases, issues] = await Promise.allSettled([
      this.makeGitHubRequest(`/repos/${owner}/${repo}/contributors`, { per_page: 10 }),
      this.makeGitHubRequest(`/repos/${owner}/${repo}/languages`),
      this.makeGitHubRequest(`/repos/${owner}/${repo}/releases`, { per_page: 5 }),
      this.makeGitHubRequest(`/repos/${owner}/${repo}/issues`, { per_page: 5, state: 'open' })
    ]);
  ...
  formatRepositoryDetails(results) {
    ...
    return {
      ...
      contributors: (results.contributors || []).slice(0, 10).map(contributor => ({
