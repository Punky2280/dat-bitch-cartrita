...
class HuggingFaceInferenceService {
  constructor() {
    ...
    console.log('[HuggingFaceInferenceService] 🔍 Token check:', {
      HF_TOKEN: process.env.HF_TOKEN ? `${process.env.HF_TOKEN.substring(0, 8)}...` : 'not found',
      HUGGINGFACE_API_TOKEN: process.env.HUGGINGFACE_API_TOKEN ? `${process.env.HUGGINGFACE_API_TOKEN.substring(0, 8)}...` : 'not found',
      HUGGINGFACE_API_KEY: process.env.HUGGINGFACE_API_KEY ? `${process.env.HUGGINGFACE_API_KEY.substring(0, 8)}...` : 'not found'
    });
    ...
    if (this.hfToken && this.hfToken.startsWith('hf_')) {
      ...
      this.client = null;
      console.warn('[HuggingFaceInferenceService] ⚠️ No valid HF token provided, service will return mock responses');
    }
    ...
    console.log('[HuggingFaceInferenceService] 🚀 Service initialized with comprehensive model support');
  }
  ...
  async chatCompletion(params) {
    try {
      ...
      console.log(`[HuggingFaceInferenceService] 💬 Chat completion request: ${params.model || 'default'}`);

      if (!this.client) {
        return this.getMockChatResponse(params);
      }

      const model = this.models.chat[params.model] || params.model || this.models.chat['deepseek-v3'];
      ...
      console.log(`[HuggingFaceInferenceService] ✅ Chat completion successful (${result.processingTime}ms)`);
      return result;

    } catch (error) {
      console.error('[HuggingFaceInferenceService] ❌ Chat completion failed:', error.message);
      return {
        success: false,
        error: error.message,
        model: params.model
  ...
  async visionChatCompletion(params) {
    try {
      const startTime = Date.now();
      console.log(`[HuggingFaceInferenceService] 👁️ Vision chat completion: ${params.model || 'default'}`);

      if (!this.client) {
        return this.getMockVisionResponse(params);
      }

      const model = this.models.vision[params.model] || params.model || this.models.vision['llava-1.5-7b'];
      const provider = params.provider || 'auto';
      ...
      const textContent = formattedMessages.find(msg => 
        typeof msg.content === 'string' || 
        (Array.isArray(msg.content) && msg.content.some(c => c.type === 'text'))
      );
      
      const imageContent = formattedMessages.find(msg => 
      ...
      const question = typeof textContent.content === 'string' 
        ? textContent.content 
        : textContent.content.find(c => c.type === 'text')?.text || 'Describe this image';
      ...
      console.log(`[HuggingFaceInferenceService] ✅ Vision completion successful (${result.processingTime}ms)`);
      return result;

    } catch (error) {
      console.error('[HuggingFaceInferenceService] ❌ Vision completion failed:', error.message);
      return {
        success: false,
        error: error.message,
        model: params.model,
  ...
  async textToImage(params) {
    try {
      const startTime = Date.now();
      console.log(`[HuggingFaceInferenceService] 🎨 Text-to-image: "${params.prompt.substring(0, 50)}..."`);

      if (!this.client) {
        return this.getMockImageResponse(params);
      }

      const model = this.models.textToImage[params.model] || params.model || this.models.textToImage['flux-1-dev'];
      const provider = params.provider || 'auto';
      ...
      console.log(`[HuggingFaceInferenceService] ✅ Image generated successfully (${result.processingTime}ms)`);
      return result;

    } catch (error) {
      console.error('[HuggingFaceInferenceService] ❌ Image generation failed:', error.message);
      return {
        success: false,
        error: error.message,
        prompt: params.prompt,
  ...
  async speechToText(params) {
    try {
      const startTime = Date.now();
      console.log(`[HuggingFaceInferenceService] 🎤 Speech-to-text transcription`);
      ...
      console.log(`[HuggingFaceInferenceService] ✅ Transcription successful (${response.processingTime}ms)`);
      return response;

    } catch (error) {
      console.error('[HuggingFaceInferenceService] ❌ Speech-to-text failed:', error.message);
      return {
        success: false,
        error: error.message,
        model: params.model
  ...
  async createEmbeddings(params) {
    try {
      const startTime = Date.now();
      console.log(`[HuggingFaceInferenceService] 📊 Creating embeddings for ${Array.isArray(params.inputs) ? params.inputs.length : 1} inputs`);

      if (!this.client) {
        return this.getMockEmbeddingResponse(params);
      }

      const model = this.models.embeddings[params.model] || params.model || this.models.embeddings['all-mpnet-base-v2'];
      ...
      console.log(`[HuggingFaceInferenceService] ✅ Embeddings created (${result.processingTime}ms)`);
      return result;

    } catch (error) {
      console.error('[HuggingFaceInferenceService] ❌ Embeddings failed:', error.message);
      return {
        success: false,
        error: error.message,
        model: params.model
