...
class SecurityMaskingService {
  ...
  async getMaskingPreferences(userId) {
    try {
      ...
      console.error('[SecurityMaskingService] Error getting preferences:', error);
  ...
  async updateMaskingPreferences(userId, preferences) {
    try {
      ...
      const result = await db.query(
        `INSERT INTO security_preferences 
         ...
         DO UPDATE SET 
           ...
           updated_at = NOW()
         RETURNING *`,
        [userId, default_visibility, reveal_timeout, require_confirmation, audit_access, mask_pattern, show_last_chars]
      );
      ...
      console.error('[SecurityMaskingService] Error updating preferences:', error);
      throw error;
    }
  }
  ...
  async maskSensitiveData(userId, data, dataType = 'api_key') {
    try {
      ...
      if (dataType === 'api_key' && data.startsWith('cartrita_')) {
        ...
        const mask = maskPattern.repeat(Math.ceil(maskLength / maskPattern.length)).substring(0, maskLength);
  ...
  async createRevealToken(userId, resourceId, resourceType) {
    try {
      const preferences = await this.getMaskingPreferences(userId);
      const expiresAt = new Date(Date.now() + (preferences.reveal_timeout * 1000));
      ...
      console.error('[SecurityMaskingService] Error creating reveal token:', error);
      throw error;
    }
  }
  ...
  async useRevealToken(token, userId) {
    try {
      ...
      // Mark token as used
      await db.query(
        'UPDATE reveal_tokens SET used_at = NOW() WHERE id = $1',
        [tokenData.id]
      );
      ...
      console.error('[SecurityMaskingService] Error using reveal token:', error);
      throw error;
    }
  }
  ...
  async getSecurityEvents(userId, limit = 50) {
    try {
      ...
      console.error('[SecurityMaskingService] Error getting security events:', error);
  ...
  async logSecurityEvent(userId, eventType, metadata = {}) {
    try {
      ...
      console.error('[SecurityMaskingService] Error logging security event:', error);
  ...
  async getMaskingAnalytics(userId) {
    try {
      ...
      return {
        ...
        reveal_tokens: tokensResult.rows[0] || { total_tokens: 0, used_tokens: 0, expired_tokens: 0 },
  ...
  async cleanupExpiredTokens(userId) {
    try {
      ...
      console.error('[SecurityMaskingService] Error cleaning up tokens:', error);
      throw error;
    }
  }
