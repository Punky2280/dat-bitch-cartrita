...
class WorkflowTemplateService {
  ...
  async _ensureTemplateCategories() {
    const result = await query('SELECT COUNT(*) as count FROM workflow_template_categories');
  ...
  async _seedCategories() {
    const categories = [
      { name: 'productivity', description: 'Productivity and task management workflows', icon: 'productivity', sort_order: 1 },
      { name: 'communication', description: 'Email, messaging, and collaboration workflows', icon: 'communication', sort_order: 2 },
      { name: 'knowledge', description: 'Information processing and knowledge management', icon: 'knowledge', sort_order: 3 },
      { name: 'automation', description: 'General automation and integration workflows', icon: 'automation', sort_order: 4 },
      { name: 'analytics', description: 'Data analysis and reporting workflows', icon: 'analytics', sort_order: 5 },
      { name: 'personal', description: 'Personal life and habit tracking workflows', icon: 'personal', sort_order: 6 }
    ];
  ...
  async getTemplates(options = {}) {
    return traceOperation('workflow.template.getTemplates', async (span) => {
      const { category_id, user_id, include_variables = false, limit = 50, offset = 0 } = options;
  ...
  async createTemplate(templateData, userId) {
    return traceOperation('workflow.template.createTemplate', async (span) => {
      const {
        name, description, category_id, config = {}, 
        variables = [], metadata = {}
      } = templateData;
      ...
      try {
        ...
        const workflowResult = await query(workflowQuery, [
          name, description, userId, JSON.stringify(metadata), category_id, JSON.stringify(config)
        ]);
        ...
        if (variables && variables.length > 0) {
          for (const variable of variables) {
            ...
            `, [
              templateId, variable.var_name, variable.description || null,
              variable.required !== false, variable.default_value || null,
              variable.var_type || 'string', variable.validation_pattern || null
            ]);
          }
        }
    ...
    return traceOperation('workflow.template.instantiate', async (span) => {
      ...
      try {
        ...
        template.variables.forEach(variable => {
          const value = variables[variable.var_name] || variable.default_value || '';
          const placeholder = `{{${variable.var_name}}}`;
          processedConfig = processedConfig.replace(new RegExp(placeholder, 'g'), value);
        });
        ...
        const instanceName = variables.__workflow_name || `${template.name} Instance`;
        const instanceDescription = variables.__workflow_description || 
          `Created from template: ${template.name}`;

        const workflowResult = await query(workflowQuery, [
          instanceName, instanceDescription, userId, templateId, processedConfig
        ]);
  ...
  async rateTemplate(templateId, userId, rating, review = null) {
    return traceOperation('workflow.template.rateTemplate', async (span) => {
      ...
      const result = await query(upsertQuery, [templateId, userId, rating, review]);
