...
class OpenAIFineTuningService {
    constructor() {
        ...
        if (!process.env.OPENAI_FINETUNING_API_KEY) {
            throw new Error('OPENAI_FINETUNING_API_KEY is required for fine-tuning operations');
        }
    }
    ...
    async listFineTuningJobs(options = {}) {
        try {
            ...
            console.log(`[FineTuning] Retrieved ${response.data.length} fine-tuning jobs`);
            return response;
        } catch (error) {
    ...
    async getFineTuningJob(jobId) {
        try {
            ...
            console.log(`[FineTuning] Retrieved job ${jobId} status: ${response.status}`);
            return response;
        } catch (error) {
            console.error(`[FineTuning] Error retrieving fine-tuning job ${jobId}:`, error);
            throw error;
        }
    }
    ...
    async cancelFineTuningJob(jobId) {
        try {
            ...
            console.error(`[FineTuning] Error cancelling fine-tuning job ${jobId}:`, error);
            throw error;
        }
    }
    ...
    async listFineTuningEvents(jobId, options = {}) {
        try {
            ...
            const response = await this.openai.fineTuning.jobs.listEvents(jobId, params);
            
            console.log(`[FineTuning] Retrieved ${response.data.length} events for job ${jobId}`);
            return response;
        } catch (error) {
            console.error(`[FineTuning] Error listing events for job ${jobId}:`, error);
            throw error;
        }
    }
    ...
    async listCheckpoints(jobId, options = {}) {
        try {
            ...
            const response = await this.openai.fineTuning.jobs.listCheckpoints(jobId, params);
            
            console.log(`[FineTuning] Retrieved ${response.data.length} checkpoints for job ${jobId}`);
            return response;
        } catch (error) {
            console.error(`[FineTuning] Error listing checkpoints for job ${jobId}:`, error);
            throw error;
        }
    }
    ...
    validateTrainingData(trainingData, method = 'supervised') {
        ...
        console.log(`[FineTuning] Validated ${trainingData.length} training examples for ${method} method`);
        return true;
    }
    ...
    validateSupervisedExample(example, index) {
        if (!example.messages || !Array.isArray(example.messages)) {
            throw new Error(`Example ${index}: 'messages' field is required and must be an array`);
        }
        ...
        example.messages.forEach((message, msgIndex) => {
            if (!message.role || !['user', 'assistant', 'system'].includes(message.role)) {
                throw new Error(`Example ${index}, Message ${msgIndex}: Invalid role "${message.role}"`);
            }

            if (!message.content && !message.tool_calls) {
                throw new Error(`Example ${index}, Message ${msgIndex}: Either 'content' or 'tool_calls' is required`);
            }
        });
    }
    ...
    validateDPOExample(example, index) {
        ...
        required.forEach(field => {
            if (!example[field]) {
                throw new Error(`Example ${index}: '${field}' field is required for DPO method`);
            }
        });
        ...
        [example.preferred_output, example.non_preferred_output].forEach((output, outputIndex) => {
            const outputName = outputIndex === 0 ? 'preferred_output' : 'non_preferred_output';
        ...
        while (true) {
            try {
                ...
                if (['succeeded', 'failed', 'cancelled'].includes(job.status)) {
                    console.log(`[FineTuning] Job ${jobId} finished with status: ${job.status}`);
                    return job;
                }

                console.log(`[FineTuning] Job ${jobId} status: ${job.status}, waiting ${pollInterval}ms...`);
