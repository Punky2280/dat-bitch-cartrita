...
export default class CameraVisionTestingService {
  ...
  async initialize() {
    ...
    try {
      console.log('[CameraVisionTesting] 🚀 Initializing Camera Vision Testing Service...');

      // Test service capabilities
      await this.testServiceCapabilities();

      this.initialized = true;
      console.log('[CameraVisionTesting] ✅ Camera Vision Testing Service initialized successfully');
    } catch (error) {
      console.error('[CameraVisionTesting] ❌ Failed to initialize service:', error);
      throw error;
    }
  }
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async (span) => {
        ...
        try {
          ...
          const testBuffer = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChAHGdXPgGAAAAASUVORK5CYII=', 'base64');
          ...
          span.setAttributes({
            ...
            'capabilities.multi_frame_analysis': capabilities.multi_frame_analysis,
            'capabilities.black_frame_detection': capabilities.black_frame_detection
          });

          console.log('[CameraVisionTesting] ✅ Service capabilities verified:', capabilities);
          return capabilities;
        } catch (error) {
          span.recordException(error);
          console.error('[CameraVisionTesting] ❌ Capability test failed:', error);
          throw error;
        }
      }
    );
  ...
  async createTestingSession(userId, sessionConfig = {}) {
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async (span) => {
        ...
        console.log(`[CameraVisionTesting] 📋 Created testing session: ${sessionId}`);
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async (span) => {
        try {
          ...
          const analysis = await this.performFrameAnalysis(imageBuffer, metadata, session.config);
          ...
          console.log(`[CameraVisionTesting] 📸 Frame analyzed for session ${sessionId}:`, {
          ...
          return {
            ...
            session_stats: {
              ...
              black_frames: session.captures.filter(c => c.analysis.isBlackFrame).length
            }
          };

        } catch (error) {
          span.recordException(error);
          console.error(`[CameraVisionTesting] ❌ Frame analysis failed for session ${sessionId}:`, error);
          return {
            success: false,
            error: error.message
          };
  ...
  async performFrameAnalysis(imageBuffer, metadata, config) {
    return await OpenTelemetryTracing.traceOperation(
      ...
      async (span) => {
        ...
        const isBlackFrame = this.detectBlackFrame(brightness, config.black_frame_threshold);
        
        // Quality assessment
        const qualityScore = this.calculateQualityScore(brightness, metadata, stats);

        // Color analysis
        const colorAnalysis = this.analyzeColorDistribution(stats.channels);

        // Frame stability assessment
        const stabilityScore = await this.assessFrameStability(imageBuffer, metadata);
        ...
        if (isBlackFrame) {
          analysis.suggestions.push({
            type: 'black_frame',
            message: 'Black frame detected. Check camera lens, lighting conditions, or permissions.',
            priority: 'high'
          });
        }
        ...
        if (qualityScore < 0.3) {
          analysis.suggestions.push({
            type: 'low_quality',
            message: 'Poor image quality detected. Improve lighting or check camera settings.',
            priority: 'medium'
          });
        }
        ...
        if (brightness.variance < 10) {
          analysis.suggestions.push({
            type: 'low_variance',
            message: 'Low color variance. Camera may be covered or in dark environment.',
            priority: 'medium'
          });
        }
  ...
  async performMultiFrameAnalysis(sessionId, frameCount = 5) {
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async (span) => {
        ...
        // Calculate trends
        const brightnessValues = recentCaptures.map(c => c.analysis.brightness.average);
        const qualityValues = recentCaptures.map(c => c.analysis.qualityScore);
        const blackFrameCount = recentCaptures.filter(c => c.analysis.isBlackFrame).length;

        const trends = {
          brightness: {
            trend: this.calculateTrend(brightnessValues),
            stability: this.calculateStability(brightnessValues),
            average: brightnessValues.reduce((a, b) => a + b, 0) / brightnessValues.length
          },
          quality: {
            trend: this.calculateTrend(qualityValues),
            stability: this.calculateStability(qualityValues),
            average: qualityValues.reduce((a, b) => a + b, 0) / qualityValues.length
          },
          consistency: {
        ...
        const performance = {
          overall_score: (trends.quality.average * 0.4 + trends.consistency.frame_consistency * 0.6),
          stability_rating: this.getStabilityRating(trends.brightness.stability, trends.quality.stability),
          issues_detected: []
        };
        ...
        console.log(`[CameraVisionTesting] 📊 Multi-frame analysis completed for session ${sessionId}:`, {
  ...
  async generateDiagnosticReport(sessionId) {
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async (span) => {
        ...
        const lowQualityFrames = captures.filter(c => c.analysis.qualityScore < 0.5);
        
        const avgBrightness = captures.reduce((sum, c) => sum + c.analysis.brightness.average, 0) / totalCaptures;
        const avgQuality = captures.reduce((sum, c) => sum + c.analysis.qualityScore, 0) / totalCaptures;
        ...
        const performance = {
          ...
          brightness_stability: this.calculateStability(captures.map(c => c.analysis.brightness.average)),
          quality_stability: this.calculateStability(captures.map(c => c.analysis.qualityScore))
        };
        ...
        // Recommendations
        const recommendations = this.generateDetailedRecommendations(performance, issues);
        ...
        const report = {
          ...
          summary: {
            ...
            overall_status: healthScore > 0.8 ? 'excellent' : healthScore > 0.6 ? 'good' : healthScore > 0.4 ? 'fair' : 'poor'
          },
          performance,
          issues,
          recommendations,
        ...
        console.log(`[CameraVisionTesting] 📋 Generated diagnostic report for session ${sessionId}:`, {
          healthScore,
          totalCaptures,
  ...
  calculateBrightnessVariance(channels) {
    ...
    return means.reduce((sum, mean) => sum + Math.pow(mean - avg, 2), 0) / means.length;
  }
  ...
  calculateQualityScore(brightness, metadata, stats) {
    ...
    const resolutionScore = Math.min((metadata.width * metadata.height) / (1920 * 1080), 1) * 0.3;
  ...
  analyzeColorDistribution(channels) {
    return {
      ...
      dominant_channel: channels.reduce((prev, current) => (prev.mean > current.mean) ? prev : current),
  ...
  getStabilityRating(brightnessStability, qualityStability) {
    ...
    if (avgStability > 0.9) return 'excellent';
    if (avgStability > 0.7) return 'good'; 
    if (avgStability > 0.5) return 'fair';
    return 'poor';
  }
  ...
  generateRecommendations(trends, performance) {
    ...
    if (performance.overall_score < 0.5) {
      recommendations.push({
        ...
        actions: ['Increase ambient lighting', 'Clean camera lens', 'Adjust camera settings']
      });
    }
    ...
    if (trends.consistency.black_frame_ratio > 0.2) {
      recommendations.push({
        ...
        actions: ['Check camera permissions', 'Verify camera is not covered', 'Restart camera application']
      });
    }
    ...
    if (trends.brightness.stability < 0.6) {
      recommendations.push({
        ...
        actions: ['Stabilize lighting conditions', 'Use fixed camera position', 'Check for auto-exposure settings']
      });
    }
  ...
  calculateHealthScore(performance) {
    ...
    // Reward stability
    score = score * (performance.brightness_stability * 0.5 + performance.quality_stability * 0.5);
  ...
  getResolutionRange(captures) {
    const resolutions = captures.map(c => ({ width: c.metadata.width, height: c.metadata.height }));
    const widths = resolutions.map(r => r.width);
    const heights = resolutions.map(r => r.height);

    return {
      min_resolution: { width: Math.min(...widths), height: Math.min(...heights) },
      max_resolution: { width: Math.max(...widths), height: Math.max(...heights) },
      unique_resolutions: [...new Set(resolutions.map(r => `${r.width}x${r.height}`))]
    };
  }
  ...
  cleanupSessions() {
    ...
    for (const [sessionId, session] of this.testSessions) {
      ...
      if (now - created > maxAge) {
        ...
        console.log(`[CameraVisionTesting] 🧹 Cleaned up expired session: ${sessionId}`);
      }
    }
  }
