...
class IntegratedAIService {
  ...
  async initialize() {
    ...
    try {
      console.log('[IntegratedAIService] 🚀 Initializing integrated AI service...');
      ...
      console.log('[IntegratedAIService] ✅ All AI services integrated and ready');
  ...
  async initializeRedis() {
    try {
      ...
      console.warn('[IntegratedAIService] ⚠️ Redis connection failed, continuing without caching:', error.message);
      this.redisClient = null;
    }
  }
  ...
  async initializeModelRegistry() {
    ...
    try {
      ...
      console.warn('[IntegratedAIService] ⚠️ Model Registry initialization failed:', error.message);
      console.warn('[IntegratedAIService] Continuing without Model Registry - workflows will use fallback models');
      this.modelRegistry = null;
    }
  }
  ...
  wireServices() {
    ...
    if (this.modelRegistry && this.workflowEngine) {
      ...
      console.log('[IntegratedAIService] ✅ Workflow Engine ↔ Model Registry connected');
    }
    ...
    if (this.knowledgeHub && this.workflowEngine) {
      ...
      console.log('[IntegratedAIService] ✅ Workflow Engine ↔ Knowledge Hub connected');
    }
  ...
  async executeWorkflow(workflowId, userId, inputData = {}, options = {}) {
    ...
    try {
      console.log(`[IntegratedAIService] 🚀 Executing integrated workflow ${workflowId}`);
      ...
      if (this.modelRegistry && result.success) {
        try {
          ...
          console.log(`[IntegratedAIService] 💰 Recent AI costs: $${costStats.data?.total_cost_usd || 0}`);
        } catch (costError) {
          console.warn('[IntegratedAIService] Failed to retrieve cost stats:', costError.message);
        }
      }
      ...
      console.error(`[IntegratedAIService] ❌ Workflow execution failed:`, error);
      throw error;
    }
  }
  ...
  async processDocument(userId, fileInfo, filePath, metadata = {}) {
    ...
    try {
      console.log(`[IntegratedAIService] 📄 Processing document: ${fileInfo.originalname}`);
      ...
      console.error('[IntegratedAIService] ❌ Document processing failed:', error);
      throw error;
    }
  }
  ...
  async semanticSearch(userId, query, options = {}) {
    ...
    try {
      const result = await this.knowledgeHub.semanticSearch(userId, query, options);
      return result;
    } catch (error) {
  ...
  async selectModel(criteria, context = {}, strategy = 'balanced') {
    ...
    try {
      const result = await this.modelRegistry.selectModel(criteria, context, strategy);
      return result;
    } catch (error) {
  ...
  async executeModelInference(modelId, prompt, options = {}) {
    ...
    try {
      const result = await this.modelRegistry.executeInference(modelId, prompt, options);
      return result;
    } catch (error) {
  ...
  async getCostAnalytics(timeRangeHours = 24) {
    ...
    if (this.modelRegistry) {
      try {
        const modelCosts = await this.modelRegistry.getCostStatistics(timeRangeHours);
  ...
  async cleanup() {
    ...
    if (this.redisClient) {
      try {
        ...
        console.warn('[IntegratedAIService] Redis disconnect error:', error.message);
      }
    }
