...
class PrivacyControlService extends EventEmitter {
  ...
  async initialize() {
    try {
      await this.setupPrivacyTables();
      await this.setupDefaultPolicies();
      await this.startRetentionScheduler();
      
      this.initialized = true;
      console.log('🔒 PrivacyControlService fully initialized');
      return true;
    } catch (error) {
      console.error('Error initializing PrivacyControlService:', error);
      throw error;
    }
  }
  ...
  async setupPrivacyTables() {
    try {
      ...
      console.log('🔒 Privacy control tables setup completed');
    } catch (error) {
      console.error('Error setting up privacy tables:', error);
      throw error;
    }
  }
  ...
  async setupDefaultPolicies() {
    try {
      // Get users who don't have retention policies set
      const usersResult = await pool.query(`
        SELECT id FROM users 
        WHERE NOT EXISTS (
          SELECT 1 FROM user_data_retention_policies 
      ...
      for (const user of usersResult.rows) {
        for (const [dataType, retentionDays] of Object.entries(this.defaultRetentionPeriods)) {
          ...
          `, [
            ...
            this.anonymizationFields.hasOwnProperty(dataType)
          ]);
        }

        // Set default consent (assuming GDPR compliance - explicit consent required)
        const consentTypes = ['data_processing', 'service_improvement', 'communications'];
        ...
        for (const consentType of consentTypes) {
          ...
          `, [
            user.id, 
            consentType, 
            consentType === 'data_processing', // Only core processing is defaulted to true
            JSON.stringify({
              scope: consentType === 'data_processing' ? 'essential' : 'optional',
              granted_features: consentType === 'data_processing' ? ['calendar', 'email', 'contacts'] : []
            })
          ]);
        }
      }

      console.log('🔒 Default privacy policies setup completed');
    } catch (error) {
  ...
  async enforceRetentionPolicies() {
    try {
      ...
      const policies = await pool.query(`
        ...
        WHERE auto_delete = true AND retention_days > 0
      `);

      for (const policy of policies.rows) {
        await this.enforceUserRetentionPolicy(policy);
      }
   ...
   */
  async enforceUserRetentionPolicy(policy) {
    try {
          ...
          if (policy.anonymize_before_delete) {
            const result = await pool.query(`
              ...
              SET sender_email = 'anonymized@privacy.local', sender_name = 'Anonymous',
                  ...
                  body_html = '[Content Anonymized]', synced_at = NOW()
      ...
      if (deletedCount > 0 || anonymizedCount > 0) {
        await this.logDataAccess(policy.user_id, null, policy.data_type, 'delete', null, 'retention_policy');
        console.log(`🔒 User ${policy.user_id} - ${policy.data_type}: deleted ${deletedCount}, anonymized ${anonymizedCount}`);
      }

    } catch (error) {
      console.error(`Error enforcing retention policy for user ${policy.user_id}, data type ${policy.data_type}:`, error);
    }
  }
   ...
   */
  async recordConsent(userId, consentType, consentGiven, details = {}, ipAddress = null, userAgent = null) {
    try {
      const result = await pool.query(`
        INSERT INTO user_consent_records (
          user_id, consent_type, consent_given, consent_details, 
        ...
        RETURNING *
      `, [userId, consentType, consentGiven, JSON.stringify(details), ipAddress, userAgent]);

      await this.logDataAccess(userId, userId, 'consent_records', 'write', result.rows[0].id, 'consent_update');
  ...
  async getUserConsent(userId) {
    try {
      ...
      await this.logDataAccess(userId, userId, 'consent_records', 'read', null, 'consent_check');
   ...
   */
  async updateRetentionPolicy(userId, dataType, retentionDays, autoDelete = true, anonymizeBeforeDelete = false) {
    try {
      const result = await pool.query(`
        UPDATE user_data_retention_policies 
        SET retention_days = $3, auto_delete = $4, anonymize_before_delete = $5, updated_at = NOW()
      ...
      if (result.rows.length === 0) {
        ...
        await this.logDataAccess(userId, userId, 'retention_policies', 'write', createResult.rows[0].id, 'policy_create');
        return createResult.rows[0];
      }

      await this.logDataAccess(userId, userId, 'retention_policies', 'write', result.rows[0].id, 'policy_update');
      return result.rows[0];
    } catch (error) {
   ...
   */
  async getUserRetentionPolicies(userId) {
    try {
      const result = await pool.query(`
      ...
      await this.logDataAccess(userId, userId, 'retention_policies', 'read', null, 'policy_check');
   ...
   */
  async requestDataExport(userId, exportType = 'full', dataTypes = []) {
    try {
      const result = await pool.query(`
      ...
      await this.logDataAccess(userId, userId, 'export_requests', 'write', result.rows[0].id, 'export_request');
   ...
   */
  async processDataExport(exportRequestId) {
    try {
      ...
      const requestResult = await pool.query(`
        SELECT * FROM user_data_export_requests WHERE id = $1
      `, [exportRequestId]);
      ...
      if (request.export_type === 'full' || request.data_types.includes('calendar_events')) {
      ...
      if (request.export_type === 'full' || request.data_types.includes('email_messages')) {
      ...
      if (request.export_type === 'full' || request.data_types.includes('contacts')) {
      ...
      if (request.export_type === 'full' || request.data_types.includes('notifications')) {
      ...
      if (request.export_type === 'full' || request.data_types.includes('conversations')) {
      ...
      await fs.writeFile(filePath, exportJson, 'utf8');
      ...
      await this.logDataAccess(request.user_id, request.user_id, 'user_data', 'export', exportRequestId, 'data_export');

      console.log(`🔒 Data export completed for user ${request.user_id}, file: ${fileName}`);
   ...
   */
  async requestDataDeletion(userId, deletionType = 'full_account', dataTypes = [], reason = '') {
    try {
      ...
      await this.logDataAccess(userId, userId, 'deletion_requests', 'write', result.rows[0].id, 'deletion_request');
  ...
  async verifyAndProcessDeletion(requestId, verificationToken) {
    try {
      const result = await pool.query(`
        ...
        SET verified_at = NOW(), status = 'processing'
        WHERE id = $1 AND verification_token = $2 AND verified_at IS NULL
        RETURNING *
      `, [requestId, verificationToken]);

      if (result.rows.length === 0) {
        throw new Error('Invalid verification token or request already processed');
      }
  ...
  async processDataDeletion(requestId) {
    try {
      const requestResult = await pool.query(`
        SELECT * FROM user_data_deletion_requests WHERE id = $1
      `, [requestId]);
      ...
      if (request.deletion_type === 'full_account') {
        // Delete all user data
        const tables = [
          'user_calendar_events',
          'user_email_messages', 
          'user_contacts',
          'user_contact_interactions',
          'user_notifications',
          'conversations',
      ...
      } else if (request.deletion_type === 'partial_data') {
      ...
      await this.logDataAccess(request.user_id, request.user_id, 'user_data', 'delete', requestId, 'data_deletion');

      console.log(`🔒 Data deletion completed for user ${request.user_id}:`, deletionResults);
   ...
   * @param {number} accessedByUserId - User who accessed the data (null for system)
   ...
   */
  async logDataAccess(userId, accessedByUserId, dataType, accessType, resourceId = null, accessReason = '', ipAddress = null, userAgent = null) {
    try {
      await pool.query(`
      ...
      `, [userId, accessedByUserId, dataType, accessType, resourceId, accessReason, ipAddress, userAgent]);
    } catch (error) {
   ...
   */
  async getUserAccessLog(userId, filters = {}) {
    try {
      const { limit = 100, offset = 0, access_type, data_type, days = 30 } = filters;
      ...
      await this.logDataAccess(userId, userId, 'access_log', 'read', null, 'access_log_check');
  ...
  cleanup() {
    ...
    console.log('🔒 PrivacyControlService cleaned up');
  }
}
