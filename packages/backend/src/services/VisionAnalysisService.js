...
class VisionAnalysisService {
  constructor() {
    ...
    console.log('[VisionAnalysisService] ✅ Initialized with capabilities:', Object.keys(this.analysisTypes));
  }
  ...
  async analyzeImage(images, options = {}) {
    try {
      ...
      console.log(`[VisionAnalysisService] 🔍 Analyzing ${processedImages.length} image(s) - type: ${analysisType}`);
      
      // Build analysis prompt
      const prompt = customPrompt || this.buildAnalysisPrompt(analysisType, options);
      ...
      if (options.structured) {
        try {
          structuredAnalysis = this.parseStructuredAnalysis(analysisText, analysisType);
        } catch (error) {
          console.warn('[VisionAnalysisService] Failed to parse structured analysis:', error.message);
        }
      }
      ...
      if (options.includeMultiple) {
        for (const type of ['objects', 'text', 'sentiment']) {
          if (type !== analysisType) {
            try {
              additionalAnalyses[type] = await this.performSpecificAnalysis(processedImages, type);
            } catch (error) {
              console.warn(`[VisionAnalysisService] Additional ${type} analysis failed:`, error.message);
            }
          }
        }
      }
      ...
      const result = {
        ...
        metadata: {
          ...
          dimensions: processedImages.map(img => ({ width: img.width, height: img.height }))
        }
      };
      ...
      console.log(`[VisionAnalysisService] ✅ Analysis completed in ${result.processingTime}ms`);
      return result;
      
    } catch (error) {
      console.error('[VisionAnalysisService] ❌ Image analysis failed:', error.message);
      return {
        success: false,
        error: error.message,
  ...
  async generateImage(prompt, options = {}) {
    try {
      ...
      console.log(`[VisionAnalysisService] 🎨 Generating image: "${prompt.substring(0, 50)}..."`);
      ...
      if (options.analyzeGenerated) {
        try {
          ...
          console.warn('[VisionAnalysisService] Failed to analyze generated image:', error.message);
        }
      }
      ...
      console.log(`[VisionAnalysisService] ✅ Image generated in ${result.generation.processingTime}ms`);
      return result;
      
    } catch (error) {
      console.error('[VisionAnalysisService] ❌ Image generation failed:', error.message);
      return {
        success: false,
        error: error.message,
        originalPrompt: prompt
...
Provide a comprehensive comparison analysis.`;
      ...
      console.error('[VisionAnalysisService] ❌ Image comparison failed:', error.message);
      return {
        success: false,
        error: error.message,
...
- Notable formatting: [any special formatting observed]`,
      ...
      console.error('[VisionAnalysisService] ❌ Text extraction failed:', error.message);
      return {
        success: false,
        error: error.message,
  ...
  async preprocessImages(images) {
    ...
    for (let i = 0; i < imageList.length; i++) {
      ...
      try {
        ...
        console.warn(`[VisionAnalysisService] Failed to process image ${i}:`, error.message);
      }
    }
  ...
  async processDataUrl(dataUrl) {
    ...
    if (!mimeMatch || !this.supportedFormats.includes(mimeMatch[1])) {
      throw new Error(`Unsupported image format: ${mimeMatch?.[1] || 'unknown'}`);
    }
  ...
  buildAnalysisPrompt(analysisType, options) {
    const prompts = {
      general: "Analyze this image comprehensively. Describe what you see, including objects, people, settings, actions, and any notable details.",
      objects: "Identify and list all objects visible in this image. Include their positions, sizes, colors, and relationships to each other.",
      text: "Extract and transcribe all text visible in this image. Maintain formatting and structure where possible.",
      faces: "Analyze any faces in this image. Describe apparent emotions, age ranges, and expressions while being respectful of privacy.",
      scene: "Describe the scene, setting, and context of this image. What is happening? Where might this be? What's the mood or atmosphere?",
      sentiment: "Analyze the emotional tone and sentiment of this image. What feelings or emotions does it convey?",
      accessibility: "Provide a detailed accessibility description of this image for visually impaired users. Be comprehensive and clear.",
      safety: "Analyze this image for content safety. Identify any potentially problematic content while being objective.",
      technical: "Provide technical details about this image including apparent quality, lighting, composition, and any technical aspects you can observe."
    };
  ...
  getSystemPrompt(analysisType, language) {
    ...
    const typeSpecific = {
      accessibility: " Focus on creating descriptions that would be helpful for visually impaired users.",
      safety: " Be objective and factual when identifying content. Avoid being overly restrictive.",
      technical: " Use appropriate technical terminology for photography and image analysis."
    };
  ...
  enhanceGenerationPrompt(prompt, options) {
    ...
    if (options.mood && !prompt.toLowerCase().includes(options.mood.toLowerCase())) {
      enhanced += `, ${options.mood} mood`;
    }
    
    if (options.quality === 'hd' && !prompt.toLowerCase().includes('high quality')) {
