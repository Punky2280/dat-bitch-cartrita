...
class KnowledgeHubService {
    ...
    async createEntry(entryData, userId) {
        try {
            ...
            const values = [
                userId, title, content, content_type, category, tags,
                `[${contentEmbedding.join(',')}]`, `[${titleEmbedding.join(',')}]`,
                importance_score, 0.9, entryContentLength, wordCount, 
                sentimentScore, complexityScore, 'user_created', source_url
            ];
            ...
            EmbeddingPipeline.emit('entry-created', { entryId: entry.id, embedding: contentEmbedding });
    ...
    async semanticSearch(query, userId, options = {}) {
        try {
            ...
            const values = [`[${queryEmbedding.join(',')}]`, userId, threshold, limit];
            ...
            const results = result.rows.map(row => ({
                id: row.id,
                title: row.title,
                content: include_content ? row.content : row.content.substring(0, 300) + '...',
    ...
    async generateGraph(userId) {
        try {
            ...
            const [entriesResult, relationshipsResult] = await Promise.all([
                this.pool.query('SELECT id, title, category, content_type, importance_score FROM knowledge_entries WHERE user_id = $1', [userId]),
                this.pool.query(`
            ...
            console.log(`[KnowledgeHub] ✅ Graph generated: ${nodes.length} nodes, ${edges.length} edges`);
    ...
    async performClustering(userId, numClusters = 5) {
        try {
            console.log(`[KnowledgeHub] 🧩 Performing K-means clustering with ${numClusters} clusters...`);
            ...
            const clusters = kmeans(embeddings, numClusters, { initialization: 'kmeans++' });
            ...
            console.log(`[KnowledgeHub] ✅ Clustering completed: ${clusterRecords.length} clusters created`);
    ...
    async getAnalytics(userId, dateRange = 7) {
        try {
            console.log(`[KnowledgeHub] 📊 Generating analytics for ${dateRange} days...`);
            ...
            const [baseResult, clustersResult, relationshipsResult] = await Promise.all([
            ...
            const analytics = {
                ...
                knowledge_density: relationshipsResult.rows[0].total_relationships / Math.max(baseResult.rows[0].total_entries, 1),
                generated_at: new Date().toISOString()
            };

            // Convert string numbers to integers/floats
            Object.keys(analytics).forEach(key => {
                if (key.includes('count') || key === 'total_entries' || key === 'categories_count') {
                    analytics[key] = parseInt(analytics[key]) || 0;
                } else if (key.includes('avg_') || key.includes('score') || key === 'knowledge_density') {
                    analytics[key] = parseFloat(analytics[key]) || 0;
                }
            });

            console.log(`[KnowledgeHub] ✅ Analytics generated: ${analytics.total_entries} entries, ${analytics.total_clusters} clusters`);
    ...
    analyzeSentiment(text) {
        ...
        const positiveWords = ['good', 'great', 'excellent', 'amazing', 'wonderful', 'positive', 'beneficial'];
        const negativeWords = ['bad', 'terrible', 'awful', 'negative', 'problem', 'issue', 'error'];
    ...
    async searchEntries(searchData, userId, sessionId) {
        try {
            const { query, search_type = 'semantic', limit = 20, threshold = 0.7 } = searchData;
            ...
            if (search_type === 'semantic') {
                ...
                const result = await this.pool.query(searchQuery, [userId, `%${query}%`, limit]);
    ...
    async createCluster(clusterData, userId) {
        try {
            const { name, description, cluster_type = 'semantic', color = '#6366f1' } = clusterData;
            ...
            const result = await this.pool.query(query, [
                userId, name, description, JSON.stringify(clusterMetadata)
            ]);
    ...
    async createRelationship(relationshipData) {
        try {
            const { source_entry_id, target_entry_id, relationship_type, strength = 0.5 } = relationshipData;
            ...
            const result = await this.pool.query(query, [
                source_entry_id, target_entry_id, relationship_type, strength
            ]);
