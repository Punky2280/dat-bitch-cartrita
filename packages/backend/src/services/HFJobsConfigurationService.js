...
class HFJobsConfigurationService {
    constructor() {
        this.config = {
            ...
            cloudProcessing: {
                ...
                minFileSizeForCloud: parseInt(process.env.HF_JOBS_MIN_FILE_SIZE) || 10 * 1024 * 1024, // 10MB
                maxLocalFileSize: parseInt(process.env.HF_JOBS_MAX_LOCAL_SIZE) || 50 * 1024 * 1024, // 50MB
    ...
    getAnalysisPreference(analysisType) {
        return this.config.cloudProcessing.analysisPreferences[analysisType] || {
            preferCloud: false,
            flavor: 'cpu-basic',
            maxWaitTime: 300
        };
    ...
    shouldUseCloud(audioInfo, analysisType, options = {}) {
        ...
        if (audioInfo.size >= this.config.cloudProcessing.maxLocalFileSize) return true;
    ...
    getOptimalFlavor(analysisType, priority = 'balanced') {
        ...
        if (priority === 'cost') {
            return analysisType === 'vad' || analysisType === 'osd' ? 'cpu-basic' : 't4-small';
