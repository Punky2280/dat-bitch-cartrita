...
class PersonaMapperService {
  ...
  static normalize(persona) {
    ...
    if (Array.isArray(out.domain_focus)) {
      out.domain_focus = out.domain_focus.map((s) => String(s).slice(0, 40)).slice(0, 5);
    } else {
      out.domain_focus = [];
    }
    if (out.style_notes) out.style_notes = String(out.style_notes).slice(0, 400);
    ...
    const modesIn = out.interaction_modes && typeof out.interaction_modes === 'object' ? out.interaction_modes : {};
    const modes = {};
  ...
  static deriveLLMParams(persona) {
    ...
    const temperature = Number(scale((p.creativity + p.humor) / 2, 0.3, 1.2).toFixed(2));
    const top_p = Number(scale(p.creativity, 0.7, 1.0).toFixed(2));
    const max_output_tokens = Math.round(scale( (10 - p.brevity) + p.analytical_depth, 256, 2048));
    const reasoning_depth = Math.round(scale(p.analytical_depth, 1, 8));
    const suggestion_interval = Math.max(2, 12 - Math.round(scale(p.proactivity, 2, 10))); // lower is more frequent
    const humor_probability = Number(scale(p.humor, 0, 0.45).toFixed(3));
    const empathy_tone = p.empathy >= 7 ? 'high' : p.empathy >= 4 ? 'balanced' : 'minimal';
    const style_directness = p.brevity >= 7 ? 'concise' : p.brevity <= 3 ? 'elaborate' : 'balanced';
    return {
      temperature,
      top_p,
      max_output_tokens,
  ...
  static buildPrompt(personaInput) {
    const { persona, temperature, top_p, max_output_tokens, reasoning_depth } = this.deriveLLMParams(personaInput);
    const modes = Object.entries(persona.interaction_modes).filter(([,v])=>v).map(([k])=>k).join(', ');
    const domains = persona.domain_focus.length ? persona.domain_focus.join(', ') : 'general knowledge';
  ...
  static async getUserPersona(userId) {
    ...
    if (res.rows.length === 0 || !res.rows[0].persona_config) return this.defaultPersona();
  ...
  static async listPresets(userId) {
    ...
    return res.rows.map((r) => ({ ...r, persona_config: this.normalize(r.persona_config) }));
  }
  ...
  static async deletePreset(userId, id) {
    await db.query('DELETE FROM persona_presets WHERE user_id = $1 AND id = $2', [userId, id]);
  }
}
