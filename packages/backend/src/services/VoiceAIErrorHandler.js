...
class VoiceAIErrorHandler {
  constructor() {
    ...
    this.fallbackConfigs = {
      transcription: {
        mockResponse: {
          success: true,
          transcript: 'Voice transcription service is temporarily unavailable. Please check your API configuration.',
          confidence: 0.5,
          metadata: { service: 'fallback', timestamp: new Date().toISOString() }
        }
      },
      synthesis: {
        mockResponse: {
          success: false,
          error: 'Text-to-speech service is temporarily unavailable. Please check your OpenAI API configuration.',
          fallback: 'silent'
        }
      }
    };
  ...
  async initializeDeepgram() {
    try {
      ...
      if (!apiKey) {
        console.warn('⚠️ [VoiceAIErrorHandler] No Deepgram API key found - transcription will use fallback');
        this.isDeepgramReady = false;
        return;
      }
      ...
      console.log('✅ [VoiceAIErrorHandler] Deepgram client initialized successfully');
      
    } catch (error) {
      console.error('❌ [VoiceAIErrorHandler] Failed to initialize Deepgram:', error.message);
      this.isDeepgramReady = false;
  ...
  async initializeOpenAI() {
    try {
      ...
      if (!apiKey) {
        console.warn('⚠️ [VoiceAIErrorHandler] No OpenAI API key found - TTS will use fallback');
        this.isOpenAIReady = false;
        return;
      }
      ...
      console.log('✅ [VoiceAIErrorHandler] OpenAI client initialized successfully');
      
    } catch (error) {
      console.error('❌ [VoiceAIErrorHandler] Failed to initialize OpenAI:', error.message);
      this.isOpenAIReady = false;
  ...
  async transcribeAudio(audioSource, options = {}) {
    ...
    try {
      ...
      if (Buffer.isBuffer(audioSource)) {
        ...
        console.log(`🎤 [VoiceAIErrorHandler] Starting transcription (${audioBuffer.length} bytes)`);
        ...
        if (audioSource.type === 'buffer' && audioSource.buffer) {
          ...
          console.log(`🎤 [VoiceAIErrorHandler] Starting URL transcription: ${audioSource.url}`);
          return this.transcribeFromURL(audioSource.url, options);
        } else {
          throw new Error('Invalid audioSource format');
        }
      } else {
        throw new Error('audioSource must be a Buffer or valid audioSource object');
      }
      
      if (!this.isDeepgramReady) {
        console.log('⚠️ [VoiceAIErrorHandler] Deepgram not ready, using fallback');
      ...
      console.log('🔧 [VoiceAIErrorHandler] Deepgram options:', transcriptionOptions);
      ...
      console.log(`✅ [VoiceAIErrorHandler] Transcription completed in ${processingTime}ms`);
      ...
      if (this.isAuthenticationError(error)) {
        console.log('🔄 [VoiceAIErrorHandler] Authentication error, switching to fallback');
        this.isDeepgramReady = false;
        return this.getTranscriptionFallback(audioBuffer, 'INVALID_AUTH');
      }
      
      if (this.isRateLimitError(error)) {
        console.log('⏳ [VoiceAIErrorHandler] Rate limit error, using fallback');
  ...
  async transcribeFromURL(url, options = {}) {
    try {
      ...
      console.error('❌ [VoiceAIErrorHandler] URL transcription failed:', error);
      return {
        success: false,
        transcript: '',
        confidence: 0,
        error: `URL transcription failed: ${error.message}`,
        metadata: { service: 'error', url, timestamp: new Date().toISOString() }
      };
    }
  }
  ...
  async synthesizeText(text, options = {}) {
    ...
    try {
      console.log(`🗣️ [VoiceAIErrorHandler] Starting TTS synthesis (${text.length} chars)`);
      
      if (!this.isOpenAIReady) {
        console.log('⚠️ [VoiceAIErrorHandler] OpenAI not ready, using fallback');
      ...
      console.log(`✅ [VoiceAIErrorHandler] TTS synthesis completed in ${processingTime}ms`);
      ...
      if (this.isAuthenticationError(error)) {
        console.log('🔄 [VoiceAIErrorHandler] OpenAI authentication error, switching to fallback');
        this.isOpenAIReady = false;
      }
  ...
  extractTranscript(result) {
    try {
      ...
      return resultRoot?.results?.channels?.[0]?.alternatives?.[0]?.transcript || '';
    } catch (error) {
      console.warn('⚠️ [VoiceAIErrorHandler] Failed to extract transcript:', error.message);
      return '';
    }
  }
  ...
  extractConfidence(result) {
    try {
      const resultRoot = result?.result || result;
      return resultRoot?.results?.channels?.[0]?.alternatives?.[0]?.confidence || 0;
    } catch (error) {
      return 0;
    }
  }
  ...
  extractAnalysis(result) {
    try {
      ...
      return {
        ...
        entities: resultRoot?.results?.channels?.[0]?.alternatives?.[0]?.entities || undefined,
      };
    } catch (error) {
      return {};
    }
  ...
  getTranscriptionFallback(audioBuffer, errorType = 'GENERAL') {
    const fallbackMessages = {
      'INVALID_AUTH': 'Deepgram API authentication failed. Please check your API key configuration.',
  ...
  getStatus() {
    return {
      ...
      totalErrors: this.errorCounts.deepgram + this.errorCounts.openai + this.errorCounts.general,
  ...
  calculateHealthScore() {
    ...
    const totalErrors = this.errorCounts.deepgram + this.errorCounts.openai + this.errorCounts.general;
  ...
  logInitializationStatus() {
    ...
    console.log(`   Deepgram: ${status.services.deepgram.ready ? '✅ Ready' : '❌ Not Ready'}`);
    console.log(`   OpenAI: ${status.services.openai.ready ? '✅ Ready' : '❌ Not Ready'}`);
