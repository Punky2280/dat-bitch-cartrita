...
export default class WorkflowAnalyticsService {
  ...
  async initialize() {
    try {
      console.log('[WorkflowAnalyticsService] 🚀 Initializing workflow analytics...');
      ...
      console.log('[WorkflowAnalyticsService] ✅ Analytics service initialized successfully');
      return true;
    } catch (error) {
      console.error('[WorkflowAnalyticsService] ❌ Initialization failed:', error);
      return false;
    }
  }
  ...
  async createAnalyticsViews() {
    ...
    for (const viewSql of views) {
      try {
        ...
        console.warn('[WorkflowAnalyticsService] View creation warning:', error.message);
      }
    }
  }
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async (span) => {
        try {
          ...
          const metricsResult = await this.dbPool.query(metricsQuery, [workflowId]);
          ...
          const trendsResult = await this.dbPool.query(trendsQuery, [workflowId]);
          ...
          const errorsResult = await this.dbPool.query(errorsQuery, [workflowId]);

          span.setAttributes({
            'analytics.total_executions': parseInt(metrics.total_executions) || 0,
          ...
          console.error('[WorkflowAnalyticsService] Analytics query error:', error);
          span.recordException(error);
          throw error;
        }
      }
    ...
    return await OpenTelemetryTracing.traceOperation(
      ...
      async (span) => {
        try {
          ...
          span.setAttributes({
            'system.active_workflows': parseInt(systemMetrics.active_workflows) || 0,
            'system.total_executions': parseInt(systemMetrics.total_executions) || 0,
            'system.success_rate': parseFloat(systemMetrics.system_success_rate) || 0
          });
          ...
          return {
            system_metrics: {
              ...
              successful_executions: parseInt(systemMetrics.successful_executions) || 0,
              failed_executions: parseInt(systemMetrics.failed_executions) || 0,
              success_rate: parseFloat(systemMetrics.system_success_rate) || 0,
              avg_execution_time: parseFloat(systemMetrics.avg_execution_time) || 0
            },
          ...
          console.error('[WorkflowAnalyticsService] System analytics error:', error);
          span.recordException(error);
          throw error;
        }
      }
        ...
        try {
          ...
          if (analytics.metrics.success_rate < 80) {
            recommendations.push({
              ...
              suggestion: 'Review error patterns and implement retry mechanisms'
            });
          }
          ...
          console.error('[WorkflowAnalyticsService] Recommendations error:', error);
          span.recordException(error);
          throw error;
        }
      }
