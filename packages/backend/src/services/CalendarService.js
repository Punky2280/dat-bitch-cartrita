...
class CalendarService {
  ...
  async legacyInitializeForUser(userId) {
    try {
      ...
      if (keyResult.rows.length === 0) {
        throw new Error('Google Calendar API credentials not found. Please connect your Google account.');
      }
    ...
    try {
      ...
      for (const calendar of calendars) {
        try {
          ...
          const eventsSynced = await this.syncCalendarEvents(userId, calendar.id);
  ...
  async suggestMeetingTimes(userId, constraints) {
    try {
      ...
      try {
        const enhancedService = await this.googleAPI.getEnhancedCalendarService(userId);
        const aiSuggestions = await enhancedService.suggestOptimalTimes(constraints);
        
        if (aiSuggestions && aiSuggestions.length > 0) {
          console.log('✨ Using AI-powered time suggestions');
          return aiSuggestions.map(suggestion => ({
            start_time: suggestion.time,
            end_time: suggestion.time, // Will be calculated based on duration
      ...
      for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
        ...
        for (let hour = parseInt(workHours.start); hour < parseInt(workHours.end); hour++) {
   ...
   */
  async getBasicStats(userId, days) {
    const statsQuery = `
      SELECT 
