...
router.get('/messages', authenticateToken, (req, res) => {
  try {
    ...
    const messageHistory = MessageBus.getMessageHistory(limit);
...
router.post('/message', authenticateToken, (req, res) => {
  try {
    ...
    const sent = MessageBus.sendMessage(message);
  ...
  try {
    ...
    const responsePromise = new Promise((resolve, reject) => {
      ...
      const responseHandler = (message) => {
        if (message.metadata?.response_to === queryMessage.id) {
    ...
    // Send the query
    const sent = MessageBus.sendMessage(queryMessage);
    
    if (!sent) {
      return res.status(400).json({
        success: false,
        error: 'Failed to send query message'
      });
    }
  ...
  try {
    ...
    res.status(isHealthy ? 200 : 503).json({
      ...
      data: {
        ...
        message_throughput: status.metrics.messages_per_minute
      },
      timestamp: new Date().toISOString()
    });
  } catch (error) {
