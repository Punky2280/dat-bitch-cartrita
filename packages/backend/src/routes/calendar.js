...
// Get user's calendars (without events)
router.get('/calendars', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;

    const result = await pool.query(`
      SELECT calendar_id, calendar_name, provider, calendar_color, 
             is_primary, access_role, sync_enabled, last_sync_at, created_at
...
router.put('/calendars/:calendarId/sync', authenticateToken, async (req, res) => {
  try {
    ...
    if (typeof sync_enabled !== 'boolean') {
      return res.status(400).json({
        success: false,
        error: 'sync_enabled must be a boolean value'
      });
    }
...
// Get calendar statistics (Enhanced)
router.get('/stats', authenticateToken, async (req, res) => {
  try {
    ...
    if (enhanced === 'true') {
      // Use enhanced analytics with AI insights
      const enhancedStats = await CalendarService.getEnhancedAnalytics(userId, parseInt(days));
...
router.post('/optimize', authenticateToken, async (req, res) => {
  try {
    ...
    if (analysis.ai_analysis?.busy_hours) {
      const busyHours = Object.entries(analysis.ai_analysis.busy_hours)
        .sort(([,a], [,b]) => b - a)
        .slice(0, 3)
    ...
    if (analysis.avg_duration_minutes > 60) {
      suggestions.push({
