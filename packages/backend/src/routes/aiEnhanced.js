...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    const allowedTypes = [
      ...
      'text/plain', 'text/javascript', 'text/python', 'application/json',
 ...
 * @access Private
 */
router.post('/vision-analysis', authenticateToken, upload.single('image'), async (req, res) => {
  if (!req.file) {
    return res.status(400).json({
      success: false,
  ...
  try {
    ...
    switch (task) {
      case 'analyze':
        prompt = 'Analyze this image in detail. Describe what you see, including objects, people, colors, composition, and any notable features.';
        break;
      case 'ocr':
        prompt = 'Extract and transcribe any text visible in this image. Return the exact text content.';
        break;
      case 'objects':
        prompt = 'Identify and list all objects visible in this image with their approximate locations.';
        break;
      case 'emotions':
        prompt = 'Analyze the emotional content of this image. If there are people, describe their emotions and expressions.';
        break;
      case 'accessibility':
        prompt = 'Provide an accessibility description of this image suitable for screen readers and visually impaired users.';
        break;
      default:
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert in sentiment analysis and emotional intelligence. Provide accurate, nuanced analysis.'
        },
        {
          role: 'user',
          content: prompt
 ...
 * @access Private
 */
router.post('/multi-modal-analysis', authenticateToken, upload.fields([
  ...
  try {
    ...
    switch (task) {
      case 'comprehensive':
        analysisPrompt = 'Provide a comprehensive analysis of all the provided content. Look for connections, patterns, and insights across text, images, and documents.';
        break;
      case 'summary':
        analysisPrompt = 'Create a concise summary that captures the key points from all provided content.';
        break;
      case 'insights':
        analysisPrompt = 'Extract key insights and actionable recommendations from the provided content.';
        break;
      case 'comparison':
        analysisPrompt = 'Compare and contrast the different pieces of content, highlighting similarities and differences.';
        break;
      default:
        analysisPrompt = req.body.customPrompt || 'Analyze the provided content.';
    }
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert analyst capable of processing and connecting insights across multiple types of content including text, images, and documents.'
        },
        {
          role: 'user',
          content: [
    ...
    res.json({
      ...
      inputSummary: {
        ...
        totalSizeKB: [...images, ...documents].reduce((sum, file) => sum + file.size, 0) / 1024
      },
      processingTime: Date.now() - req.startTime
    });
