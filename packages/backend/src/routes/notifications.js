...
router.get('/preferences', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      SELECT notification_type, enabled, delivery_method, advance_minutes,
  ...
  try {
    ...
    for (const pref of preferences) {
      ...
      if (pref.delivery_method && !validDeliveryMethods.includes(pref.delivery_method)) {
        return res.status(400).json({
          success: false,
          error: `Invalid delivery method: ${pref.delivery_method}`
        });
      }
    }
    ...
    res.json({
      success: true,
      message: `${updatedPrefs.length} preferences updated`,
...
// Register user for real-time notifications (WebSocket connection)
router.post('/register', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'User registered for real-time notifications'
    });
  } catch (error) {
...
router.post('/unregister', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'User unregistered from real-time notifications'
    });
  } catch (error) {
