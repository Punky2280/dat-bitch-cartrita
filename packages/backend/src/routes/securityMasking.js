...
router.get('/preferences', async (req, res) => {
  try {
    ...
    const preferences = await SecurityMaskingService.getMaskingPreferences(userId);
...
router.put('/preferences', async (req, res) => {
  try {
    ...
    if (preferences.default_visibility && !validVisibility.includes(preferences.default_visibility)) {
    ...
    if (preferences.reveal_timeout && (preferences.reveal_timeout < 10 || preferences.reveal_timeout > 300)) {
      return res.status(400).json({
        success: false,
    ...
    const updatedPreferences = await SecurityMaskingService.updateMaskingPreferences(userId, preferences);
...
router.post('/reveal-token', async (req, res) => {
  try {
    ...
    const tokenData = await SecurityMaskingService.createRevealToken(userId, resource_id, resource_type);
...
router.post('/reveal', async (req, res) => {
  try {
    ...
    const revealData = await SecurityMaskingService.useRevealToken(token, userId);
...
router.post('/mask', async (req, res) => {
  try {
    ...
    const maskedData = await SecurityMaskingService.maskSensitiveData(userId, data, data_type);
...
router.get('/events', async (req, res) => {
  try {
    ...
    const events = await SecurityMaskingService.getSecurityEvents(userId, limit);
