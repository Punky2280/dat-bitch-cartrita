...
router.post('/', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await OpenTelemetryTracing.traceOperation('cartrita.router', { attributes: { 'router.task': task } }, async () => {
    ...
    res.json({ success: true, task, ...result, timing_ms: Date.now() - started });
  } catch (e) {
    const code = e?.statusCode || 500;
    res.status(code).json({ success: false, error: e.message, task, timing_ms: Date.now() - started });
  }
});
