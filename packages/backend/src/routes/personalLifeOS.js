...
journalService.initialize().then(success => {
  ...
  console.error('[PersonalLifeOS] ❌ Journal service integration failed:', error);
});
...
router.get('/', authenticateToken, (req, res) => {
  res.json(ok({ message: 'Personal Life OS root online', services: {
    journal: isJournalServiceReady ? 'ready' : 'initializing'
  }, endpoints: [
    '/google-account/profile','/calendar/upcoming','/email/summary','/contacts/list','/tasks/list',
    '/journal/list','/journal/create','/journal/:id','/journal/:id/derive-tasks','/metrics/activity'
  ] }));
});
...
router.get('/journal/list', authenticateToken, async (req, res) => {
  try {
    if (!isJournalServiceReady) {
      return res.status(503).json({ success: false, error: 'Journal service not ready' });
    }
...
router.post('/journal/create', authenticateToken, async (req, res) => {
  try {
    ...
    if (mood_score !== undefined && (mood_score < 1 || mood_score > 10)) {
      return res.status(400).json({ success: false, error: 'Mood score must be between 1 and 10' });
    }
...
router.post('/journal/:id/derive-tasks', authenticateToken, async (req, res) => {
  try {
    if (!isJournalServiceReady) {
      return res.status(503).json({ success: false, error: 'Journal service not ready' });
    }
    ...
    if (error.message === 'Journal entry not found') {
      ...
      res.status(500).json({ success: false, error: 'Failed to derive tasks from journal entry' });
    }
  }
});
