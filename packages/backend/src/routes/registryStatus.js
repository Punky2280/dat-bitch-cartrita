...
router.get('/status', async (req, res) => {
  try {
    ...
    if (!registry.initialized) {
      process.env.USE_COMPOSITE_REGISTRY = process.env.USE_COMPOSITE_REGISTRY || '1';
...
router.get('/search', async (req, res) => {
  ...
  try {
    ...
    const tools = [...registry.tools.values()].map(t => ({ name: t.name, description: t.description || '', category: t.category || null }));
    ...
    res.json({ success: true, query: q, count: scored.length, results: scored });
  } catch (e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
