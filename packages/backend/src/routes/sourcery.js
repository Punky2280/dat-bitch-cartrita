  ...
  try {
    ...
    if (req.file) {
      try {
        ...
        console.warn('[SourceryRoutes] Failed to cleanup uploaded file:', cleanupError);
      }
    }
    ...
    if (req.file) {
      try {
        ...
        console.warn('[SourceryRoutes] Failed to cleanup uploaded file after error:', cleanupError);
      }
    }
...
router.post('/refactor', upload.single('code'), async (req, res) => {
  try {
    ...
    const result = await sourceryService.applyAutoRefactoring(targetPath, options);
...
router.post('/report', async (req, res) => {
  try {
    ...
    const { projectPath, includeMetrics = true, includeRecommendations = true } = req.body;
...
router.post('/batch', upload.array('files', 10), async (req, res) => {
  try {
    ...
    const analysisPromises = files.map(async (file) => {
      try {
        const result = await sourceryService.analyzeCodeQuality(file.path, options);
        return {
    ...
    if (req.files) {
      req.files.forEach(async (file) => {
        try {
          ...
          console.warn('[SourceryRoutes] Failed to cleanup file after batch error:', file.path);
        }
      });
    }
