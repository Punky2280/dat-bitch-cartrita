...
router.put('/', authenticateToken, async (req, res) => {
  try {
    let saved;
    try {
      saved = await PersonaMapperService.saveUserPersona(req.user.id, req.body || {});
    } catch (inner) {
  if (isLightweight()) {
...
router.post('/presets', authenticateToken, async (req, res) => {
  try {
    ...
    const preset = await PersonaMapperService.createPreset(req.user.id, name.slice(0,60), persona_config || {});
    await db.query(
      `INSERT INTO event_log (event_type, actor_user_id, metadata) VALUES ($1,$2,$3)`,
      ['persona.preset.created', req.user.id, JSON.stringify({ id: preset.id, name })]
    );
