...
export default class HuggingFaceInferenceService {
  constructor() {
    ...
    if (!token) {
      throw new Error('HUGGINGFACE_API_TOKEN is required for HuggingFace operations');
    }
  ...
  initializeTaskModelMap() {
    return {
      // Multimodal
      'audio-text-to-text': ['openai/whisper-large-v3', 'facebook/wav2vec2-large-960h'],
      'image-text-to-text': ['Salesforce/blip-image-captioning-large', 'nlpconnect/vit-gpt2-image-captioning'],
      'visual-question-answering': ['dandelin/vilt-b32-finetuned-vqa', 'microsoft/git-large-vqav2'],
      'document-question-answering': ['impira/layoutlm-document-qa', 'microsoft/layoutlmv3-base'],
      'video-text-to-text': ['MCG-NJU/videomae-base', 'microsoft/xclip-base-patch32'],
      ...
      'image-segmentation': ['facebook/detr-resnet-50-panoptic', 'nvidia/segformer-b0-finetuned-ade-512-512'],
      'text-to-image': ['stabilityai/stable-diffusion-xl-base-1.0', 'runwayml/stable-diffusion-v1-5'],
      'image-to-text': ['Salesforce/blip-image-captioning-base', 'microsoft/git-base-coco'],
      'image-to-image': ['timbrooks/instruct-pix2pix', 'lllyasviel/sd-controlnet-canny'],
      'image-to-video': ['ali-vilab/i2vgen-xl', 'runwayml/stable-video-diffusion-img2vid-xt'],
      ...
      'text-to-video': ['ali-vilab/text-to-video-ms-1.7b', 'cerspense/zeroscope_v2_576w'],
      'zero-shot-image-classification': ['openai/clip-vit-large-patch14', 'laion/CLIP-ViT-B-32-laion2B-s34B-b79K'],
      'mask-generation': ['facebook/sam-vit-huge', 'facebook/sam-vit-base'],
      'zero-shot-object-detection': ['google/owlvit-base-patch32', 'facebook/detr-resnet-50'],
      ...
      'text-classification': ['cardiffnlp/twitter-roberta-base-sentiment-latest', 'microsoft/DialoGPT-medium'],
      'token-classification': ['dbmdz/bert-large-cased-finetuned-conll03-english', 'microsoft/DialoGPT-medium'],
      'table-question-answering': ['google/tapas-large-finetuned-wtq', 'microsoft/tapex-large-finetuned-wtq'],
      'question-answering': ['deepset/roberta-base-squad2', 'microsoft/DialoGPT-medium'],
      'zero-shot-classification': ['facebook/bart-large-mnli', 'microsoft/DialoGPT-medium'],
      ...
      'feature-extraction': ['sentence-transformers/all-MiniLM-L6-v2', 'microsoft/DialoGPT-medium'],
      ...
      'automatic-speech-recognition': ['openai/whisper-large-v3', 'facebook/wav2vec2-large-960h'],
      'audio-to-audio': ['facebook/musicgen-small', 'microsoft/speecht5_vc'],
      'audio-classification': ['MIT/ast-finetuned-audioset-10-10-0.4593', 'facebook/wav2vec2-large-960h'],
  ...
  async visualQuestionAnswering(imageData, question, options = {}) {
    const model = this.getRecommendedModel('visual-question-answering', options);
  ...
  async documentQuestionAnswering(imageData, question, options = {}) {
    const model = this.getRecommendedModel('document-question-answering', options);
  ...
  async zeroShotImageClassification(imageData, labels, options = {}) {
    const model = this.getRecommendedModel('zero-shot-image-classification', options);
  ...
  async automaticSpeechRecognition(audioData, options = {}) {
    const model = this.getRecommendedModel('automatic-speech-recognition', options);
    return await this.hf.automaticSpeechRecognition({
      data: audioData,
      model
    });
