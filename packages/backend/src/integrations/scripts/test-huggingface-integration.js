...
async function runTests() {
  ...
  // Test 2: Capabilities
  results.capabilities = await testEndpoint('Get Capabilities', `${BASE_URL}/capabilities`);
  ...
  results.questionAnswering = await testTextEndpoint(
    ...
    {
      context: 'Paris is the capital and most populous city of France. It is located in northern central France.',
  ...
  for (const [testName, result] of Object.entries(results)) {
    ...
    if (result.success) {
      ...
      if (result.status === 503) {
        console.log(`   Note: Service unavailable - may need HuggingFace API token`);
      }
      if (result.status === 401) {
        console.log(`   Note: Authentication failed - check JWT_TOKEN environment variable`);
      }
    }
  }
  ...
  if (passCount === totalCount) {
    console.log('🎉 All tests passed! HuggingFace integration is working correctly.');
  } else if (passCount > 0) {
