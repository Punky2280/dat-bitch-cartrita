...
export default class HFLanguageAgent {
  ...
  async execute(message, language, userId) {
      ...
      if (stored) {
        if (stored.userId && userId && String(stored.userId) !== String(userId)) {
          ...
          if (global.otelCounters?.hfTokenMisuse) {
            try { global.otelCounters.hfTokenMisuse.add(1, { modality: 'language' }); } catch(_) {}
          }
    ...
    return await OpenTelemetryTracing.traceAgentOperation('LanguageMaestro','bridge.execute',{ 'hf.modality':'language','user.id':userId || 'anonymous' }, async () => {
      let structured = { task: 'language.generic', status: 'ok', data: {} };
      try {
        if (lower.includes('summarize')) {
          const res = await this.hfAgent.summarizeText(message);
          structured = { task: 'language.summarization', status: 'ok', data: res };
        } else if (lower.includes('translate')) {
          const res = await this.hfAgent.translateText(message, { targetLang: 'en' });
          structured = { task: 'language.translation', status: 'ok', data: res };
        } else if (lower.includes('classify') || lower.includes('sentiment')) {
          const res = await this.hfAgent.classifyText(message);
          structured = { task: 'language.classification', status: 'ok', data: res };
          ...
          structured = { task: 'language.generation', status: 'ok', data: res };
        } else {
          structured = { task: 'language.response', status: 'ok', data: { response: this.hfAgent.generateResponse(message) } };
        }
      } catch (e) {
        structured = { task: 'language.error', status: 'error', error: e.message };
      }
      const summary = structured.status === 'error' ? `Language error: ${structured.error}` : `Language ${structured.task} complete.`;
      return {
        messages: [ { type: 'ai', content: summary, structured } ],
        next_agent: 'cartrita',
        meta: { source: 'huggingface', modality: 'language', task: structured.task, status: structured.status }
      };
    });
  }
}
