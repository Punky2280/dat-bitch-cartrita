...
        // Emitting a specific event for each agent is not standard for broadcasts
        // The general 'mcp:broadcast' is usually sufficient.
        // this.emit(`mcp:message:${agentId}`, message);
        delivered++;
      }
    }
    
    // Also emit general broadcast event for agents that listen to all broadcasts
...
      // Update MessageBus.system's own heartbeat
      this.heartbeat('MessageBus.system');
...
  checkAgentHealth() {
    const now = Date.now();
    const staleThreshold = this.heartbeatInterval * 4; // 120 seconds (4x heartbeat interval)
    
    for (const [agentId, agent] of this.agents) {
...
        agent.status = 'stale';
        
        // FIX: Construct a complete and valid payload for the alert.
        // This ensures the receiving agent doesn't crash on an undefined `details` object.
        this.broadcast(new MCPMessage({
          type: 'SYSTEM_ALERT',
          sender: 'MessageBus.system',
          payload: {
            alert_type: 'agent_stale',
            severity: 'warning',
            details: {
                message: `Agent ${agentId} has not sent a heartbeat in ${Math.round(age / 1000)} seconds.`,
                agentId: agentId,
...
      },
      // Note: this.listenerCount() is a method on the EventEmitter prototype itself.
      listeners: {
        total: this.listenerCount('mcp:broadcast') + this.listenerCount('mcp:message')
      }
    };
  }
