...
class EnhancedOpenTelemetryService {
    ...
    async initialize(options = {}) {
        ...
        try {
            console.log('[EnhancedOpenTelemetryService] 🚀 Initializing Enhanced OpenTelemetry Service with upstream integration...');
            ...
            if (process.env.NODE_ENV === 'production' || process.env.ENABLE_PROMETHEUS === 'true') {
            ...
            console.log('[EnhancedOpenTelemetryService] ✅ Enhanced OpenTelemetry Service initialized successfully');
            ...
            console.error('[EnhancedOpenTelemetryService] ❌ Failed to initialize:', error);
            throw error;
        }
    }
    ...
    async initializeDatabase() {
        try {
            ...
            client.release();
            console.log('[OpenTelemetry] ✅ Database connection for telemetry initialized');
        } catch (error) {
            console.error('[OpenTelemetry] ❌ Failed to initialize database connection:', error);
        }
    }

    initializeEnhancedCustomMetrics() {
        // Original metrics
        this.metrics.httpRequests = this.meter.createCounter('cartrita_http_requests_total', {
            description: 'Total number of HTTP requests',
        });

        this.metrics.dbQueries = this.meter.createCounter('cartrita_db_queries_total', {
            description: 'Total number of database queries',
        });

        this.metrics.agentInteractions = this.meter.createCounter('cartrita_agent_interactions_total', {
            description: 'Total number of agent interactions',
        });

        this.metrics.workflowExecutions = this.meter.createCounter('cartrita_workflow_executions_total', {
            description: 'Total number of workflow executions',
        });

        this.metrics.errorCounts = this.meter.createCounter('cartrita_errors_total', {
            description: 'Total number of errors',
        });

        this.metrics.responseTime = this.meter.createHistogram('cartrita_response_time_seconds', {
            description: 'Response time in seconds',
            boundaries: [0.1, 0.5, 1.0, 2.0, 5.0, 10.0]
        });

        this.metrics.systemHealth = this.meter.createObservableGauge('cartrita_system_health_score', {
        ...
        this.metrics.agentOperations = {
            ...
            duration: this.meter.createHistogram('cartrita_agent_operations_duration_ms', {
                description: 'Agent operation duration in milliseconds',
                boundaries: [10, 50, 100, 500, 1000, 5000, 10000]
            }),
            errors: this.meter.createCounter('cartrita_agent_operations_errors_total', {
        ...
        this.metrics.mcpCommunications = {
            ...
            protocolErrors: this.meter.createCounter('cartrita_mcp_protocol_errors_total', {
        ...
        this.metrics.multiModalProcessing = {
            ...
            processingTime: this.meter.createHistogram('cartrita_multimodal_processing_duration_ms', {
                description: 'Multi-modal processing duration in milliseconds',
                boundaries: [100, 500, 1000, 5000, 10000, 30000]
            }),
            modalityTypes: this.meter.createCounter('cartrita_multimodal_modality_types_total', {
        ...
        this.metrics.userInteractions = {
            ...
            responseTime: this.meter.createHistogram('cartrita_user_response_duration_ms', {
                description: 'User response time distribution in milliseconds',
                boundaries: [100, 500, 1000, 2000, 5000, 10000]
            }),
            satisfaction: this.meter.createHistogram('cartrita_user_satisfaction_score', {
        ...
        console.log('[EnhancedOpenTelemetryService] ✅ Enhanced custom metrics initialized');
            ...
            try {
                ...
                if (this.dbPool) {
                    await this.dbPool.query(
                        ...
                        [
                            'opentelemetry',
                            healthScore > 80 ? 'healthy' : healthScore > 50 ? 'warning' : 'error',
    ...
    async traceEnhancedAgentOperation(agentName, operation, attributes = {}, handler) {
        const startTime = Date.now();
        ...
        try {
            ...
            this.recordEnhancedAgentMetrics(agentName, operation, Date.now() - startTime, true);
            
            return result;
        } catch (error) {
            // Record error metrics
            this.recordEnhancedAgentMetrics(agentName, operation, Date.now() - startTime, false, error);
            throw error;
        }
    }

    async traceMCPCommunication(messageType, direction, attributes = {}, handler) {
        const startTime = Date.now();
        ...
        try {
            const result = await this.baseTracing.traceOperation(spanName, {
    ...
    async traceMultiModalProcessing(modalityType, processingType, attributes = {}, handler) {
        const startTime = Date.now();
        ...
        try {
            ...
            if (this.metrics.multiModalProcessing?.processingTime) {
                this.metrics.multiModalProcessing.processingTime.record(Date.now() - startTime, {
                    'modality': modalityType,
                    'processing_type': processingType
                });
            }
            ...
            console.error(`[EnhancedOpenTelemetryService] Multi-modal processing error:`, error);
            throw error;
        }
    }

    recordEnhancedAgentMetrics(agentName, operation, duration, success, error = null) {
        try {
            ...
            console.warn('[EnhancedOpenTelemetryService] Failed to record enhanced agent metrics:', metricsError);
        }
    }
    ...
    async shutdown() {
        try {
            console.log('[EnhancedOpenTelemetryService] 🔽 Shutting down enhanced observability...');
            ...
            if (this.sdk) {
                ...
                console.log('[EnhancedOpenTelemetryService] ✅ Enhanced SDK shutdown completed');
            }
            
            // Close Prometheus exporter
            if (this.prometheusExporter) {
                await this.prometheusExporter.shutdown();
                console.log('[EnhancedOpenTelemetryService] ✅ Prometheus exporter shutdown completed');
            }
            ...
            this.isInitialized = false;
            console.log('[EnhancedOpenTelemetryService] ✅ Enhanced observability shutdown complete');
        } catch (error) {
