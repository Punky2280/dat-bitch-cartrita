...
async function authenticateApiKey(req, res, next) {
  try {
    ...
    if (!apiKey && req.headers['x-api-key'] && req.headers['x-api-key'].startsWith(API_KEY_PREFIX)) {
    ...
    if (!apiKey) {
      return res.status(401).json({ 
        ...
        message: 'Provide API key in Authorization header as "Bearer cartrita_..." or x-api-key header'
      });
    }
    ...
    console.log(`[APIKeyAuth] ✅ Authenticated via API key: ${keyData.name} (User: ${keyData.user_name})`);
    next();
