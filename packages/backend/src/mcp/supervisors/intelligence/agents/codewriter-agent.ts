...
const { CodeWriterAgent: BackendCodeWriterAgent } = await import('../../../../backend/src/agi/agents/CodeWriterAgent.js');
...
export class CodeWriterAgent {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Failed to initialize Code Writer Agent', error as Error);
      throw error;
    }
  }
    ...
    try {
      ...
      switch (request.taskType) {
        ...
        case 'codewriter.document.code':
          ...
          break;
        default:
          throw new Error(`Unsupported code writer task type: ${request.taskType}`);
      }
  ...
  private async generateFunction(parameters: any): Promise<any> {
    const { description, language = 'javascript', parameters: funcParams = [], returnType } = parameters;
  ...
  private async generateClass(parameters: any): Promise<any> {
    const { description, language = 'javascript', methods = [], properties = [] } = parameters;
  ...
  private async refactorCode(parameters: any): Promise<any> {
    const { code, language, refactorType = 'improve', guidelines = [] } = parameters;
  ...
  private async optimizePerformance(parameters: any): Promise<any> {
    const { code, language, optimizationTarget = 'speed', constraints = [] } = parameters;
  ...
  private async addTests(parameters: any): Promise<any> {
    const { code, language, testFramework = 'jest', coverage = 'basic' } = parameters;
  ...
  private async documentCode(parameters: any): Promise<any> {
    const { code, language, docStyle = 'JSDoc', includeExamples = true } = parameters;
