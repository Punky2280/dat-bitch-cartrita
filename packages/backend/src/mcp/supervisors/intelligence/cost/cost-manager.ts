...
export class CostManager {
  ...
  calculateCost(model: string, inputTokens: number, outputTokens: number): number {
    const modelCost = this.modelCosts[model as keyof typeof this.modelCosts] || this.modelCosts['gpt-4o'];
   ...
   * Record cost
   */
  async recordCost(taskId: string, model: string, cost: number, tokens: { input: number; output: number }): Promise<void> {
