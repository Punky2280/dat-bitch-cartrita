...
import { CostContext, ModelRegistryEntry, CostEvent, BudgetTracking } from './types';
...
export interface CostOptimizationSuggestion {
  type: 'model_switch' | 'quantization' | 'batch_processing' | 'caching';
...
export class CostEstimator extends EventEmitter {
  private costCache = new Map<string, { cost: number; timestamp: Date }>();
  private readonly CACHE_TTL_MS = 300000; // 5 minutes

  constructor(
    private logger: any = console,
    private redisClient?: any // Optional Redis cache
  ) {
    super();
  }
  ...
  async estimateCost(
    ...
    } else if (context.hardware_hourly_usd && context.measured_tokens_per_second) {
   ...
   */
  private estimateFromCostProfile(
    context: CostContext,
    model: ModelRegistryEntry
    ...
    if (profile.moe_efficiency_factor && model.architectural_type === 'mixture-of-experts') {
   ...
   */
  private estimateFromHardwareMetrics(context: CostContext): CostEstimationResult {
   ...
   */
  private estimateFromFallbackMethod(context: CostContext): CostEstimationResult {
  ...
  async estimateBatchCost(
    requests: CostContext[],
    model?: ModelRegistryEntry
  ): Promise<{
    individual_costs: CostEstimationResult[];
    batch_cost: CostEstimationResult;
    batch_discount_percent: number;
  }> {
    const individualCosts = await Promise.all(
      requests.map(context => this.estimateCost(context, model))
    );

    const totalIndividualCost = individualCosts.reduce(
      (sum, cost) => sum + cost.cost_usd, 0
    );
    ...
    const totalTokens = requests.reduce(
      (sum, req) => sum + req.tokens_in + req.tokens_out, 0
    );

    const batchResult: CostEstimationResult = {
      cost_usd: batchCost,
  ...
  async generateOptimizationSuggestions(
    ...
    const alternatives = availableModels
      .filter(model => 
        model.model_id !== currentModel && 
        model.status === 'active' &&
        model.cost_profile.estimated_cost_per_1k_tokens_usd < currentCostPer1k
      )
      .sort((a, b) => 
        a.cost_profile.estimated_cost_per_1k_tokens_usd - 
        b.cost_profile.estimated_cost_per_1k_tokens_usd
      );

    for (const alt of alternatives.slice(0, 3)) { // Top 3 alternatives
      const savingsPercent = ((currentCostPer1k - alt.cost_profile.estimated_cost_per_1k_tokens_usd) / currentCostPer1k) * 100;
   ...
   * Cache management
   */
  private generateCacheKey(context: CostContext): string {
    return `cost:${context.model_id}:${context.tokens_in}:${context.tokens_out}:${context.hardware_hourly_usd || 'default'}`;
  }

  private getCachedCost(key: string): CostEstimationResult | null {
    const cached = this.costCache.get(key);
    if (cached && Date.now() - cached.timestamp.getTime() < this.CACHE_TTL_MS) {
      return cached.cost as any; // Type assertion for simplicity
    }
    this.costCache.delete(key);
    return null;
  }

  private setCachedCost(key: string, result: CostEstimationResult): void {
   ...
   * Helper methods
   */
  private calculateTimeRemaining(budget: BudgetTracking, currentSpend: number): string {
    // Simple linear projection
    const hoursInPeriod = budget.budget_type === 'daily' ? 24 : 
                         budget.budget_type === 'weekly' ? 168 : 
                         budget.budget_type === 'monthly' ? 720 : 1;
  ...
  private projectBudgetExhaustion(budget: BudgetTracking, currentSpend: number): Date {
  ...
  private analyzeTradeOffs(currentModel: string, alternativeModel: ModelRegistryEntry): string[] {
    const tradeOffs: string[] = [];
    
    if (alternativeModel.parameters_billion && alternativeModel.parameters_billion < 30) {
  ...
  private calculateConfidence(model: ModelRegistryEntry): number {
    let confidence = 0.5; // Base confidence
    
    if (model.last_benchmark && 
        Date.now() - new Date(model.last_benchmark).getTime() < 7 * 24 * 60 * 60 * 1000) {
      confidence += 0.3; // Recent benchmark data
    }
    
    if (model.quality_metrics.composite_score && model.quality_metrics.composite_score > 0.7) {
