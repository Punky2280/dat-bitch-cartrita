...
export interface SafetyAction {
  action: 'allow' | 'block' | 'redact' | 'regenerate' | 'human_review' | 'warning';
  confidence: number;
  reason: string;
...
export class SafetyPipeline extends EventEmitter {
   ...
   */
  private createPassthroughResult(request: SafetyRequest, reason: string): SafetyResult {
    return {
      request_id: request.id,
      is_safe: true,
      risk_score: 0,
  ...
  getStatistics(timeRangeHours: number = 24): {
    ...
    const recentResults = this.auditLog.filter(result => 
      Date.now() - new Date(result.request_id).getTime() < cutoff
    );
