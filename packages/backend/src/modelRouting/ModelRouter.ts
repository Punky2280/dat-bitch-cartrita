...
export class ModelRouter extends EventEmitter {
  ...
  private loadBalancer = new Map<string, number>(); // Track model load
  ...
  private routingStrategies: Map<string, RoutingStrategy> = new Map([
    ['quality_first', {
      ...
      weights: { quality: 0.6, cost: 0.1, latency: 0.1, safety: 0.15, availability: 0.05 },
    ...
    ['cost_optimized', {
      ...
      weights: { quality: 0.2, cost: 0.5, latency: 0.1, safety: 0.15, availability: 0.05 },
      fallback_enabled: true,
      budget_aware: true
    }],
    ['balanced', {
      ...
      weights: { quality: 0.35, cost: 0.25, latency: 0.25, safety: 0.1, availability: 0.05 },
      fallback_enabled: true,
      budget_aware: true
    }],
    ['speed_first', {
      ...
      weights: { quality: 0.2, cost: 0.1, latency: 0.5, safety: 0.15, availability: 0.05 },
      fallback_enabled: true,
      budget_aware: false
    }],
    ['safety_critical', {
      ...
      weights: { quality: 0.25, cost: 0.1, latency: 0.15, safety: 0.45, availability: 0.05 },
  ...
  async selectModel(
    ...
    try {
      ...
      const rankedModels = budgetFilteredModels.sort(
      ...
      const finalSelection = await this.checkAvailabilityAndBalance(
        selectedModel, 
        rankedModels.slice(1, 4), // Top 3 alternatives
        context
      );
  ...
  private async getCandidateModels(
    ...
    return allModels.filter(model => {
      ...
      if (!model.task_types.includes(criteria.task_type)) return false;
      if (criteria.commercial_use && !model.commercial_allowed) return false;
      ...
      if (criteria.max_cost_per_1k_tokens && 
          model.cost_profile.estimated_cost_per_1k_tokens_usd > criteria.max_cost_per_1k_tokens) {
        return false;
      }
      ...
      // Safety filter
      if (criteria.safety_required && model.safety_risk_level === 'high') return false;
      ...
      if (context.safety_requirements?.max_risk_level) {
        ...
        if (modelRiskIndex > maxRiskIndex) return false;
      }
  ...
  private async scoreModels(
    ...
    for (const model of candidates) {
      ...
      const compositeScore = 
        scores.quality_score * strategy.weights.quality +
  ...
  private calculateSafetyScore(
    ...
    if (model.routing_tags.includes('safety')) baseScore += 0.1;
    if (model.routing_tags.includes('moderated')) baseScore += 0.1;
   ...
   */
  private async calculateAvailabilityScore(model: ModelRegistryEntry): Promise<number> {
    // Check current load
   ...
   */
  private async checkAvailabilityAndBalance(
    primaryChoice: ScoredModel,
    ...
    if (currentLoad > loadThreshold && alternatives.length > 0) {
      ...
      const availableAlternative = alternatives
        .sort((a, b) => {
   ...
   */
  registerModel(model: ModelRegistryEntry): void {
   ...
   */
  updateModelMetrics(modelId: string, metrics: any): void {
