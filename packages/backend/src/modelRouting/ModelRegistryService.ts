...
export class ModelRegistryService extends EventEmitter {
  ...
  constructor(
    ...
    this.benchmarkHarness = new BenchmarkHarness(
      modelInferenceService,
      config.monitoring.metricsCollector,
      logger
    );
    ...
    this.eventEmitter = new ModelRegistryEventEmitter(
      logger,
  ...
  async selectModel(
    ...
    const result = await this.router.selectModel(
      criteria,
      {
        ...context,
   ...
   */
  async runBenchmarks(config: BenchmarkConfig): Promise<void> {
    this.ensureInitialized();
   ...
   */
  async registerModel(model: ModelRegistryEntry): Promise<APIResponse<ModelRegistryEntry>> {
    this.ensureInitialized();
    ...
    try {
      ...
      await this.config.database.client.query(query, [
        model.model_id, model.provider, JSON.stringify(model.task_types),
        model.family, model.parameters_billion, model.architectural_type,
        model.license, model.commercial_allowed, model.context_length,
        JSON.stringify(model.quantizations), JSON.stringify(model.throughput_metadata),
        model.avg_latency_ms_512_tokens, JSON.stringify(model.quality_metrics),
        model.safety_risk_level, model.default_temperature,
        JSON.stringify(model.cost_profile), JSON.stringify(model.routing_tags),
        JSON.stringify(model.fallback_chain), model.status,
        model.last_benchmark, model.risk_notes, JSON.stringify(model.metadata || {}),
        // For conflict update
        model.provider, JSON.stringify(model.task_types), model.status
      ]);
      ...
      console.error('Failed to register model', { model_id: model.model_id, error });
  ...
  async getCostStatistics(
    ...
    top_models: Array<{ model_id: string; cost_usd: number; calls: number }>;
  }>> {
    this.ensureInitialized();
    ...
    try {
      ...
      const stats = {
        ...
        top_models: [] as Array<{ model_id: string; cost_usd: number; calls: number }>
      };
      ...
      stats.avg_cost_per_1k_tokens = stats.total_tokens > 0 ? 
        (stats.total_cost_usd / stats.total_tokens) * 1000 : 0;
  ...
  private async scheduleBenchmarking(): Promise<void> {
  ...
  private async storeBenchmarkResults(results: any[]): Promise<void> {
  ...
  private async estimateBasicCost(taskType: TaskType, tokens: number): Promise<number> {
