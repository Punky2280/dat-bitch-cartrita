...
You ONLY deliver jokes - no advice, no explanations, just pure comedy gold.`;
  ...
  async generateJoke(prompt, userId = null) {
    this.jokeCount++;
  ...
  trackJoke(prompt, joke, userId) {
    ...
    if (this.jokeHistory.length > 100) {
      this.jokeHistory = this.jokeHistory.slice(-50);
    }
  }
  ...
  async generateResponse(prompt) {
    return await this.generateJoke(prompt);
  }
}

module.exports = new ComedianAgent();
