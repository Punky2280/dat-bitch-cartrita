...
drive real business growth and competitive advantage with data-backed confidence.`,
        ...
        this.strategicFrameworks = {
            ...
            lean_canvas: {
                ...
                components: ['Problem', 'Solution', 'Key Metrics', 'Unique Value Proposition', 'Unfair Advantage'],
        ...
        this.businessMetrics = {
            financial: ['Revenue Growth', 'Profit Margins', 'ROI', 'Cash Flow', 'EBITDA', 'Customer LTV'],
            operational: ['Market Share', 'Customer Acquisition Cost', 'Churn Rate', 'Conversion Rate'],
            strategic: ['Brand Recognition', 'Innovation Index', 'Competitive Position', 'Market Timing']
        };
    ...
    async invoke(state) {
        ...
        try {
            ...
            const businessAnalysis = await this.analyzeBusinessRequest(businessRequest, state);
            
            // Determine strategic approach
            const strategicApproach = this.determineStrategicApproach(businessAnalysis);
            
            // Generate business strategy response
            let response = await this.generateBusinessResponse(businessAnalysis, strategicApproach, state);
            
            // Apply strategic frameworks and business intelligence
            response = await this.enhanceWithStrategicFrameworks(response, businessAnalysis);
            
            // Add business personality with commercial acumen
            response = this.enhanceWithBusinessPersonality(response, businessAnalysis);
            
            // Execute business analysis tools
            const toolResults = await this.executeBusinessTools(businessAnalysis, state);
    ...
    async analyzeBusinessRequest(request, state) {
        ...
        analysis.recommendedFrameworks = this.recommendStrategicFrameworks(analysis);
    ...
    generateMarketIntelligence(analysis) {
        ...
        if (analysis.marketFactors.includes('technology_trends')) {
            intelligence.market_trends.push('AI and automation driving market transformation');
            intelligence.opportunity_assessment.tech_adoption = 'High potential for early adopters';
        }

        if (analysis.marketFactors.includes('consumer_behavior')) {
            intelligence.market_trends.push('Shift towards digital-first customer experiences');
            intelligence.opportunity_assessment.customer_experience = 'Differentiation through superior UX';
        }
    ...
    createBusinessIntroduction(analysis) {
        const intros = {
            market_analysis: "Market intelligence mode activated! Let me break down the competitive landscape and opportunities:",
            financial_planning: "Financial strategy time! I'm diving into the numbers to optimize your revenue and profitability:",
            growth_strategy: "Growth acceleration incoming! Here's how we scale this business systematically:",
            customer_strategy: "Customer-centric strategy development! Let's maximize lifetime value and retention:",
            pricing_strategy: "Pricing optimization expertise deployed! Time to maximize revenue per customer:",
            default: "Strategic business analysis activated! Here's how we drive sustainable competitive advantage:"
        };
    ...
    async enhanceWithStrategicFrameworks(response, analysis) {
        ...
        response += "\n\n**Next Steps:**\n";
        response += "Strategy development ready! I can create detailed business plans, financial models, competitive analyses, or implementation roadmaps - whatever drives your business forward faster! 📈";
    ...
    async executeBusinessTools(analysis, state) {
        ...
        if (analysis.requestType === 'market_analysis') {
            toolResults.toolsUsed.push('market_researcher', 'competitor_analyzer', 'industry_analyst');
        } else if (analysis.requestType === 'financial_planning') {
            toolResults.toolsUsed.push('financial_modeler', 'revenue_forecaster', 'roi_calculator');
        } else if (analysis.requestType === 'growth_strategy') {
            toolResults.toolsUsed.push('strategy_planner', 'opportunity_evaluator', 'scenario_planner');
        }
