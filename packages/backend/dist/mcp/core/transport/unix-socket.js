...
export class MCPUnixSocketTransport extends EventEmitter {
    ...
    metrics;
    options;
    tracer = trace.getTracer('mcp-unix-socket-transport');
    server = null;
    clientSocket = null;
    connections = new Map();
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                this.server.listen(this.options.socketPath, () => {
                    this.logger.info('Server listening', { socketPath: this.options.socketPath });
                    resolve();
                });
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                ...
                this.clientSocket.on('connect', () => {
                    ...
                    this.logger.info('Connected to server', { socketPath: this.options.socketPath });
                    resolve();
                });
    ...
    async sendMessage(message, connectionId) {
        ...
        try {
            ...
            if (this.options.enableMetrics) {
                ...
                this.metrics.recordTransportBytesTransferred(messageBuffer.length, 'unix-socket', 'sent');
            }
            span.setStatus({ code: SpanStatusCode.OK });
        }
        catch (error) {
     ...
     */
    async sendTaskRequest(request, recipientId, senderId, timeoutMs = 30000, connectionId) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const responseHandler = (receivedMessage) => {
                if (receivedMessage.messageType === MessageTypes.TASK_RESPONSE &&
    ...
    async shutdown() {
        ...
        for (const [id, conn] of this.connections) {
            try {
                ...
                this.logger.warn('Error closing connection', { connectionId: id, error });
            }
        }
        ...
        socket.on('data', (data) => {
            ...
            while (buffer.length >= 4) {
                ...
                if (frameLength > this.options.maxFrameSize) {
                    this.logger.error('Frame too large', { frameLength, maxFrameSize: this.options.maxFrameSize });
                    socket.destroy();
                    return;
                }
                if (buffer.length >= frameLength + 4) {
                    ...
                    try {
                        ...
                        this.logger.error('Failed to parse message frame', error);
    ...
    handleReceivedMessage(message, connectionId) {
    ...
    deserializeMessage(data) {
        try {
            ...
            // Validate if enabled
            if (this.options.enableValidation) {
                const validation = MCPValidator.safeValidate(MCPValidator.validateMessage, unpacked);
                if (!validation.success) {
                    throw new Error(`Message validation failed: ${validation.errors.join(', ')}`);
                }
    ...
    async sendToConnection(connectionId, data) {
        ...
        // Check backpressure
        if (this.options.enableBackpressure && connection.messageQueue.length >= this.options.maxQueueSize) {
        ...
        const promises = Array.from(this.connections.entries()).map(([id, conn]) => {
            if (conn.socket.writable) {
                return this.sendToConnection(id, data).catch((error) => {
                    this.logger.warn('Broadcast failed for connection', { connectionId: id, error });
                });
            }
