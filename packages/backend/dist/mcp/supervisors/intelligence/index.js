...
export class IntelligenceSupervisor extends EventEmitter {
    ...
    modelCache;
    taskRouter;
    tracer = trace.getTracer('intelligence-supervisor');
    metrics = getMetrics();
    isInitialized = false;
    activeTasks = new Map();
    constructor(config = {}) {
    ...
    async initialize() {
        ...
        try {
            ...
            this.logger.error('Failed to initialize Intelligence Supervisor', error);
            throw error;
        }
        finally {
            span.end();
    ...
    async handleTaskRequest(message) {
        ...
        try {
            ...
            this.metrics.recordTaskFailed(taskRequest.taskType, this.config.supervisorId, (Date.now() - (this.activeTasks.get(taskRequest.taskId)?.startTime || Date.now())) / 1000, 'SUPERVISOR_ERROR');
            span.recordException(error);
            this.logger.error('Task execution failed', error, { taskId: taskRequest.taskId });
        }
        finally {
