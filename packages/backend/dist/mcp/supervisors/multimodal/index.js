...
import { MCPInProcessTransport, Logger, TaskStatus, MessageTypes, TaskTypes, MCPValidator, getMetrics, createPerformanceTimer } from '../../core/index.js';
...
export class MultiModalSupervisor extends EventEmitter {
    ...
    async handleMessage(message) {
        ...
        try {
            ...
            span.recordException(error);
            this.logger.error('Error handling message', error, { messageId: message.id });
        }
        finally {
            span.end();
        }
    ...
    async handleTaskRequest(message) {
        ...
        try {
            ...
            timer.complete();
            this.metrics.recordTaskCompleted(taskRequest.taskType, this.config.supervisorId, (Date.now() - execution.startTime) / 1000);
            ...
            span.recordException(error);
            this.logger.error('Multi-modal task execution failed', error, { taskId: taskRequest.taskId });
        }
        finally {
