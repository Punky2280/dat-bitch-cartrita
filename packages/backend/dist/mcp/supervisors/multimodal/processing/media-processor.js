...
export class MediaProcessor {
    ...
    supportedVideoFormats = ['mp4', 'avi', 'mov', 'webm', 'mkv'];
    ...
    async process(parameters) {
        ...
        switch (parameters.mediaType) {
            ...
            default:
                this.logger.warn('Unknown media type', { mediaType: parameters.mediaType });
                return parameters;
        }
    }
    /**
     ...
     * Validate media file
     */
    async validateMedia(mediaType, data) {
        const errors = [];
        // Check file size
        ...
        if (supportedFormats.length > 0) {
            ...
            switch (mediaType) {
                case 'audio':
                    isSupported = supportedFormats.some(fmt => this.supportedAudioFormats.includes(fmt));
                    break;
                case 'video':
                    isSupported = supportedFormats.some(fmt => this.supportedVideoFormats.includes(fmt));
                    break;
                case 'image':
                    isSupported = supportedFormats.some(fmt => this.supportedImageFormats.includes(fmt));
                    break;
            }
            if (!isSupported) {
