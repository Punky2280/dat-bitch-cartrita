...
export class SafetyPipeline extends EventEmitter {
    ...
    async evaluatePrompt(request) {
        ...
        try {
            const result = await this.runSafetyEvaluation(request, 'pre_generation', this.config.pre_generation.classifier_model);
            ...
            this.logger.error('Prompt evaluation failed', { request_id: request.id, error });
    ...
    async evaluateGeneration(request) {
        ...
        this.logger.debug('Evaluating generation safety', { request_id: request.id });
        try {
            const result = await this.runSafetyEvaluation(request, 'post_generation', this.config.post_generation.safety_model);
            ...
            if (this.config.post_generation.redaction_enabled &&
                result.risk_score > this.config.post_generation.risk_threshold) {
                const redactedContent = this.redactUnsafeContent(request.content, result);
            ...
            // Audit logging
            if (this.config.audit.log_all_interactions ||
            ...
            if (this.config.audit.human_review_threshold &&
            ...
            this.logger.error('Generation evaluation failed', { request_id: request.id, error });
            return {
                request_id: request.id,
                is_safe: false,
                risk_score: 1.0,
    ...
    async runSafetyEvaluation(request, phase, modelName) {
        ...
        const enabledCategories = Array.from(this.categoryDefinitions.entries())
            ...
            .map(([name, _]) => name);
        const detectedCategories = evaluation.categories.filter((cat) => enabledCategories.includes(cat));
        // Calculate overall risk score
        const riskScore = Math.max(evaluation.overall_risk || 0, ...detectedCategories.map((cat) => {
            const category = this.categoryDefinitions.get(cat);
            return category ? evaluation.category_scores[cat] || 0 : 0;
        }));
        // Determine if content is safe
        const threshold = phase === 'pre_generation'
    ...
    generateSafetyActions(result, phase) {
        ...
        if (phase === 'pre_generation') {
            ...
            if (this.config.post_generation.redaction_enabled && result.risk_score > 0.6) {
                actions.push({
                    action: 'redact',
                    confidence: result.confidence,
            ...
            else if (this.config.post_generation.regeneration_enabled && result.risk_score > 0.7) {
                actions.push({
                    action: 'regenerate',
                    confidence: result.confidence,
    ...
    redactUnsafeContent(content, result) {
        ...
        if (result.categories.includes('profanity')) {
            ...
            redactedContent = redactedContent.replace(/\b(fuck|shit|damn|hell|bitch|asshole)\b/gi, '[REDACTED]');
        }
        if (result.categories.includes('privacy_violation')) {
            // Redact email addresses and phone numbers
            redactedContent = redactedContent.replace(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g, '[EMAIL_REDACTED]');
            redactedContent = redactedContent.replace(/\b\d{3}[-.]?\d{3}[-.]?\d{4}\b/g, '[PHONE_REDACTED]');
        }
    ...
    generateExplanation(categories, riskScore) {
        ...
        const severityText = riskScore > 0.8 ? 'High' :
    ...
    getStatistics(timeRangeHours = 24) {
        ...
        const recentResults = this.auditLog.filter(result => Date.now() - new Date(result.request_id).getTime() < cutoff);
        ...
        const avgProcessingTime = recentResults.length > 0
            ? recentResults.reduce((sum, r) => sum + r.processing_time_ms, 0) / recentResults.length
            : 0;
        return {
            total_evaluations: totalEvaluations,
            safe_content_rate: totalEvaluations > 0 ? (safeContent / totalEvaluations) : 1.0,
            flagged_content_rate: totalEvaluations > 0 ? ((totalEvaluations - safeContent) / totalEvaluations) : 0,
    ...
    updateConfig(newConfig) {
        ...
        this.logger.info('Safety pipeline configuration updated', { config: this.config });
    }
    /**
