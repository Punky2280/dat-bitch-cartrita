...
export class BenchmarkHarness extends EventEmitter {
    ...
    generateSummary() {
        ...
        for (const [modelId, results] of modelSummaries) {
            const avgTokensPerSecond = results.reduce((sum, r) => sum + r.metrics.tokens_per_second, 0) / results.length;
            const avgLatencyP95 = results.reduce((sum, r) => sum + r.metrics.latency_p95_ms, 0) / results.length;
            summary[modelId] = {
                total_runs: results.length,
                avg_tokens_per_second: avgTokensPerSecond,
                avg_latency_p95_ms: avgLatencyP95,
                avg_memory_usage_gb: results.reduce((sum, r) => sum + r.metrics.memory_usage_gb, 0) / results.length,
