...
async function testEnhancedDeepgram() {
  ...
  // Load the audio file
  const audioFilePath = path.join(__dirname, '../../deepgram-aura-2-janus-en.wav');
  ...
  for (const agentType of agentTypes) {
    ...
    try {
      ...
      const result = await DeepgramService.transcribeForAgent(audioBuffer, agentType);
      ...
      if (result.topics && result.topics.length > 0) {
        ...
        result.topics.forEach((topic, index) => {
          console.log(`   ${index + 1}. ${topic.topic} (confidence: ${topic.confidence || 'N/A'})`);
        });
      }
      ...
      if (result.intents && result.intents.length > 0) {
        ...
        result.intents.forEach((intent, index) => {
          console.log(`   ${index + 1}. ${intent.intent} (confidence: ${intent.confidence || 'N/A'})`);
        });
      }
      ...
      if (result.entities && result.entities.length > 0) {
        ...
        result.entities.forEach((entity, index) => {
          console.log(`   ${index + 1}. ${entity.label}: "${entity.value}" (confidence: ${entity.confidence || 'N/A'})`);
        });
      }
