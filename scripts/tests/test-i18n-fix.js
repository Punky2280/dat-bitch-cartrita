...
function testReactI18n() {
  ...
  const hasLanguageSelector =
    document.querySelector('[class*="language"]') !== null;
  const hasTranslationElements = document.querySelector('[data-i18n]') !== null;

  console.log(
    `Language selector present: ${hasLanguageSelector ? '✅' : '❌'}`
  );
  console.log(
    `Translation elements present: ${hasTranslationElements ? '✅' : '❌'}`
  );
  ...
  console.error = (...args) => {
    if (
      args.some(
        arg =>
          typeof arg === 'string' &&
          (arg.includes('Promise') ||
            arg.includes('i18n') ||
            arg.includes('translation'))
      )
    ) {
      errorCount++;
    }
...
async function runAllTests() {
  ...
  if (window.i18next) {
    try {
      const testKey =
        window.i18next.t('common.loading') ||
  ...
  const success =
    results.i18nLoaded &&
    Object.values(results.translationLoading).some(r => r.status === 'success');
