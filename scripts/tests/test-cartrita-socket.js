...
class CartritaSocketTester {
    ...
    return new Promise((resolve) => {
      ...
      const responseHandler = (data) => {
        ...
        if (data.text && !data.error) {
          ...
          // Track agent usage
          if (detectedAgent) {
            const currentCount = this.testResults.statistics.agent_usage.get(detectedAgent) || 0;
            this.testResults.statistics.agent_usage.set(detectedAgent, currentCount + 1);
          }
        } else {
        ...
        console.log(`      → Agent: ${detectedAgent || 'unknown'} | Time: ${responseTime}ms | Success: ${!data.error}`);
  ...
  detectUsedAgent(query, response) {
    ...
    if (toolsUsed.includes('getCurrentDateTime') || queryLower.includes('time')) {
      return 'CoreAgent-Time';
    }
    if (toolsUsed.includes('create_art') || queryLower.includes('image') || queryLower.includes('create')) {
      return 'ArtistAgent';
    }
    ...
    if (queryLower.includes('code') || queryLower.includes('function') || queryLower.includes('python')) {
  ...
  async testMCPProtocol() {
    ...
    try {
      ...
      console.log(`   System status: ${response.data.system_status || 'unknown'}\n`);
  ...
  generateStatistics() {
    ...
    const successRate = stats.total_queries > 0 
      ? ((stats.successful_delegations / stats.total_queries) * 100).toFixed(2)
      : 0;
    ...
    const sortedAgents = Array.from(stats.agent_usage.entries())
      .sort((a, b) => b[1] - a[1]);
    ...
    const avgResponseTime = this.testResults.delegation_tests.length > 0
      ? this.testResults.delegation_tests.reduce((sum, test) => sum + test.response_time_ms, 0) / this.testResults.delegation_tests.length
      : 0;
    
    console.log(`Average Response Time: ${avgResponseTime.toFixed(0)}ms`);
    console.log(`Most Used Agent: ${sortedAgents[0]?.[0] || 'None'}`);
    console.log(`Agent Diversity Score: ${stats.agent_usage.size}/${this.testQueries.length} (${((stats.agent_usage.size / this.testQueries.length) * 100).toFixed(1)}%)`);
    
    // Save detailed results
    console.log('\n💾 Saving detailed test results...');
    const fs = require('fs');
    fs.writeFileSync('cartrita-test-results.json', JSON.stringify(this.testResults, null, 2));
