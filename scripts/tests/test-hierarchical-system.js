...
class HierarchicalSystemTester {
  ...
  async testSystemHealth() {
    ...
    try {
      ...
      this.assert(
        ...
        'Health endpoint returned valid status'
      );
      this.assert(
        typeof response.uptime === 'number',
        'Health endpoint returned uptime'
      );
      this.assert(
        response.database && response.database.status === 'connected',
  ...
  async testHierarchicalAgent() {
    ...
    try {
      ...
      this.assert(
        metricsResponse.success === true,
        'Agent metrics endpoint accessible'
      );
      this.assert(
        typeof metricsResponse.metrics === 'object',
        'Agent metrics returned'
      );
      ...
      this.assert(
        ...
        'Agent health endpoint accessible'
      );
      ...
      this.assert(
        toolsResponse.success === true,
        'Agent tools endpoint accessible'
      );
  ...
  async testToolRegistry() {
    ...
    try {
      ...
      const systemTools = tools.filter(
        t => t.name === 'getCurrentDateTime' || t.name === 'getSystemStatus'
      );
      ...
      const agentTools = tools.filter(
        t => t.name === 'create_art' || t.name === 'write_code'
      );
  ...
  async testUserManagementRoutes() {
    ...
    try {
      ...
      this.assert(
        testResponse.message.includes('working'),
        'User test endpoint accessible'
      );
  ...
  async testChatHistoryRoutes() {
    ...
    try {
      ...
      try {
        ...
        this.recordFailure(
          'Chat History',
          'Chat routes should require authentication'
        );
      } catch (error) {
        if (error.message.includes('401') || error.message.includes('403')) {
  ...
  async testWorkflowRoutes() {
    ...
    try {
      ...
      try {
        ...
        this.recordFailure(
          'Workflows',
          'Workflow routes should require authentication'
        );
      } catch (error) {
        if (error.message.includes('401') || error.message.includes('403')) {
  ...
  async testKnowledgeHubRoutes() {
    ...
    try {
      ...
      try {
        ...
        this.recordFailure(
          'Knowledge Hub',
          'Knowledge routes should require authentication'
        );
      } catch (error) {
        if (error.message.includes('401') || error.message.includes('403')) {
          log(
            'SUCCESS',
            'Knowledge Hub routes properly require authentication'
          );
        }
      }
  ...
  async testVaultRoutes() {
    ...
    try {
      ...
      const providersResponse = await this.makeRequest(
        'GET',
        '/api/agent/vault/providers'
      );
      this.assert(
        providersResponse.success === true,
        'Vault providers endpoint accessible'
      );
      this.assert(
        Array.isArray(providersResponse.providers),
        'Providers list returned'
      );
      this.assert(
        providersResponse.providers.length >= 20,
        'Multiple API providers available (19+ preset)'
      );

      // Verify Google provider exists
      const googleProvider = providersResponse.providers.find(
        p => p.name === 'google-ai'
      );
  ...
  async testAgentDelegation() {
    ...
    try {
      ...
      this.assert(
        ...
        'Agent chat endpoint responds'
      );
      this.assert(
        typeof chatResponse.response === 'object',
        'Agent response structure valid'
      );
      this.assert(
        typeof chatResponse.response.text === 'string',
        'Agent response contains text'
      );
  ...
  async testWorkflowExecution() {
    ...
    try {
      ...
      log(
        'SUCCESS',
        'Workflow execution endpoint structure verified (auth required for full test)'
      );
    } catch (error) {
  ...
  async testSocketCommunication() {
    ...
    try {
      ...
      this.assert(
        typeof healthResponse.performance === 'object',
        'Performance metrics available (socket tracking)'
      );
  ...
  async testSystemPerformance() {
    ...
    try {
      ...
      this.assert(
        responseTime < 5000,
        `Response time acceptable (${responseTime}ms < 5000ms)`
      );

      log(
        'SUCCESS',
        `System performance test passed (${responseTime}ms response time)`
      );
    } catch (error) {
  ...
  printFinalResults() {
    ...
    if (this.testResults.failed === 0) {
      log(
        'SUCCESS',
        '🎉 ALL TESTS PASSED! Hierarchical multi-agent system is operational'
      );
    } else {
