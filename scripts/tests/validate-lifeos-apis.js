...
async function testNotificationAPI() {
  ...
  if (statusResult.status === 401) {
    log.success(
      'Notification status endpoint working (auth required as expected)'
    );
  } else if (statusResult.success) {
  ...
  if (notificationsResult.status === 401) {
    log.success(
      'Notifications list endpoint working (auth required as expected)'
    );
  } else {
    log.warn(
      'Notifications list test inconclusive:',
  ...
  if (prefsResult.status === 401) {
    log.success(
      'Notification preferences endpoint working (auth required as expected)'
    );
  } else {
...
async function testPrivacyAPI() {
  ...
  if (dataTypesResult.status === 401) {
    log.success(
      'Privacy data types endpoint working (auth required as expected)'
    );
  } else {
